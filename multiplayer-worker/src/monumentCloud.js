// 纪念丰碑·云同步：把玩家**个人**的丰碑（英灵卡集合）整存进 R2，按 Discord 顺序 UID 私有寻址。
// 与聊天室/助战/交易行共用 Discord 身份（chatToken → cuid）。每人一个 JSON blob：mon/<uid>.json。
// 纯 R2、无 D1（私有单 blob，不需索引/列表/排行）。复用云存档桶 CLOUD_BUCKET。
//
//   GET  /api/monument   (Bearer chatToken)                         → { entries, updatedAt }
//   POST /api/monument   (Bearer chatToken, body={entries,updatedAt}) → { ok, count, updatedAt }
import { verifyChatToken, bearer } from './auth.js';

const MAX_BYTES = 6 * 1024 * 1024;   // 单人丰碑上限 6MB（含压缩立绘；超出请精简英灵/立绘）
...
const keyOf = (uid) => 'mon/' + String(uid) + '.json';

// GET /api/monument  拉取本人云端丰碑
export async function handleMonumentGet(request, env, ch) {
  if (!env.CLOUD_BUCKET) return json({ error: '后端未配置 R2，无法使用丰碑云同步' }, { status: 503 }, ch);
...
// POST /api/monument  整体回传本人丰碑（覆盖云端单 blob）
export async function handleMonumentPut(request, env, ch) {
  if (!env.CLOUD_BUCKET) return json({ error: '后端未配置 R2，无法使用丰碑云同步' }, { status: 503 }, ch);
  const a = await auth(env, request);
  if (!a) return json({ error: '未登录或会话过期，请重新登录 Discord' }, { status: 401 }, ch);
  const raw = await request.text();
  if (raw.length > MAX_BYTES) return json({ error: '丰碑数据过大（上限 6MB），请精简英灵或立绘' }, { status: 413 }, ch);
  let body;
