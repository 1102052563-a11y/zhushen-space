...
本目录已自带一套**过程合成的占位音效（`.wav`，无版权·可商用）**，开音效就能听到。
**想换更好的真素材**：把同名 `.mp3` 丢进本目录即可——引擎按 `mp3 → wav → ogg` 顺序加载，**你的 mp3 会自动覆盖自带的 wav**（缺文件不报错、自动跳过）。
重新生成占位：`node tools/gen-placeholder-audio.mjs`。
