...
import { namesMentionedIn, serializePlayerCard, serializeNpcCard, pickOffsceneRescue, rankNpcsLocal } from './structuredRecall';
...
describe('pickOffsceneRescue（离场角色被正文点名→带记忆救回·有界）', () => {
  ...
  it('已归档（玩家封存）的离场角色被点名 → 不救回（封存优先于「离场救回」，须玩家重新上场）', () => {
    const arc = { ...mk('C1', '龙王', false, 5), archived: true } as unknown as NpcRecord;
    expect(pickOffsceneRescue([arc], '龙王的身影再次浮现', [])).toEqual([]);
  });
...
describe('rankNpcsLocal（本地兜底召回·归档封存排除）', () => {
  const mk = (id: string, name: string, onScene: boolean, archived = false) =>
    ({ id, name, onScene, isDead: false, favor: 0, lastSeenTurn: 0, archived } as unknown as NpcRecord);

  it('在场 NPC 入选', () => {
    expect(rankNpcsLocal([mk('C1', '甲', true)], 5).map((r) => r.id)).toEqual(['C1']);
  });

  it('归档 NPC 被排除召回——即便 onScene 被误置为 true（!archived 硬门，防归档者靠不变量漏洞泄漏进正文）', () => {
    const arc = mk('C2', '乙', true, true);   // 归档=玩家封存，无论在场态都不进召回
    expect(rankNpcsLocal([mk('C1', '甲', true), arc], 5).map((r) => r.id)).toEqual(['C1']);
  });
});
