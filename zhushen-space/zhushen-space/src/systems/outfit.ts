...
/** 穿搭参考图在 imageDb 里的 key。 */
export function outfitImageKey(charId: string, outfitId: string): string { return `outfit:${charId}:${outfitId}`; }
...
export const OUTFIT_REF_HINT = '【参考图】随附一张该角色的钦定穿搭参考图——服装的款式/颜色/材质以图为准（脸型、发色、体型等仍按文字设定）。';
...
export function outfitRosterLine(charId?: string): string {
  const o = activeOutfit(charId);
  if (!o) return '';
  const parts = [`钦定穿搭「${o.name}」（服装以此为准·最高优先）：${o.desc}`];
