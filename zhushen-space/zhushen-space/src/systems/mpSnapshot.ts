...
4. 默认多人共处同一场景与时间线：保持各人位置/状态一致，别对同一事件给两人写出互相矛盾的结果（有人标记分头行动时见第 6 条）。
5. 善用各角色名带出对白与配合，强化组队临场感。
6. **分头行动**：若某玩家的行动带【分头行动·脱离主队独自行动】标记，表示他本回合脱离主队、在别处独自行动——请在**同一份正文里**为他单开一条支线分别描写（去了哪、做了什么、所遇所获、心境），与主队线并行推进；勿强行把他拉回主队同场、也勿无视。等他不再带分头标记（汇合）时，让其支线见闻与主线**自然汇合并产生联动**。**全程只有你这一份客观正文**，绝不为不同玩家写出互相矛盾的世界/NPC 生死或状态——这正是分头行动必须由你统一执笔的原因。`;
