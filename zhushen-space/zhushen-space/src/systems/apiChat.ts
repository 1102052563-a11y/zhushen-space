...
export function narrativeLangDirective(): string {
  ...
  return `【输出语言＝${name}·仅本回合正文】本回合正文的一切**自然语言文字**（叙述、对白、心理、旁白、场景与动作描写，以及文中出现的人物/物品/技能/地点等名称与其描述）一律用 ${name} 书写。`
    ...
    + `⚠**特别强调（系统播报/结算/公告块）**：即便世界书/预设里这类块的模板是用中文写的，你也**必须**把块内的**说明句、旁白、状态描述、提示文字**改写成 ${name}——只保留【模块标题关键词】和上面②的枚举词/③的字段名为中文。`
    + `例：时间结算块里「时间结算：+30分钟」→标题词与单位保留、其余照常；但「准备时间已结束」「即将进行首次试炼」「初始试炼强化难度」这类**描述句一律写成 ${name}**，不得整段照抄中文模板。`
