...
function hydrate() { if (_hydrated) return; _hydrated = true; try { const raw = localStorage.getItem(KEY); if (raw) core.restore(JSON.parse(raw)); } catch { /* */ } }
...
export function npcCoreReset(): void { core.reset(); _hydrated = true; persist(); }
export function npcCoreSnapshot() { hydrate(); return core.snapshot(); }
export function npcCoreRestore(s: Parameters<typeof core.restore>[0]): void { core.restore(s); _hydrated = true; }
