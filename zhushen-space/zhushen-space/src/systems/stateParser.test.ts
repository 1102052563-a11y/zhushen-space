...
describe('updateItem/updateItemQuantity 身份安全（Phase 3·幻觉 id 不改错/不扣错隔壁那件）', () => {
  const run = (s: string) => applyItemCommands(parseAllItemCommands(`<upstore>${s}</upstore>`));
  const seedTwoWeapons = () => useItems.setState({ items: [
    { id: 'I_B1_01', name: '寒铁长剑', category: '武器', gradeDesc: '蓝色', quantity: 1, effect: '', affix: '', equipped: false, tags: [], addedAt: 0 },
    { id: 'I_B1_02', name: '玄铁巨斧', category: '武器', gradeDesc: '蓝色', quantity: 1, effect: '', affix: '', equipped: false, tags: [], addedAt: 0 },
  ] as any });

  it('★updateItem 写错 itemId(指向斧) 但 name 是剑 → 改到剑、绝不动斧', () => {
    seedTwoWeapons();
    run('updateItem({"itemId":"I_B1_02","name":"寒铁长剑","patch":{"affix":"【裂空】：斩击附带真伤"}})');
    const items = useItems.getState().items;
    expect(items.find((i) => i.name === '寒铁长剑')!.affix).toContain('裂空');   // 剑被改
    expect(items.find((i) => i.name === '玄铁巨斧')!.affix).toBe('');             // 斧未被误改
  });

  it('updateItem 正确 id+name → 正常落地', () => {
    seedTwoWeapons();
    run('updateItem({"itemId":"I_B1_01","name":"寒铁长剑","patch":{"combatStat":"攻击力 120"}})');
    expect(useItems.getState().items.find((i) => i.name === '寒铁长剑')!.combatStat).toBe('攻击力 120');
  });

  it('★updateItemQuantity 写错 itemId 但 name 对 → 扣对那件、不扣错隔壁', () => {
    useItems.setState({ items: [
      { id: 'I_B1_01', name: '止血喷雾', category: '消耗品', gradeDesc: '白色', quantity: 5, effect: '', equipped: false, tags: [], addedAt: 0 },
      { id: 'I_B1_02', name: '解毒剂', category: '消耗品', gradeDesc: '白色', quantity: 9, effect: '', equipped: false, tags: [], addedAt: 0 },
    ] as any });
    run('updateItemQuantity({"itemId":"I_B1_02","name":"止血喷雾","newQuantity":2})');
    const items = useItems.getState().items;
    expect(items.find((i) => i.name === '止血喷雾')!.quantity).toBe(2);   // 改对
    expect(items.find((i) => i.name === '解毒剂')!.quantity).toBe(9);     // 隔壁未被错改
  });
});
