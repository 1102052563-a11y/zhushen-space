...
import { effectiveResource, lvFromRealm, fullMaxHp, fullMaxEp, computeDerived, realAttrMult, attrCapForTier, ratioOf, hpPerConOf, epPerIntOf } from './derivedStats';
...
export function serializePlayerCard(
  ...
  const pRatio = ratioOf(profile);   // 主角自定义「体质→HP / 智力→EP」转化比（空=默认 20/15）
  const pHpr = hpPerConOf(pRatio), pEpr = epPerIntOf(pRatio);
  ...
  const stat = [
    `HP:${effectiveResource(game.hp, game.maxHp, pMaxHp)}/${pMaxHp}（满状态上限=${pMaxHp}，已含体质×${pHpr}（四阶起真实属性×5＝体质×${pHpr * 5}）及天赋/装备/技能/技能树/团队全部加成，前端实算；回满/满血即回到 ${pMaxHp}，勿按基础体质自行重算、勿沿用历史旧上限${profile.hpLabel ? `；正文叙述称「${profile.hpLabel}」，状态行/指令仍写 HP` : ''}）`,
    `EP:${effectiveResource(game.mp, game.maxMp, pMaxEp)}/${pMaxEp}（满状态上限=${pMaxEp}，已含智力×${pEpr}（四阶起×5＝智力×${pEpr * 5}）及天赋/装备/技能/技能树/团队全部加成，前端实算；回满即回到 ${pMaxEp}，勿自行重算、勿沿用历史旧上限${profile.epLabel ? `；正文叙述称「${profile.epLabel}」，状态行/指令仍写 EP` : ''}）`,
...
export function serializeNpcCard(
  ...
  const nRatio = ratioOf(npc);   // NPC 自定义转化比（空=默认 20/15）
  const nHpr = hpPerConOf(nRatio), nEpr = epPerIntOf(nRatio);
  const nMaxHp = a ? fullMaxHp(a, nEqp, skills, talents, rmN, nRatio) : 0;
  const nMaxEp = a ? fullMaxEp(a, nEqp, skills, talents, rmN, nRatio) : 0;
  ...
  const stat = [
    a && `HP:${effectiveResource(npc.hp, npc.maxHp, nMaxHp)}/${nMaxHp}（上限=体质×${nHpr}，自动算）`,
    a && `EP:${effectiveResource(npc.mp, npc.maxMp, nMaxEp)}/${nMaxEp}（上限=智力×${nEpr}，自动算）`,
