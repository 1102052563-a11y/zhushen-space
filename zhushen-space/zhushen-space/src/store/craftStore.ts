...
const DEFAULT_CRAFT_WB: WorldBook = {
  ...
  entries: [
    ...
    cwbEntry(7, '炼丹药理', '炼丹讲君臣佐使：主药定效、辅药增幅、佐药调和、使药引导；药性相冲（水火/生克）会废丹甚至炸炉，火候过猛生丹毒。产物是丹药/药剂（恢复/增益/解毒/淬体），效果与持续写进 effect。名贵药材才出高阶丹，凡草只能凑普通药剂。', ['炼丹', '丹炉', '草药', '药性', '火候', '丹毒', '药剂', '丹', '灵药']),
    cwbEntry(8, '符箓咒纹', '符箓＝把某种本源之力封进承载媒介（符纸/玉简/卡片）。灵墨与媒介要能承载对应能量，能量越猛越需高阶符纸，否则一触即溃。产物多为一次性消耗品（触发即耗），高阶可做可复用的符宝。效果写清触发方式与作用。', ['符箓', '符纸', '灵墨', '卡片', '咒', '本源', '阵纹', '符']),
    ...
    cwbEntry(11, '炼器机关', '炼器造的是机关/傀儡/道具/召唤物（主神空间科技造物）。结构与动力要自洽：核心供能、零件成形、能源驱动。产物功能由零件合理推导（推进器→机动、护盾发生器→防御、傀儡核心→可召唤的战斗傀儡）。能源不足则出半成品废件。', ['炼器', '机关', '傀儡', '造物', '核心', '能源', '齿轮', '召唤', '装置']),
    cwbEntry(12, '御兽契灵', '御兽是把精魂/兽核与契约媒介结契，孕育出一只可随行的**宠物生灵**（不是道具、不是凭证）。精魂强度、血脉纯度、媒介品阶共同决定宠物的种族、形态与天赋；结契不稳则得残缺或狂暴之物。**请把产物当一只活物来写**：name=宠物名、subType=种族、intro=性情性格、appearance=外观、effect/affix=天赋与能力、combatStat=可选的战力概述；gradeDesc 表示这只宠物的品阶。', ['御兽', '契灵', '精魂', '随从', '宠物', '契约', '血脉', '兽核']),
