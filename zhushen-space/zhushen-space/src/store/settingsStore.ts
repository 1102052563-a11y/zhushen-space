...
interface SettingsState {
  ...
  audio: { enabled: boolean; volume: number; ambient: boolean; ambientVolume: number; music: boolean; musicVolume: number; musicShuffle: boolean; musicConsent: '' | 'granted' };   // 游戏音效（开关/总音量0~1/环境音开关+音量/背景音乐开关+音量+随机播放/音乐流量确认：''=未确认先不下载·granted=用户同意后才加载）
  setAudio: (patch: Partial<{ enabled: boolean; volume: number; ambient: boolean; ambientVolume: number; music: boolean; musicVolume: number; musicShuffle: boolean; musicConsent: '' | 'granted' }>) => void;
...
export const useSettings = create<SettingsState>()(
  persist(
    (set, get): SettingsState => ({
      ...
      weatherFx: true,
      audio: { enabled: true, volume: 0.7, ambient: true, ambientVolume: 0.4, music: true, musicVolume: 0.5, musicShuffle: false, musicConsent: '' },
