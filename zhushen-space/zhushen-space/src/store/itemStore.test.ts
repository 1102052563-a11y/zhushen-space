...
import { normalizeGradeLabel, scoreToGradeNum, useItems, getItemLog, clearItemLog, formatItemLine } from './itemStore';
...
describe('formatItemLine（物品演化注入·含逻辑关键字段的完整视图，非仅摘要）', () => {
  it('装备类含 词缀/攻防/评分/强化/杀敌/锁定/镶嵌 全字段', () => {
    const line = formatItemLine({
      id: 'I_B1_03', name: '寒铁长剑', category: '武器', gradeDesc: '暗金', quantity: 1,
      equipped: true, equipSlot: 'weapon', enhanceLevel: 12, locked: true,
      combatStat: '攻击力 450', affix: '【裂空】：斩击附带真伤', score: '480', killCount: '37',
      durability: '90/90', gems: [{}, {}], effect: '斩杀低血敌人',
    });
    expect(line).toContain('[I_B1_03] 寒铁长剑（武器·暗金）×1');
    expect(line).toContain('【已装备:weapon】');
    expect(line).toContain('+12');
    expect(line).toContain('🔒锁定');
    expect(line).toContain('攻防:攻击力 450');
    expect(line).toContain('词缀:【裂空】：斩击附带真伤');
    expect(line).toContain('评分:480');
    expect(line).toContain('杀敌:37');
    expect(line).toContain('镶嵌:2颗');
    expect(line).toContain('效果:斩杀低血敌人');
  });

  it('缺省字段一律省略（不输出空的攻防/词缀 / 不输出 +0）', () => {
    const line = formatItemLine({ id: 'X', name: '止血喷雾', category: '消耗品', gradeDesc: '白色', quantity: 5, effect: '回血' });
    expect(line).toBe('[X] 止血喷雾（消耗品·白色）×5  效果:回血');
    expect(line).not.toContain('攻防');
    expect(line).not.toContain('词缀');
