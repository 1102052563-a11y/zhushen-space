...
function ApiSection() {
  ...
  const tApi       = useTerritory((s) => s.territoryApi);
  const useShared  = useTerritory((s) => s.territoryUseSharedApi);
  const models     = useTerritory((s) => s.territoryAvailableModels);
  const loading    = useTerritory((s) => s.territoryModelsLoading);
  const error      = useTerritory((s) => s.territoryModelsError);
  const setApi     = useTerritory((s) => s.setTerritoryApi);
  const setUseShared = useTerritory((s) => s.setTerritoryUseSharedApi);
  const fetchModels  = useTerritory((s) => s.fetchTerritoryModels);
    ...
    <div className="space-y-6 max-w-xl">
      <div className="flex items-center gap-3 p-3 bg-panel border border-edge rounded-lg">
        ...
        <div>
          ...
          <div className="text-sm text-dim mt-0.5">开启时复用正文/世界选择 API；关闭则为领地演化单独配置</div>
        </div>
      </div>
      ...
      {!useShared && (
        <div className="space-y-4">
          <Field label="API 地址">
            <input type="text" value={tApi.baseUrl} onChange={(e) => setApi({ baseUrl: e.target.value })} placeholder="https://api.openai.com/v1" className="input-base" />
          </Field>
          <Field label="API Key">
            <input type="password" value={tApi.apiKey} onChange={(e) => setApi({ apiKey: e.target.value })} placeholder="sk-..." className="input-base font-mono" />
          </Field>
          <Field label="模型">
            <div className="flex gap-2">
              {models.length > 0 ? (
                <select value={tApi.modelId} onChange={(e) => setApi({ modelId: e.target.value })} className="input-base flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <Field label={`Top-P (${tApi.topP})`}>
              <input type="range" min={0} max={1} step={0.05} value={tApi.topP} onChange={(e) => setApi({ topP: parseFloat(e.target.value) })} className="w-full accent-god mt-1" />
            </Field>
            <Field label="Max Tokens">
              <input type="number" value={tApi.maxTokens} onChange={(e) => setApi({ maxTokens: parseInt(e.target.value) || 512 })} min={128} max={32768} step={128} className="input-base" />
            </Field>
          </div>
        </div>
      )}
