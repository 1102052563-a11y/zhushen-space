...
function ApiSection() {
  ...
  const useShared  = useMisc((s) => s.miscUseSharedApi);
  const models     = useMisc((s) => s.miscAvailableModels);
  const loading    = useMisc((s) => s.miscModelsLoading);
  const error      = useMisc((s) => s.miscModelsError);
  const setMiscApi = useMisc((s) => s.setMiscApi);
  const setUseShared = useMisc((s) => s.setMiscUseSharedApi);
  const fetchModels  = useMisc((s) => s.fetchMiscModels);

  const effective = useShared ? (textUseSharedApi ? api : textApi) : miscApi;
    ...
    <div className="space-y-6 max-w-xl">
      <div className="flex items-center gap-3 p-3 bg-panel border border-edge rounded-lg">
        ...
        <div>
          ...
          <div className="text-sm text-dim mt-0.5">开启时复用正文/世界选择 API；关闭则为杂项演化单独配置</div>
        </div>
      </div>
      ...
      {!useShared && (
        <div className="space-y-4">
          <Field label="API 地址">
            <input type="text" value={miscApi.baseUrl} onChange={(e) => setMiscApi({ baseUrl: e.target.value })} placeholder="https://api.openai.com/v1" className="input-base" />
          </Field>
          <Field label="API Key">
            <input type="password" value={miscApi.apiKey} onChange={(e) => setMiscApi({ apiKey: e.target.value })} placeholder="sk-..." className="input-base font-mono" />
          </Field>
          <Field label="模型">
            <div className="flex gap-2">
              {models.length > 0 ? (
                <select value={miscApi.modelId} onChange={(e) => setMiscApi({ modelId: e.target.value })} className="input-base flex-1">
                  {models.map((m) => <option key={m} value={m}>{m}</option>)}
                </select>
              ) : (
                <input type="text" value={miscApi.modelId} onChange={(e) => setMiscApi({ modelId: e.target.value })} placeholder="gpt-4o" className="input-base flex-1 font-mono" />
              )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Field label={`温度 (${miscApi.temperature})`}>
              <input type="range" min={0} max={2} step={0.05} value={miscApi.temperature} onChange={(e) => setMiscApi({ temperature: parseFloat(e.target.value) })} className="w-full accent-god mt-1" />
            </Field>
            <Field label={`Top-P (${miscApi.topP})`}>
              <input type="range" min={0} max={1} step={0.05} value={miscApi.topP} onChange={(e) => setMiscApi({ topP: parseFloat(e.target.value) })} className="w-full accent-god mt-1" />
            </Field>
            <Field label="Max Tokens">
              <input type="number" value={miscApi.maxTokens} onChange={(e) => setMiscApi({ maxTokens: parseInt(e.target.value) || 512 })} min={128} max={32768} step={128} className="input-base" />
            </Field>
          </div>
        </div>
      )}
