...
/* 公会面板🏰（右导航）——无公会：创建 + 浏览/申请；有公会：名册/军衔 · 贡献→等级→增益 · 仓库 · 编年史 · 退出/解散。
   见 指导/家族系统-设计.md。异步公会·账号级（guildStore.my 持久化）。⚠受控输入子组件全模块级（防输入法重挂）。*/
...
function CreateGuildForm() {
  ...
  return (
    <div className="rounded-xl border border-amber-500/25 bg-panel p-4 space-y-2.5">
      <div className="text-sm font-bold text-amber-100">🏰 创立公会</div>
      <div className="grid grid-cols-2 gap-2">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="公会名（如 无间战队）" className={`${inputCls} w-full`} />
      ...
      <div className="flex gap-2">
        ...
        <input value={manifesto} onChange={(e) => setManifesto(e.target.value)} placeholder="公会宣言（一句话）" className={`${inputCls} flex-1`} />
      </div>
      <button onClick={create} disabled={!name.trim() || !tag.trim()} className={`${btnPrimary} w-full ${(!name.trim() || !tag.trim()) ? 'opacity-40' : ''}`}>创立公会（你为会长）</button>
    </div>
  );
}
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="搜索公会名 / 标签" className={`${inputCls} flex-1`} />
      ...
      {ranked.length === 0
        ? <div className="text-center text-dim/50 text-sm py-10">还没有公会，去上面创立第一个。</div>
...
function RosterRow({ m, myRole, myPid, weekly }: { m: GuildMember; myRole: GuildRank; myPid: string | undefined; weekly: boolean }) {
  ...
  return (
    <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg border border-edge bg-panel/50">
      ...
      {canManage && <button onClick={() => { if (confirm(`将「${m.name}」移出公会？`)) guildClient.kick(m.pid); }} className="text-blood/60 hover:text-blood text-sm px-1 shrink-0" title="踢出">✕</button>}
    </div>
  );
}

/** 仓库物品副信息行（只读字符串/数字字段·避免 affix 对象渲染成 [object Object]）。 */
function chestInfoLine(c: any): string {
  const bits: string[] = [];
  const sub = typeof c.subType === 'string' ? c.subType : (typeof c.category === 'string' ? c.category : '');
  if (sub) bits.push(sub);
  if (typeof c.combatStat === 'string' && c.combatStat) bits.push(c.combatStat);
  if (typeof c.score === 'number' && c.score) bits.push(`评分 ${c.score}`);
  if (typeof c.quantity === 'number' && c.quantity > 1) bits.push(`×${c.quantity}`);
  const eff = typeof c.effect === 'string' ? c.effect : '';
  return (bits.join(' · ') + (eff ? ` · ${eff}` : '')).trim();
}
...
function ChestSection() {
  ...
  const deposit = () => {
    ...
    guildClient.deposit({ ...it });   // ⚠存入公会仓库带**完整物品快照**（词缀/效果/评分/数量全保留·防丢·同账户仓库铁则）
    removeItem(it.id); setPick('');
  };
  const withdraw = (i: number) => { const c = chest[i]; if (!c) return; const { _by, _at, ...item } = c; addItem({ ...item, acquisition: '公会仓库' } as any); guildClient.withdraw(i); };   // 取出=完整还原
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        <select value={pick} onChange={(e) => setPick(e.target.value)} className={`${inputCls} flex-1`}>
          <option value="">从储存空间选物品存入公会仓库…</option>
          {donatable.map((it) => <option key={it.id} value={it.id}>{it.name}{it.gradeDesc ? `（${it.gradeDesc}）` : ''}</option>)}
        </select>
        <button onClick={deposit} disabled={!pick} className={`${btnPrimary} ${!pick ? 'opacity-40' : ''}`}>存入</button>
      </div>
      <div className="text-[10px] font-mono text-dim/40">存入后**完整保留**词缀 / 效果 / 评分 / 数量；公会成员可取出（装备中 / 已锁定的物品不可存）。</div>
      {chest.length === 0
        ? <div className="text-center text-dim/45 text-[12px] py-4">仓库空空——从储存空间存入第一件物品吧。</div>
        : <div className="space-y-1">
            {chest.map((c: any, i: number) => {
              const info = chestInfoLine(c);
              return (
                <div key={i} className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg border border-edge bg-panel/50">
                  <div className="flex-1 min-w-0">
                    <div className="text-sm text-slate-100 truncate">{c.name || '物品'}{c.gradeDesc ? ` · ${c.gradeDesc}` : ''}</div>
                    {info && <div className="text-[11px] text-dim/55 truncate">{info}</div>}
                  </div>
                  {c._by && <span className="text-[10px] font-mono text-dim/45 shrink-0" title="存入者">{c._by}</span>}
...
function ChainBar() {
  ...
  return (
    <div className="rounded-xl border border-orange-500/25 bg-panel p-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-sm font-bold text-orange-100">公会连击 {chain.count} 连{!alive && <span className="ml-1 text-[10px] text-dim/40">已断</span>}</div>
...
function WeekTaskSection() {
  ...
  return (
    <div className="rounded-xl border border-emerald-500/20 bg-panel p-3 space-y-2">
      <div className="flex items-center gap-2">
        <div className="text-[13px] font-bold text-slate-100">📅 本周公会任务</div>
...
/* 公会战·确定性对决：种子=我族id+对手id+当日 → 同对手同日结果固定（防 reroll）。战力对决 + ±30% 种子扰动。 */
...
function WarSection({ cards, my }: { cards: GuildCard[]; my: GuildSummary }) {
  ...
  const declareWar = (opp: GuildCard) => {
    ...
    setResult(`${win ? '⚔️ 大胜' : '💢 战败'}「${opp.name}」 · 我族 ${myScore} vs ${theirScore}${win ? '（公会贡献 +）' : ''}`);
  };
  return (
    <div className="space-y-1.5">
      <div className="flex items-center gap-2">
        <div className="text-[13px] font-bold text-slate-100">⚔️ 公会战</div>
      ...
      {opponents.length === 0
        ? <div className="text-[11px] text-dim/45 px-1">暂无其他公会可宣战。</div>
        : <div className="space-y-1">
...
function BaseSection({ onGenerateBuildings, canManage }: { onGenerateBuildings?: (p: string) => void | Promise<void>; canManage: boolean }) {
  ...
  const upgrade = (b: any) => {
    ...
    adjustCurrency('乐园币', -cost, `公会·${my.name}·据点·${b.name}`);
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        <div className="text-[13px] font-bold text-slate-100">🏯 公会据点</div>
        <div className="flex-1" />
    ...
    <div className="fixed inset-0 z-[85] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="w-full max-w-md rounded-2xl border border-amber-500/30 bg-void shadow-[0_0_50px_rgba(0,0,0,0.85)] flex flex-col">
        <header className="shrink-0 flex items-center gap-2 px-4 py-3 border-b border-amber-500/20 bg-panel">
          ...
          <div className="flex-1 text-sm font-bold text-amber-100">公会设置</div>
          <button onClick={onClose} className="text-dim/50 hover:text-blood text-lg">✕</button>
        </header>
        <div className="p-4 space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <label className="flex flex-col gap-1"><span className="text-[11px] font-mono text-dim/55">公会名</span><input value={name} onChange={(e) => setName(e.target.value)} className={`${inputCls} w-full`} /></label>
          ...
          <label className="flex flex-col gap-1"><span className="text-[11px] font-mono text-dim/55">公会宣言</span><textarea value={manifesto} onChange={(e) => setManifesto(e.target.value)} rows={2} className="w-full bg-void border border-edge rounded-lg px-3 py-2 text-[13px] text-slate-200 resize-y focus:outline-none focus:border-amber-400/40" /></label>
...
function MyGuildView({ cards, onGenerateBuildings }: { cards: GuildCard[]; onGenerateBuildings?: (p: string) => void | Promise<void> }) {
  ...
  const doDonate = () => {
    ...
    adjustCurrency('乐园币', -amt, `公会·${my.name}·捐献`);
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-amber-500/25 bg-panel p-4">
        <div className="flex items-center gap-3">
          ...
          {my.role === 'leader' && <button onClick={() => setSettingsOpen(true)} className="text-lg text-dim/50 hover:text-amber-200 shrink-0" title="公会设置">⚙</button>}
        </div>
        <div className="mt-3">
          <div className="flex items-center justify-between text-[11px] font-mono mb-1">
            <span className="text-amber-300/70">公会等级 Lv.{my.level}</span>
      ...
      <div className="rounded-xl border border-edge bg-panel p-3 space-y-2">
        <div className="text-[13px] font-bold text-slate-100">捐献乐园币 → 公会贡献 / 升级</div>
        <div className="flex items-center gap-2">
      ...
      {/* 公会战 · 本周贡献榜（各公会本周贡献 PK） */}
      {cards.length > 1 && (
        <div className="space-y-1.5">
          <div className="text-[13px] font-bold text-slate-100">⚔️ 公会战 · 本周榜</div>
      ...
      {/* 仓库 */}
      <div className="space-y-1.5">
        <div className="text-[13px] font-bold text-slate-100">📦 公会仓库</div>
        <ChestSection />
      </div>
      ...
      {chronicle.length > 0 && (
        <div className="space-y-1.5">
          <div className="text-[13px] font-bold text-slate-100">📜 公会编年史</div>
      ...
      {hallOfFame.length > 0 && (
        <div className="space-y-1.5">
          <div className="text-[13px] font-bold text-slate-100">🪦 公会丰碑</div>
      ...
      <div className="flex items-center gap-2 pt-1">
        <div className="flex-1" />
        {my.role === 'leader'
          ? <button onClick={() => { if (confirm('解散公会？此操作不可撤销，所有成员将退出。')) guildClient.disband(); }} className="text-[12px] font-mono py-1.5 px-3 rounded-lg border border-blood/40 text-blood/80 hover:bg-blood/10">解散公会</button>
          : <button onClick={() => { if (confirm('退出当前公会？')) guildClient.leave(); }} className={btnGhost}>退出公会</button>}
      </div>
...
/** 每日公会津贴（stipend perk 生效）：有该 perk 且今日未领 → 发乐园币 + 场外通报。每日一次（localStorage 日期门）。 */
function claimDailyStipend() {
  try {
    ...
    useItems.getState().adjustCurrency('乐园币', amt, `公会·${my.name}·每日津贴`);
    pushSceneNotice(`【场外·公会】今日公会津贴 +${amt} 乐园币（公会「${my.name}」Lv.${my.level} 增益）`);
  } catch { /* 非阻塞 */ }
}
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-void/95 backdrop-blur-sm flex flex-col">
      <header className="shrink-0 flex items-center gap-2 px-4 py-3 border-b border-amber-500/20 bg-panel">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-bold text-amber-100">公会</div>
          <div className="text-[11px] font-mono text-amber-300/50">{my ? `${my.name} [${my.tag}]` : '异步契约者战队 · 归属 / 共享进度 / 公会增益'}</div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {gated
          ? <div className="text-center text-dim/50 text-sm py-16">公会需先登录 Discord（聊天室 / 联机身份）。</div>
          : my
            ? <MyGuildView cards={cards} onGenerateBuildings={onGenerateBuildings} />
            : <div className="space-y-4">
                <CreateGuildForm />
                <div className="text-[12px] font-mono text-dim/55">{status === 'connected' ? `🏆 公会总榜（${cards.length} 个公会·按等级排名）` : status === 'connecting' ? '连接中…' : ''}</div>
