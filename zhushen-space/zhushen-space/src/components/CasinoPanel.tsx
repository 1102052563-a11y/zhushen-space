...
export default function CasinoPanel({ onClose, onGenMatch, onGenBattle, onGenRewards, onBanter, onGenSoul, onGenPortraits }: {
  ...
  function buyChips() {
    ...
    adjustCurrency(coinKey, -q.spend, `赌坊·兑换筹码（${q.chips} ${chipName}）`);
  ...
  function cashOut() {
    ...
    cas.getState().addChips(kind, -q.chips);
    adjustCurrency(coinKey, q.coins, `赌坊·筹码兑现（${q.chips} ${chipName}）`);
  ...
  return (
    <div className="fixed inset-0 z-[65] bg-black/70 backdrop-blur-sm flex items-center justify-center p-3"
      ...
      <div className="w-full max-w-5xl h-[92dvh] rounded-2xl border border-edge bg-void shadow-[0_0_60px_rgba(0,0,0,0.9)] overflow-hidden flex flex-col">
