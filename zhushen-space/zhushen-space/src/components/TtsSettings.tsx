  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-xl border border-edge bg-void/95 p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex gap-2 flex-wrap">
            {(['webspeech', 'edge'] as const).map((e) => (
              <button key={e} onClick={() => tts.set({ engine: e })}
                ...
                {e === 'webspeech' ? '🔈 本地（离线免费）' : '☁️ Edge 云端（更好听·需部署）'}
              </button>
            ))}
          </div>
        </div>
        ...
        {tts.engine === 'edge' && (
          <div className="mt-4 text-[12px] text-amber-300/70">☁️ Edge 云端需先部署 multiplayer-worker，否则朗读无声；本地引擎离线即用。</div>
        )}
