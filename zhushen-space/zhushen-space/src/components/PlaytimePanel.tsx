...
/* 游玩时长 · 排行榜面板：凡 Discord 登录者自动累计"活跃游玩"时长，这里看自己时长+名次 + 全服排行榜。 */
...
export default function PlaytimePanel({ onClose }: { onClose: () => void }) {
  ...
  const [board, setBoard] = useState<{ items: PlaytimeTopEntry[]; players: number }>({ items: [], players: 0 });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-void border border-edge rounded-2xl w-full max-w-lg max-h-[88dvh] flex flex-col shadow-[0_0_60px_rgba(0,0,0,0.8)] overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {logged ? (
            ...
            <div className="rounded-xl border border-edge bg-panel p-4 text-center text-[13px] text-dim/70 leading-relaxed">
              登录 <span className="text-god">Discord</span> 后，你的游玩时长会自动记录并上榜。<br />
              <span className="text-[12px] text-dim/50">（在「💬 聊天室」或「☁️ 云存档」处登录）</span>
            </div>
          )}
          ...
          <div>
            <div className="flex items-center gap-2 mb-2 px-1">
              ...
              <span className="text-[11px] font-mono text-dim/40">共 {board.players} 人</span>
        ...
        <footer className="shrink-0 px-4 py-2 border-t border-edge bg-panel text-[11px] font-mono text-dim/40 text-center">
          时长按"页面可见的活跃游玩"每分钟累计 · 切后台挂机不计
        </footer>
      </div>
    </div>
  );
