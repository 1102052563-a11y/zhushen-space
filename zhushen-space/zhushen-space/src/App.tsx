...
import { apiChatFallback, fetchWithProxy, abortAllApiCalls, narrativeLangDirective, orderedKeys, markKeyBad, markKeyGood, isKeyRotatableStatus } from './systems/apiChat';
import { applyOmitParams, buildProtocolRequest, protocolOf, extractStreamDelta, extractOnceContent } from './systems/apiUrl';
...
import { expandPromptText, makePromptExpandCtx } from './systems/promptExpand';   // 统一展开：表格模板（<if var/cell/seed/cond>/计算标签）→ ST 宏
import { passActiveWhen } from './systems/tableTemplate';   // 🎯 activeWhen 条件门（世界书条目/自定义注入·<if cond> 语法）
import { applyCustomInjects } from './systems/customInject';   // 🎯 自定义注入：预设中心条目 → 最终 messages（发送前最后一步）
import { renderPhaseExpand } from './systems/renderVars';   // 🧩 渲染期变量：楼层显示时就地替换 {{getvar}}/<if var>（默认关·仅显示层）
...
import { runRosterGate, localNameScan, type RosterGateItem } from './systems/rosterGate';
import { TIME_ANCHOR_RULE, WORLD_EVENT_VISIBILITY_RULE, WORLD_NEWS_RULE, NPC_INNER_VOICE_RULE, CHANNEL_WORLD_POOL_RULE, CHANNEL_ANTI_SLOP_RULE, WORLD_PAPER_RULE, SPECTATOR_CHATTER_RULE, DM_REPLY_PROTOCOL_RULE, GROUP_CHAT_RULE, NPC_EVO_GATE_RULE, PARADISE_SEARCH_RULE } from './promptRules';
...
import { realmFromLevel, normalizeTier, lvFromRealm, computeMaxHp, computeMaxEp, effectiveResource, attrCapForTier, clampBaseAttrs, fullMaxHp, fullMaxEp, TIERS, TIER_LEVEL_RANGE, realAttrMult, ratioOf, npcBaseAttrs, isAbyssLocked } from './systems/derivedStats';
...
import { drawPool, divinationSeed, buildDivinationInjection, hasDivinationLeak, scrubDivination } from './systems/divination';
import {
  worldRumors, serializeRumorsForEvo, buildRumorInjection, needsCompress, compressRumor,
  shouldPromote, pruneRumors, latestNode, seedRumorsFromWorldCard, type Rumor,
} from './systems/rumor';
import {
...
import { outfitRosterLine, outfitRefImages, OUTFIT_REF_HINT } from './systems/outfit';
...
import { buildVolumes, buildCompileInput, sanitizeCompiled, buildPriorSaga, ORPHAN_VOLUME } from './systems/chronicle';
...
import { useMisc, buildMiscSystemPrompt, isMainQuest, type WorldEvent } from './store/miscStore';
...
import { recordMpReward, clearMpPending, promoteMpPending, discardMpReplay, replayMpPendingOnBoot } from './systems/mpPendingRewards';   // 联机来宾·奖励防回滚（离房 loadSlot 还原会吞掉在房入账）
import { reportFacilityOutcome, peekFacilityLog, clearFacilityLog } from './systems/facilityBridge';   // 设施→正文统一接入口（通报+发放登记+成长交代一次扇出）+ 洞察设施区数据源
import { buildCastBriefInput } from './systems/castBrief';   // 角色立场简报·输入侧：给细纲/推进喂只含人格的瘦档案（治配角同质化/OOC/谄媚）
import { runMindflowBatch } from './systems/mindflow';   // 🫀 逐角色心流独白（借鉴SoulLink·默认关）：发送时并发起跑，正文组装时限时收割
import { getPrompt, renderPrompt, setLastUserMessage, usePromptOverride } from './store/promptOverrideStore';   // 预设中心：主提示词 override（有玩家自定义用之，否则回退内置默认常量）；renderPrompt=给 field 类补一遍变量标签替换；setLastUserMessage=记玩家当前输入供 {{lastUserMessage}}   // 场外操作通报→正文前置须知；星图习得/开箱→入戏交代块；设施发放物→物品阶段勿再建
import { rollGemDrops, hasHighTierKill } from './systems/gemDrop';   // 正文击杀 → 结算掉落宝石（前端确定性）
import { guildPerkValue } from './store/guildStore';   // 公会增益数值（dropRate/settlement…前端逐期接；服务端只判解锁）
...
const LivePanel = lazy(() => import('./components/LivePanel'));   // 📺 乐园直播间（借鉴Abstract外置手机 live·弹幕+礼物分档演出）
...
const OutfitWorkshop = lazy(() => import('./components/OutfitWorkshop'));   // 👗 形象工坊（每角色衣柜·试衣·生图提示词·成衣库）
...
import { parseGroupReply, groupMsgToHistoryText } from './systems/groupChat';   // 👥 群聊行协议（借鉴Abstract外置手机）
import { buildStickerInjection, stickerNameSet } from './store/stickerStore';   // 😊 表情包库（名称注入+消费侧硬过滤）
import { parseSearchReply, searchCandidateBlock } from './systems/paradiseSearch';   // 🔎 乐园检索（借鉴Abstract外置手机 browser）
import { buildEavesdropSys, rollDiscovered, eavesdropNotice, EAVESDROP_COST, EAVESDROP_FAVOR_HIT } from './systems/eavesdrop';   // 🕳 窃听（借鉴Abstract外置手机 peek）
...
import { getCanonStation, enforceCanonLock, captureCanonSettlement, suxiaoEntryStats as canonSuxiaoEntryStats, upsertSuxiaoNpc } from './systems/canonRoute';
const CanonRoutePanel = lazy(() => import('./components/CanonRoutePanel'));
...
import StoryStrip from './components/StoryStrip';   // 楼层信息条：贴在最新正文末尾（时间·天气/任务/伏笔）；零 props+memo，App 重渲不碰它
import BookmarkButton from './components/BookmarkButton';   // ⭐ 楼层收藏：自订阅 bookmarkStore，App 不订阅
...
/* 五阶前·「深渊」封印判定已抽到 systems/derivedStats.isAbyssLocked（AbyssPanel 标题也要判，共用一份）。 */
...
function scrubAbyss(text: string): string {
  return text.replace(/深渊/g, '幽冥');
}

/* 🧊 世界作用域·每回合兜底（见 systems/worldScope.ts）：把归属于"当前世界以外的任务世界"的土著补冻。
   与 enterWorld 里那次 freezeWorld 分工——那次负责切世界瞬间（含补写在场者的 worldName），
   这里负责读档 / AI 改写 worldName / 历史存档遗留 等 freezeWorld 抓不到的路径。幂等，可每回合调。 */
function reconcileWorldScope(): void {
  try {
...
/* 当前世界的活跃（未结算）世界事件 —— 演化输入与正文注入共用这一份口径。 */
function currentActiveEvents(): WorldEvent[] {
  try {
    const M = useMisc.getState();
    return activeEvents(M.worldEvents ?? [], M.worldName || '', sameWorld);
  } catch { return []; }
}

/* 世界事件·前端确定性维护（演化阶段之后跑）：超配额裁剪。
   ⚠ 裁剪 = 标记为「湮灭」而不是物理删除（铁律「库房只存不删」）——它们仍留在世界大事流水账里可回看。 */
function reconcileWorldEvents(): void {
  try {
    const M = useMisc.getState();
    const ids = overflowIds(currentActiveEvents());
    for (const id of ids) M.settleWorldEvent(id, 'faded', '（超出本世界事件维护上限，就此淡出）');
    if (ids.length) console.log(`[世界事件] ✂ 超配额，${ids.length} 条淡出：${ids.join('、')}`);
  } catch (e) { console.warn('[世界事件] 维护失败（跳过）:', e); }
}

/* 当前世界的活跃传闻（world 作用域：worldName 为空=老数据放行，与 worldScope 同口径「宁漏勿误」）。 */
function currentWorldRumors(): Rumor[] {
  try {
    const M = useMisc.getState();
    return worldRumors(M.rumors ?? [], M.worldName || '', sameWorld);
  } catch { return []; }
}

/* 传闻·前端确定性维护（**演化阶段之后**跑，把 AI 不该管的机械活接管过来）：
   ① 节点数 ≥3 → 压缩成一条（保留最早日期 + 全程诱因串，不丢"怎么一步步变成这样"）
   ② 达「文化烙印」→ 升格：写成一条世界大事收尾，然后移除（编年史那条线由 chronicle 自然吃到）
   ③ 总数 >5 → 按价值裁剪
   都是纯函数算好再落库，AI 只负责写内容。 */
function reconcileRumors(): void {
  try {
    const M = useMisc.getState();
    const all = M.rumors ?? [];
    ...
    let next = all;
    // ① 压缩
    next = next.map((r) => (needsCompress(r) ? compressRumor(r) : r));
    // ② 升格「文化烙印」→ 世界大事 + 移除
    const promoted = next.filter(shouldPromote);
    for (const r of promoted) {
      const n = latestNode(r);
      M.addWorldEvent({ time: n?.date || M.worldTime || '', location: r.scope || '', desc: `【传世】${r.name}：${n?.told || ''}` });
      console.log(`[传闻] 🏛「${r.name}」已达文化烙印 → 升格为世界大事并移除`);
    }
    next = next.filter((r) => !shouldPromote(r));
    // ③ 裁剪
    const { kept, dropped } = pruneRumors(next);
    if (dropped.length) console.log(`[传闻] ✂ 超出上限，裁掉 ${dropped.length} 条低价值传闻：${dropped.map((d) => d.name).join('、')}`);
    next = kept;
    if (next !== all) useMisc.getState().setRumors(next);
  } catch (e) { console.warn('[传闻] 维护失败（跳过·不阻断演化）:', e); }
}

/* 命运罗盘（systems/divination）：给世界大事的走向做外部随机锚，打破 LLM 的模式坍缩
   （否则后台事件永远是"阴谋→危机→反转"三件套）。
   ⚠ **只进演化阶段，永不进正文**——赛博/西幻世界跳出卦象与塔罗牌名会直接破沉浸；
     注入块自带封词铁则，要求 AI 只把**中文走向描述**写回事件字段。
   种子绑「世界记录 id + 回合区段」：同一段时间内的罗盘稳定，跨读档/回退不跳。 */
function divinationInjection(): string {
  try {
    const M = useMisc.getState();
    if (isHomeWorld(M.worldName || '')) return '';   // 乐园不推演世界大事
    const wid = useWorldRecord.getState().getActive()?.id || M.worldName || 'w';
    const seg = Math.floor((M.turnCount || 0) / 3);   // 每 3 回合换一组，免得每轮都在重掷
    const pool = drawPool(divinationSeed(wid, `seg${seg}`), 3);
    return '\n\n' + buildDivinationInjection(pool, '本期世界走向');
  } catch { return ''; }
}

/* 因果权重（systems/causalWeight）：主角 vs 本世界群体基准 的力量结构，一行机读结论 + 该档的叙事约束。
   在乐园/无世界记录 → 不注入（乐园里"群体 vs 个体"不是叙事主轴）。随各演化阶段同注。 */
function causalWeightInjection(): string {
  try {
    const M = useMisc.getState();
    if (isHomeWorld(M.worldName || '')) return '';
    const rec = useWorldRecord.getState().getActive();
    const p = usePlayer.getState().profile;
    if (!p?.tier && p?.level == null) return '';
    const rep = worldPowerReport(
      { tier: p.tier, level: p.level, bioStrength: undefined },
      { tier: rec?.tier || M.worldTier || '', peakPower: `${rec?.cardSnapshot?.peakPower ?? ''} ${rec?.cardSnapshot?.['世界巅峰战力'] ?? ''}` },
    );
    return `\n\n${getPrompt('CAUSAL_WEIGHT_RULE', CAUSAL_WEIGHT_RULE)}\n${formatPowerReport(rep)}`;
  } catch { return ''; }
}
    ...
    try {
      ...
      if (!localStorage.getItem(AKEY) || assetsGone) {
        ...
        for (const [n, f] of pairs) {
          ...
          if (!t) { okAll = false; continue; }
          try {
            const rep = await importEmbeddedAgentAssets(JSON.parse(t), n, true);
            if (rep.skills || rep.subagents || rep.writerNotes) console.log(`[AgentAssets] 「${n}」内嵌资产：skill×${rep.skills} 子代理×${rep.subagents}${rep.writerNotes ? ' +作者指令' : ''}${rep.errors.length ? ' · 错误:' + rep.errors.join(';') : ''}`);
          } catch (e) { okAll = false; console.warn('[AgentAssets] 导入失败', n, e); }
        }
        if (okAll) localStorage.setItem(AKEY, '1');
      }
    } catch { /* 资产导入失败不阻断启动 */ }
...
const TASK_SOURCE_RULE = `
【任务来源铁则·分清"被指派的任务"与"主角自己的行动"（最高优先·治"把主角的行动当成任务"）】新建任务前，先判定这条**是谁提出的、是不是一个被指派的委托**：
...
【任务评分（完成/失败时给定）】当某任务被标记为已完成/已达成/失败/放弃（status 进入结算态）时，在该任务的 updateTask 载荷里**额外给一个 rating 字段**，按完成质量评级（S/A/B/C/D/E）：
- **评级基准（默认落 B·别惯性给 A/S）**：**B＝按预期正常达成**（多数任务的默认档）；**A＝超预期完成且代价可控**（更快/更省/额外收益等明确亮点）；**S＝以弱胜强、或达成近乎不可能之事且代价极小**（罕见，须正文有"惊艳/越阶/扭转"的明确依据）；**C＝踩线勉强/留有后患/靠外力侥幸**；**D＝仅部分达成**；**E＝失败/放弃**。判断不了给 C。
- **给 A/S 须摘正文依据**：先自问"正文哪一句支撑这个高评级？"——摘不出"超预期/以弱胜强"的具体情节就落 B，不许凭感觉拔高（rating 会加权进世界结算收益，虚高＝凭空加数值）。
- 主线任务综合各环完成度评级；支线/隐藏任务按达成度评级。
- 形如：updateTask("T_5", {"5":"已完成","rating":"A"})。rating 会显示在任务面板"已结束"列表，并供世界结算综合参考。
- **逐环评级**：多环任务的每一环在达成时已用 \`ringAdvance("T_x",{"summary":"…","rating":"…"})\` 各记了「行为总结＋评级」；整条任务的 rating 应是**对各环评级的综合**（勿与各环明显矛盾）。各环的行为总结与评级也会在【结算任务】时注入正文，供正文 API 逐环核算发奖。`;
...
- **【路线图·创建即锁定·铁则·最高优先】新建任务时就把整条路线图一次规划死——全部环（主线 3~12、支线按需 1~12）的 goal/reward/penalty **一次写完整、不留占位、不留"待规划"**，总环数上限 12。一经创建，整条路线图（环数＋各环内容）就**锁死不再改**：之后每回合 **只能推进（ringAdvance 把当前环标 done、记 summary/rating）＋更新 progress 进度**，**绝不许重规划、改写、增删、缩水任何环**（前端已强制冻结：对已定实的环只接收 status/summary/rating，改内容的指令一律忽略）。这样任务内容进世界就定死、只做进度更新、不会老变。**
...
- **何时规划主线**：仅当主角**真正进入一个具体任务世界（衍生世界，非枢纽）**——【进入新世界信号】=是、或本轮正文明确把主角投放进了一个新任务世界，且【当前任务列表】里**没有**属于当前世界的 active 主线时——把**该任务世界自身的核心目标**（用该世界真实的地名/反派/势力，绝不用框架套话）立成一条主线，**一次把整条路线图（总环数＋终局＋每一环的目标/奖惩）规划死、之后不再改**：
  ...
  · **货币·每环基础给量（削减后·按世界阶·铁则）**：六选三里的货币奖励按当前世界阶给基础值（主角当前阶位＝{{getvar::主角.阶位}}、本世界难度 {{getvar::世界.难度}}，据此取货币档：一~三阶乐园币、四阶起灵魂钱币）、**别再随手大放**——**一阶~三阶发乐园币**：一阶+500 / 二阶+1500 / 三阶+4000；**四阶起改发灵魂钱币（魂币）·不再发乐园币**：四阶+1 / 五阶+3 / 六阶+6 / 七阶+12 / 八阶+25 / 九阶+50（**每环基础**·±50% 按环规模浮动；后续环超线性递增、贪婪环再跳一档，但起步以此为准）。此切换契合下条「灵魂钱币·四阶门槛」：三阶及以下绝不出灵魂钱币、四阶起货币奖励即为灵魂钱币。
...
const ADVANCED_TASK_PROTOCOL = `
【进阶任务·自动生成协议 v1.0（乐园高危通告·可选·在合适时机由乐园下达）】除常规主线/支线外，乐园会在**主角发生阶位跃升 / 重大实力突破 / 达成关键里程碑**时，向其下达一条**进阶任务**——一条唯一机会、极高危的史诗契约。何时下达由你据正文判断（没到火候就别硬发；一次最多下达一条）。

【生成逻辑与内容】
- 严格基于**当前世界已揭示的设定与主线线索**，绝不凭空造与本世界格格不入的元素；主线不明则据已有信息自建一条合理且有张力的分支主线作锚点。
- 要有"唯一机会·只有现在·只有你"的紧张感与史诗感。
- **任务目标＝以动词开头的冷酷指令**，从以下六类任选其一：
    · 猎杀：彻底击杀 / 封印某个有真名的强大存在。
    · 夺取：取得一件被严密守护 / 处于极危环境 / 与主线命脉相连的特定物品。
    · 摧毁：彻底毁灭某关键设施 / 仪式核心 / 能量节点 / 禁忌造物，使其无法修复或重现。
    · 存活：在"极高危"绝境中存活指定时长，期间遭不间断致命追杀或环境侵蚀。
    · 守护：在压倒性围攻下确保某指定人物 / 物品 / 据点存活完整，直至撤离条件达成（常关联主线关键幸存者 / 信标）。
    · 颠覆：以暗杀 / 策反 / 伪造神谕 / 引爆政治丑闻等，瓦解或更替一个强大统治势力，使其无法继续主导主线冲突。
- **击杀 / 目标阶位仍受【击杀阶位上限】约束**（据本世界锁定难度），靠"极危绝境 / 严苛限制 / 时限"制造死局压迫，不靠让主角越阶送死。

【文风·强制】严格《轮回乐园》契约通告风格：**极致客观、冷峻、绝对理智**；**禁止任何感叹号、修辞手法、情绪化词汇**；单句信息密度拉满，多用陈述句与硬性指标；不解释任务原因、不作道德评判、不提供额外协助。

【难度·奖惩·时限】任务难度＝极危 或 死局；**奖励**结合契约者当前实力等级给（如 乐园币15万；灵魂钱币仅四阶及以上世界可给·三阶及下只发乐园币）；**失败惩罚固定＝扣除全属性30%、降低当前世界50%收益**；**时限**给具体到时分秒的硬指标（如 23小时59分58秒）。

【落库·输出为任务命令】要下达进阶任务时，用 \`set(...)\` 建成**一条单目标支线任务·不分环**（放进 <upstore> 内、走杂项演化输出）：
  \`set({"0":"<系统给的下一个可用任务ID>","1":"<任务目标·一句以动词开头的冷酷指令>","kind":"支线","2":"<下面这份通告正文全文·保留格式>","5":"进行中","3":"<奖励>","4":"扣除全属性30%·降低当前世界50%收益","startTime":"<当前游戏时间>","endTime":"<当前+时限>"})\`
  其中第 2 列(desc)填这份**通告正文全文**：
【来自乐园的进阶通告】｜契约者编号：[有则填/无则略]｜触发条件：[主角核心属性/技能/天赋]阶位跃升判定。｜任务类型：[猎杀/夺取/摧毁/存活/守护/颠覆]｜任务难度：极危（或死局）｜任务目标：[具体目标·一句话]｜提示：[严苛条件·如 禁用空间位移装备/目标重兵护卫/不可用恢复药剂/环境侵蚀]｜时限：[具体时间]｜奖励：[基础奖励·结合实力等级]｜失败惩罚：扣除全属性30%，降低当前世界50%收益｜祝你好运，契约者`;
...
【契约者对主角的态度·按相对实力·反谄媚（重要·最常犯错）】频道里的契约者**绝不无脑追捧主角**；对主角的态度**只由「双方阶位/实力对比」+「主角在正文里的真实名声」决定**，不因为他是主角就给排面：
- 契约者**明显强于主角**（阶位更高，如四阶对一阶）：把主角当**新人 / 无名小卒 / 路人**，态度是不屑 / 居高临下 / 教训 / 懒得搭理 / 顶多随口点拨——**严禁喊「大佬 / 巨佬 / 前辈 / 大神」**（那是弱者对强者的敬称，强者这样称呼低阶新人极其荒谬出戏）。
- 双方**阶位相近**：平等相待，可竞争 / 调侃 / 结交 / 看不顺眼 / 约组队，正常江湖气，别刻意捧。
- 契约者**明显弱于主角**：才可能仰视、客气、求带、喊前辈/大佬——但弱者也有骨气，别一味谄媚。
- 主角是**低阶（一~二阶）新人**、且正文里没打出名气时：频道里**基本没人认识他、更没人捧**，多是各聊各的、拿他当路人；**除非正文明确写过主角的硬战绩/名号，否则绝不凭空给他称号、排面或簇拥**。
...
const rightMenuItems = [
  { icon: '⚔', label: '装备' },
  { icon: '🎒', label: '储存空间' },
  { icon: '👗', label: '形象工坊' },   // 每角色专属衣柜：读形象/搭配/试衣生图/生图提示词/成衣库导入
  { icon: '📇', label: 'NPC' },
  { icon: '🔗', label: '调教' },
  { icon: '🐾', label: '宠物/召唤物' },
  { icon: '✨', label: '技能' },
  ...
  { icon: '🌐', label: '联机' },
  { icon: '💬', label: '聊天室' },
  { icon: '⏱', label: '游玩时长' },   // runNavAction/NAV_FX/⌘K别名/PlaytimePanel 四处早已就绪，唯独这里漏了 → 曾是零入口死功能
  { icon: '🛒', label: '交易行' },
  { icon: '🏪', label: '产业' },
  { icon: '🏰', label: '公会' },
  { icon: '🆘', label: '助战' },
  ...
  { icon: '💾', label: '存档' },
  { icon: '🌿', label: '分支树' },
  { icon: '⭐', label: '坐标' },   // 收藏楼层（存正文快照·纯前端，与分支树相邻：一个存"没走的路"，一个存"值得留的段落"）
  { icon: '🔊', label: '语音' },
  { icon: '⚙', label: '设置' },
];

/* P4·分组导航（默认开·外观设置「导航分组」可关回平铺）：近 50 项按功能域分 7 组，组内相关功能相邻。
   成员写 rightMenuItems 的 label（「幽冥」按「深渊」归组）；没归到组的新增项自动落「其他」——
   加新导航项忘了归组也不会消失（宁可显示在兜底组，绝不静默丢失）。 */
const NAV_GROUPS: { name: string; members: string[] }[] = [
  { name: '角色', members: ['装备', '储存空间', '形象工坊', '技能', '技能树', '体系', '副职业', '称号', '成就'] },
  ...
  { name: '社交联机', members: ['频道', '私信', '采访', '直播', '聊天室', '联机', '交易行', '产业', '公会', '助战', '世界竞技场', '游玩时长'] },
  { name: '传承', members: ['纪念丰碑', '账户仓库', '创意工坊', '存档', '分支树', '坐标'] },
  { name: '系统', members: ['回合洞察', '审计', '记忆', '语音', '设置'] },
];

/* P4·⌘K 深动作表：不进右侧导航、只出现在命令面板的二级功能（run 分支在 runNavAction）。
   欢愉宫按开关另行追加（见组装处 paletteItems）。 */
const PALETTE_ACTIONS: { icon: string; label: string }[] = [
  { icon: '⚒', label: '装备强化' },
  { icon: '🔼', label: '技能升级' },
  { icon: '🏟', label: '竞技场' },
  { icon: '🎰', label: '赌场' },
...
const PREFETCH_PANELS: (() => Promise<unknown>)[] = [
  ...
  () => import('./components/OutfitWorkshop'),
...
const NAV_FX: Record<string, string> = {
  '装备': 'fx-sword', '储存空间': 'fx-bag', '形象工坊': 'fx-card', 'NPC': 'fx-card', '调教': 'fx-card', '技能': 'fx-sparkle',
  ...
  '频道': 'fx-signal', '私信': 'fx-mail', '采访': 'fx-signal', '直播': 'fx-signal', '好友': 'fx-friends', '聊天室': 'fx-signal', '交易行': 'fx-bag', '产业': 'fx-bag', '公会': 'fx-castle', '助战': 'fx-clash', '世界竞技场': 'fx-trophy', '游玩时长': 'fx-trophy', '纪念丰碑': 'fx-pillar', '账户仓库': 'fx-bag', '记忆': 'fx-brain', '创意工坊': 'fx-sparkle', '存档': 'fx-save', '设置': 'fx-gear',
};
...
function rollAndApplyGemDrops(rawNarrative: string): string {
  try {
    ...
    const p = usePlayer.getState().profile;
    // 公会增益 dropRate（Lv5 perk·+0.05）：掉率加成由前端接（GuildDO 注释"数值效果由前端逐期接"，此前 guildPerkValue 零调用、捐献无任何机械回报）
    const drops = rollGemDrops(rawNarrative, {
      tier: p?.tier, level: p?.level,
      config: { enabled: true, rate: (st.gemDropRate ?? 0.2) + guildPerkValue('dropRate'), maxPerTurn: 1 },   // 仅击杀强敌可能掉·每回合至多 1 颗
    });
    // 公会贡献：击杀强敌就记（原先挂在"掉到宝石"之后——掉率仅 20%，八成强敌击杀对公会零贡献）
    try { if (useGuild.getState().my && hasHighTierKill(rawNarrative)) guildClient.contributeRest(60, 'kill'); } catch { /* 击杀强敌→公会贡献(异步·非阻塞) */ }
...
export default function App() {
  ...
  const [settingsDeepPage, setSettingsDeepPage] = useState<string | undefined>(undefined);   // P4·空态深链：navBus 请求直达的设置子页
  ...
  const [liveOpen,         setLiveOpen]         = useState(false);   // 📺 乐园直播间（借鉴Abstract外置手机）
  ...
  const [outfitWorkshopOpen, setOutfitWorkshopOpen] = useState(false);  // 👗 形象工坊（每角色衣柜·试衣·生图提示词·成衣库）
  ...
  // 五阶前「深渊」封印同步到 UI：导航/⌘K 显示「幽冥」（与正文 scrubAbyss 同口径），五阶解锁恢复真名。
  // 布尔选择器：只在跨过五阶那一刻触发一次重渲染，平时零开销。
  const abyssLocked = usePlayer((s) => isAbyssLocked(s.profile));
  const navItems = useMemo(
    () => (abyssLocked ? rightMenuItems.map((it) => (it.label === '深渊' ? { ...it, label: '幽冥' } : it)) : rightMenuItems),
    [abyssLocked],
  );
  // P4·分组导航：navGrouped 开 → 按 NAV_GROUPS 分区；关 → null（渲染处走原平铺路径，逐像素不变）
  const navGrouped = useSettings((s) => s.navGrouped);
  const navSections = useMemo(() => {
    if (!navGrouped) return null;
    const canon = (l: string) => (l === '幽冥' ? '深渊' : l);
    const byLabel = new Map(navItems.map((it) => [canon(it.label), it]));
    const used = new Set<string>();
    const sections = NAV_GROUPS.map((g) => ({
      name: g.name,
      items: g.members.map((m) => { const it = byLabel.get(m); if (it) used.add(m); return it; }).filter(Boolean) as typeof rightMenuItems,
    })).filter((s) => s.items.length > 0);
    const rest = navItems.filter((it) => !used.has(canon(it.label)));
  ...
  function applyRaidReward(rw: any) {
    ...
    try { const I = useItems.getState(); for (const [k, v] of Object.entries(rw.currency || {})) I.adjustCurrency(k as any, Number(v) || 0, '联机讨伐·副本奖励'); } catch (e) { console.warn('[Raid] 奖励货币入账失败', e); }
    ...
    // 来宾：记一笔防回滚流水（离房 loadSlot 会把上面全部入账还原掉；房主/单机不记）
    recordMpReward({ id: `raid_${rw.rewardId}`, note: '副本通关豪华奖励', currency: rw.currency, items: rw.items, potentialPoints: rw.potentialPoints, title: rw.title });
    // 场外通报 + 发放登记：讨伐奖励此前零通报、物品也没登记 granted（审计确认）
    try {
      const itemNames = (rw.items || []).map((x: any) => String(x?.name || '')).filter(Boolean);
      reportFacilityOutcome({
        source: '联机讨伐',
        summary: `通关「${rw.themeName || '攻坚战'}」（评级 ${rw.rating || '—'}）：豪华奖励已全套入账——${itemNames.slice(0, 6).map((n: string) => `「${n}」`).join('、')}${itemNames.length > 6 ? ' 等' : ''}${rw.title?.name ? `，获称号「${rw.title.name}」` : ''}`,
        granted: itemNames,
        growth: rw.title?.name ? `因通关「${rw.themeName || '攻坚战'}」获得称号「${rw.title.name}」——可自然带过战后授勋的一幕，不强求成段` : undefined,
      });
    } catch { /* 通报失败不阻断入账 */ }
  ...
  // reload 恢复（回退/重新生成/仅重算变量/读档 都靠 loadSlot→reload）时，同步读 resume 标志（localStorage+TTL，见 resumeFlag.ts）
  // → started 从首帧就是 true，不再先闪一下主菜单再切回游戏（用户报"重新生成/回退总会回到主界面再回来"）。
  // 标志仍由挂载 effect 正常读取并消费，这里只提前决定初始画面。
  ...
  const [canonRouteOpen, setCanonRouteOpen] = useState(false);   // 🛤 原著路线·路线图弹窗
  const canonEnabled = useCanonRoute((s) => s.enabled);          // 原著路线激活时世界栏替换为路线栏
  const canonIdx = useCanonRoute((s) => s.stationIndex);
  const [canonSettleNotice, setCanonSettleNotice] = useState('');   // 🛤 结算盖章横幅（你 vs 白夜）
  ...
  useEffect(() => {
    useMp.getState().setHandlers({
      ...
      onRelay: (m) => {
        ...
        switch (m.event) {   // m.payload 按 event 收窄（见 mpProtocol RelayPayloads）
          ...
          case 'raid_loot':
            ...
            try {
              useItems.getState().adjustCurrency('乐园币', Number(m.payload.currency) || 0, '联机讨伐·战利品货币');   // 货币全员均得
              recordMpReward({ id: `raidloot_${m.payload.lootId}`, note: '讨伐战利货币', currency: { 乐园币: Number(m.payload.currency) || 0 } });   // 来宾防回滚
            } catch { /* */ }
            break;
          case 'raid_roll':
          ...
          case 'raid_loot_result': {
            ...
            if (lt && m.payload?.lootId === lt.lootId) {
              for (const it of (lt.items || [])) { const r = m.payload.results?.[it.id]; if (r?.winnerId === myPlayerId()) { try { useItems.getState().addItem({ name: it.name, category: it.category, gradeDesc: it.gradeDesc, effect: it.effect, quantity: it.quantity } as any); recordMpReward({ id: `roll_${lt.lootId}_${it.id}`, note: '讨伐战利 ROLL 所得', items: [{ name: it.name, category: it.category, gradeDesc: it.gradeDesc, effect: it.effect, quantity: it.quantity }] }); } catch { /* */ } } }
      ...
      onGuestJoin: () => {   // 来宾进房：把当前单机态快照到保留槽，隔离单机存档（联机存档）
        ...
        clearMpPending();   // 防回滚流水从零记：当前态刚被快照，旧流水已随态持久化、作废
      },
      onGuestRestore: async () => {   // 来宾离开/关房：还原单机存档（loadSlot 会整页 reload；无备份则 no-op）
        // 促账必须在 loadSlot 之前（loadSlot 末尾同步 reload，之后的代码不执行）；reload 后开机 replayMpPendingOnBoot 补发在房入账
        promoteMpPending();
        try {
          const ok = await loadSlot('mp-solo-backup');
          if (!ok) discardMpReplay();   // 无备份=没回滚 → 奖励仍在当前态，补发就是双发
        } catch (e) { discardMpReplay(); console.warn('[MP] 单机还原失败', e); }
      },
  ...
  // 联机来宾·离房奖励补发：离房 loadSlot 还原单机档并 reload，到这里把在房期间的入账流水重放回来
  // （讨伐货币/ROLL 物品/豪华奖励/赠予——否则来宾一离房全部蒸发）。无流水=零开销 no-op。
  useEffect(() => { try { replayMpPendingOnBoot(); } catch (e) { console.warn('[MP] 离房奖励补发异常', e); } }, []);

  // 预设中心·底稿版本对账（幂等）：把「保存过但从没真改」的主提示词旧副本自动升级到新版内置默认
  // （典型：记忆提示词按全文初始化进持久层，内置更新后老档会静默冻在旧版）。真自定义的绝不动，只在预设中心挂徽章。
  // ⚠ 动态 import：promptRegistry 牵着全套提示词常量，静态引会把它们拽进主 chunk（它本属预设中心的 lazy 分包）。
  useEffect(() => { void import('./systems/promptRegistry').then((m) => m.reconcilePromptDefaults()).catch((e) => console.warn('[预设中心] 底稿对账异常', e)); }, []);

  // P4·空态深链：面板经 navBus 请求「打开设置并直达子页」（CustomEvent 解耦·免穿 9 个 lazy 面板的 prop）
  useEffect(() => {
    const h = (e: Event) => { const p = (e as CustomEvent).detail; if (typeof p === 'string' && p) { setSettingsDeepPage(p); setSettingsOpen(true); } };
  ...
  useEffect(() => {
    (async () => {
      ...
      if (getResumeFlag(PENDING_STARTED_KEY)) {
        setStarted(true);
        clearResumeFlag(PENDING_STARTED_KEY);
      }
      // 仅重算变量：回退点已 reload 恢复 → 复用本回合原正文重跑演化（不重新生成正文）
      const revar = getResumeFlag(PENDING_REVAR_KEY);
      if (revar) {
      ...
      const regen = getResumeFlag(PENDING_REGEN_KEY);
      if (regen) {
  ...
  // 📖 漫画写回楼层：comic.ts 任务全页成功后经事件桥把页图追加进目标楼层正文（anchor 空=追加正文末尾；随消息进 chatDb）
  useEffect(() => {
    const onComicInsert = (e: Event) => {
      const d = (e as CustomEvent).detail as { msgId: number; title: string; pages: { url: string; prompt: string }[] } | undefined;
      if (!d || typeof d.msgId !== 'number' || !Array.isArray(d.pages) || !d.pages.length) return;
      setMessages((prev) => prev.map((m) => m.id === d.msgId
        ? { ...m, images: [...(m.images ?? []), ...d.pages.map((p) => ({ anchor: '', url: p.url, prompt: p.prompt, nsfw: 'sfw', ts: Date.now() }))] }
        : m));
    };
    window.addEventListener('zs-comic-insert', onComicInsert);
    return () => window.removeEventListener('zs-comic-insert', onComicInsert);
  }, []);
  ...
  function buildPresetMessages(preset: (typeof textPresets)[0] | undefined, ctx: string, userInput = '') {
    ...
    // 统一展开 expandPromptText：先表格模板(<if var/cell/seed/cond>/计算标签/$ref·seedContent=末条正文)，再 ST 宏。
    //   按预设顺序对每个块求值（setvar→后续 getvar 生效·runtimeVars 已灌入可 {{getvar::主角.HP}}/${世界.名}/<if var>），
    //   未识别宏末尾清掉防泄漏；轮回乐园/双人成行无宏=无操作；让导入的任意 ST 预设不必再手工摊平宏。
    const _expandCtx = makePromptExpandCtx({
      user: usePlayer.getState().profile?.name || '主角',
      char: usePlayer.getState().profile?.name || '主角',
      lastUserMessage: userInput,
      seedContent: lastNarrativeRef.current || '',
    });
    const enabled = (preset?.entries ?? []).filter((e) => e.enabled).map((e) => ({ ...e, content: expandPromptText(e.content || '', _expandCtx) }));
    ...
    addRule('主角状态输出', '前端规则 · 主角状态输出', PLAYER_STATE_EMIT_RULE);
...
- **万一确需写低阶契约者，必须交代合理性且态度自洽**：要么是刚被投放就濒死 / 仓皇逃命 / 躲藏苟活的倒霉蛋，要么已是尸体 / 残躯 / 前人遗骸；面对**明显强于自己**的主角，低阶者只会**恐惧、逃避、噤声、谄媚或求饶**，**绝不会像没事人一样窥视、嘲讽、点评一个远比自己强大的存在**——那是找死。（此为「弱者对强者」态度，与「强者对弱者不屑」互为镜像。）` : ''}`); }
    ...
    if (/【结算任务】/.test(userInput)) {
      ...
      try {
        ...
        if (done.length) {
          ...
          // 公会增益 settlement（Lv3 perk·value=满档占比，0.05≈0.35档）：结算评级小幅加成——机械接入（GuildDO 只判解锁、数值由前端接，此前零调用）
          const guildSettleBonus = guildPerkValue('settlement') * 7;
          // 环评分主导：各环平均评分×0.75 + 世界之源×0.25，再叠加贪婪环完成加成 + 公会增益
          const finalPts = Math.max(0, Math.min(7, Math.round(wsPts * 0.25 + avgRing * 0.75 + greedyBonus + guildSettleBonus)));
          addRule('本世界综合评级建议', '前端数据 · 综合评级建议（环均分×0.75 + 世界之源×0.25·环评分主导）',
            ...
            (guildSettleBonus > 0 ? `- 公会增益：世界结算评级小幅加成 **+${guildSettleBonus.toFixed(2)} 档**（已计入综合评级）\n` : '') +
    ...
    if (povSel && povSel !== 'off') {
      ...
      addRule('叙事人称', '前端规则 · 叙事人称（' + povSel + '）', povRule);
    }

    // 四阶前·真实属性绝对封锁：主角未达四阶(realAttrMult<5)时，正文严禁出现/给予真实属性·真实属性点（含技能/天赋/装备/掉落/任务）；与世界书常驻条目 + 解析器拒收三重保险。
    { const _pp = usePlayer.getState().profile; if (realAttrMult(_pp?.tier, _pp?.level) < 5) { addRule('四阶前禁真实属性', '前端规则 · 四阶前禁真实属性', REAL_POINT_LOCK_RULE); } }
...
- 禁止输出正文内容，只输出指令块`;
  ...
  async function runPlayerEvolutionPhaseCore(narrative: string) {
    ...
    try {
      ...
      const systemPrompt = buildPlayerSystemPrompt(enabledEntries)
        ...
        + (isHomeWorld(useMisc.getState().worldName || '') ? '' : '\n' + getPrompt('REPUTATION_RULE', REPUTATION_RULE)   // 四维声誉是 world 作用域：人在乐园不注入
          + `\n【当前四维声誉】${formatRepute(usePlayer.getState().profile.repute ?? defaultRepute())}`);
  ...
  function applyNpcShortCommandsImpl(reply: string, onlyId?: string, narrative?: string): number {
    ...
    while ((m = hpRe.exec(reply))) {
      ...
      const dmax = fullMaxHp(npcBaseAttrs(rec), (rec?.items ?? []).filter((it) => it.equipped) as any[], nc?.skills, nc?.traits, realAttrMult(rec?.realm, lvFromRealm(rec?.realm)), ratioOf(rec));   // npcBaseAttrs=attrs+真实属性点直加(realAttrs)
    ...
    while ((m = mpRe.exec(reply))) {
      ...
      const rec = npc.npcs[m[1]];
      const nc = useCharacters.getState().characters[m[1]];
      const dmax = fullMaxEp(npcBaseAttrs(rec), (rec?.items ?? []).filter((it) => it.equipped) as any[], nc?.skills, nc?.traits, realAttrMult(rec?.realm, lvFromRealm(rec?.realm)), ratioOf(rec));   // npcBaseAttrs=attrs+真实属性点直加(realAttrs)
  ...
  function applyEntryResult(result: any, turn: number): Set<string> {
    ...
    for (const e of result.entries ?? []) {
      ...
      if (e.type === 'new') {
        ...
        // 同时落 worldName（世界作用域·公理1）：土著归属本世界，离世时 freezeWorld 靠它定位该冻谁。
        {
        ...
        // 同时补 worldName（老存档迁移）：能重新登场说明他就在本世界，补上后离世才冻得掉。
        {
          const curWorld = (useMisc.getState().worldName || '').trim();
          if (!npc.npcs[rid]?.npcTag) npc.upsertNpc(rid, { npcTag: isHomeWorld(curWorld) ? '契约者' : '土著' });
          if (!npc.npcs[rid]?.worldName && curWorld && !isHomeWorld(curWorld)) npc.upsertNpc(rid, { worldName: curWorld });
        }
        const loc = /loc\.[CG]\d+\s*=\s*([^\n]+)/.exec(e.stateCommands ?? '');
        if (loc) npc.upsertNpc(rid, { extra: { ...(npc.npcs[rid]?.extra ?? {}), 位置: loc[1].trim() } });
      }
    }
    // 离场：顺手记一句"去向"（systems/castHint.ts），下一轮注入 <角色动向提示> 的「刚刚离场」段——
    // 治"人说走就走、正文里再无交代"。确定性取词，不花 API。
    for (const x of result.exits ?? []) {
      if (!x?.id || !npc.npcs[x.id]) continue;   // 不为不存在的ID凭空建离场空壳
      npc.setScene(x.id, false);
      const rec = npc.npcs[x.id];
      if (rec) npc.upsertNpc(x.id, { auto: { ...(rec.auto ?? { phase: 'hub', turns: 0 }), exitReason: pickExitReason(rec, turn), readyToEnter: false, enterReason: undefined } });
    }
  ...
  /* 驱动力上下文：任务面板 + 当前世界的近期大事 + 最近正文。名字命中即算有关联（见 systems/npcDrive）。 */
  function buildDriveCtx(): DriveCtx {
    try {
      const M = useMisc.getState();
      const questText = (M.tasks ?? []).map((t) => `${t.name ?? ''} ${t.desc ?? ''} ${(t.rings ?? []).map((r) => r.goal ?? '').join(' ')}`).join('\n');
      const worldText = (M.worldEvents ?? [])
        .filter((e) => !e.worldName || sameWorld(e.worldName, M.worldName || ''))
        .slice(-5).map((e) => `${e.location ?? ''} ${e.desc ?? ''}`).join('\n');
      return { questText, worldText, narrative: lastNarrativeRef.current ?? '' };
    } catch { return { questText: '', worldText: '', narrative: '' }; }
  }
  ...
  function computeFocusList(createdIds: Set<string>, turn: number): string[] {
    ...
    Object.values(npcs).filter((n) => n.onScene && alive(n) && !n.frozenAt).forEach((n) => must.add(n.id));

    // 驱动力闸门（systems/npcDrive）：离场角色必须至少有一项行动驱动力（羁绊/任务/局势/社交），
    // 三无 → 判定【静滞】，本轮不花演化 API（他仍然跑轨道A 零 API 自治，不会被冻住）。
    const driveCtx = buildDriveCtx();
    const staticIds: string[] = [];
    const offCands = Object.values(npcs)
      .filter((n) => !n.onScene && !n.archived && !n.frozenAt && alive(n) && !must.has(n.id))   // 归档=玩家封存 / 🧊冻结=已离开该世界，都不参与演化
      .filter((n) => { if (driveOf(n, driveCtx) != null) return true; staticIds.push(n.id); return false; })
    ...
    const friendIds = friendsPerTurn > 0
      ? Object.values(npcs)
          .filter((n) => n.isFriend && alive(n) && !n.frozenAt && !must.has(n.id))
          .sort((a, b) => (a.lastEvolvedTurn ?? 0) - (b.lastEvolvedTurn ?? 0))
          .slice(0, friendsPerTurn)
          .map((n) => n.id)
      : [];

    if (staticIds.length) console.log(`[驱动力] ⏸ 本轮静滞 ${staticIds.length} 人（无羁绊/任务/局势关联·不花演化API，仍走轨道A）`);
  ...
  /* ─── 演化预筛 Gate（借鉴 SoulLink 思想·systems/rosterGate.ts·默认 off）───
     只裁「在场·非新建」的焦点：正文没写到实际变化的在场 NPC 本轮跳过大演化调用——
     省调用之外，更是从源头减少"没变化硬编变化"的防漂压力（与下游成长闸门互补）。
     ⚠ 边界钉死：新建必过（必须建档）；离场目标是驱动力闸门/好友轮换选出的后台自转，不归 Gate 管；
       Gate 失败/解析失败一律 fail-open 全跑；每 N 回合强制全量一轮，防「沉默变化」漏更累积。 */
  async function gateFocusIds(focusIds: string[], narrative: string, createdIds: Set<string>, turn: number): Promise<string[]> {
    const sched = useNpcEvo.getState().settings.scheduling;
    const mode = sched.evoGateMode ?? 'off';
    if (mode === 'off') return focusIds;
    const forceEvery = sched.evoGateForceFullEvery ?? 4;
    if (forceEvery > 0 && turn > 0 && turn % forceEvery === 0) {
      console.log(`[NPC预筛] 第 ${turn} 回合强制全量（每 ${forceEvery} 回合防漏更）`);
      return focusIds;
    }
    const { npcs } = useNpc.getState();
    const gateable = focusIds.filter((id) => npcs[id]?.onScene && !createdIds.has(id));
    if (gateable.length === 0) return focusIds;
    const roster: RosterGateItem[] = gateable.map((id) => ({
      id, name: npcs[id].name,
      hint: [npcs[id].npcTag, npcs[id].profession].filter(Boolean).join('·') || undefined,
    }));
    const trimmed = trimNarrative(narrative).slice(-6000);   // Gate 是轻量调用，正文只喂尾段
    let selected: Set<string>;
    if (mode === 'local') {
      selected = localNameScan(roster, trimmed);   // 零 API：名字出现在正文即入选（抓不到沉默目击者）
    } else {
      // 独立接口槽 npcGate；未配置路由则沿用 NPC 演化接口链
      const gateRouteIds = useSettings.getState().apiRoutes?.['npcGate'] ?? [];
      const chain = gateRouteIds.length ? resolveApiChain('npcGate', getNpcApi()) : resolveApiChain('npc', getNpcApi());
      const res = await runRosterGate({ chain, criteria: NPC_EVO_GATE_RULE, roster, narrative: trimmed, label: 'NPC演化预筛Gate' });
      if (!res.ok) {
        reportConsistency('evo-gate', `预筛 Gate 失败→本轮回退全量演化：${(res.error ?? '未知').slice(0, 120)}`);
        return focusIds;
      }
      selected = res.selected;
      if (selected.size === 0) {
        // 0 入选可审计（安静回合是合法结果，但要留原文供排查漏选）
        reportConsistency('evo-gate', `预筛 0 入选（在场 ${roster.length} 人未见变化）·Gate 原文：${res.raw.slice(0, 160) || '（空）'}`);
      }
      const unmatched = res.rawNames.length - selected.size;
      if (unmatched > 0) console.warn(`[NPC预筛] Gate 输出 ${unmatched} 个名单外/对不上的名字（已忽略）：`, res.rawNames.join('、'));
    }
    const skipped = gateable.filter((id) => !selected.has(id));
    if (skipped.length) console.log(`[NPC预筛] ⏸ 本轮跳过在场 ${skipped.length} 人（${mode === 'local' ? '正文未点名' : '判定无实际变化'}）：${skipped.map((id) => npcs[id]?.name || id).join('、')}`);
    return focusIds.filter((id) => !npcs[id]?.onScene || createdIds.has(id) || selected.has(id));
  }
  ...
  async function runNpcFocusEvolution(narrative: string, createdIds: Set<string>) {
    ...
    try {
      ...
      // 演化预筛 Gate（借鉴 SoulLink·默认 off）：先裁掉「在场但本轮无实际变化」的目标，再套每回合上限
      focusIds = await gateFocusIds(focusIds, narrative, createdIds, turn);
      // 每回合最多演化几个（0=不限）
...
${AFFIX_EFFECT_RULE}`;
  ...
  async function povRender(outline: string, name: string, others: string[], profile?: string): Promise<string> {
    const chain = myTextChain();
    if (!chain[0]?.baseUrl || !chain[0]?.apiKey) return '';
    try {
      const { content } = await apiChatFallback(chain, [
        { role: 'system', content: buildPovRenderRule(name, others, povWordOf(), profile) },
  ...
  async function collectPovDrafts(guests: { seatId: string; name: string }[], outlines: Record<string, string>, drafts: Record<string, string>, otherNames: (self: string) => string[], timeoutMs: number): Promise<void> {
    ...
    const fallbackRender = async (g: { seatId: string; name: string }) => {
      const snap = (useMp.getState().cards || []).find((c) => c.seatId === g.seatId)?.snapshot;   // 房主代渲染该来宾时注入其角色档案，别写成新手/别人
      return (await povRender(outlines[g.seatId] || '', g.name, otherNames(g.name), snap ? buildCardProfile(snap) : '')) || outlines[g.seatId] || '';
    };
...
【队伍】${roster}　【当前已有关键物品】${haveItems}　【世界】${world}`;
  ...
  async function manualGenTask(tendency: string): Promise<{ ok: boolean; msg: string }> {
    ...
    const sys = [
      ...
      renderPrompt(QUEST_PLANNING_RULE), TASK_CANON_RULE, QUEST_KILL_TIER_RULE,
      `【当前世界】：${M.worldName}`,
  ...
  async function runQuestEvolutionPhase(narrative: string) {
    ...
    const systemPrompt = QUEST_PHASE_FORMAT_RULE
      ...
      + '\n\n' + renderPrompt(QUEST_PLANNING_RULE)
      + '\n\n' + QUEST_KILL_TIER_RULE
      + '\n\n' + ADVANCED_TASK_PROTOCOL
      + '\n\n' + QUEST_RATING_RULE
      + '\n\n' + TASK_RECONCILE_RULE
      + '\n\n' + TASK_PROGRESS_RULE
      + '\n\n' + TASK_CANON_RULE
      ...
      + questCodexInjection
      + buildDerivationInjection(useMisc.getState().worldEvents ?? [])   // 后台事件结算为「派生后续」→ 支线候选钩子（建不建仍由任务闸门定）
    ...
    try {
      ...
      // ⚠ 无论 AI 是否真的建了支线都标记——建议是一次性的，钩不上就该过去，不该每轮再劝一遍。
      try {
        for (const e of pendingDerivations(useMisc.getState().worldEvents ?? [])) useMisc.getState().markEventDerived(e.id);
      } catch { /* 标记失败不阻断 */ }
    } catch (e: any) {
  ...
  async function runMiscEvolutionPhase(narrative: string) {
    ...
    const systemPrompt = buildMiscSystemPrompt(M.settings.entries)
      ...
      // 🛤 原著路线：把本站剧本+苏晓轨道给杂项 AI 参照，并追加 canon* 状态维护指令（不在站内世界则两个 builder 返回空、整块不加）
      + (() => { const blocks = [...buildCanonWorldInjection(), ...buildSuxiaoTrackInjection()].map((b) => b.content).join('\n'); return blocks ? `\n\n${blocks}\n\n${getPrompt('CANON_MISC_RULE', CANON_MISC_RULE)}` : ''; })()
      + '\n\n' + TRUTHS_MAINTAIN_RULE
      // 🗓 世界历：把现有条目喂回去（AI 才知道哪些已建档、不重复给），再附维护规则。空清单时也给一行提示，引导它在正文确立节日/生日时建档
      + `\n【世界历·当前条目（用 almanac([...]) 增量维护，规则见下）】：${(() => {
        const list = useCalendar.getState().items;
        if (!list.length) return '（空——正文确立了"每年这天"的节日/生日/纪念日时再建档，没有就别硬造）';
    ...
    try {
      ...
      try {
        for (const ev of useMisc.getState().worldEvents) {
          if (!hasDivinationLeak(ev.desc)) continue;
          const cleaned = scrubDivination(ev.desc);
          useMisc.getState().updateWorldEvent(ev.id, { desc: cleaned || ev.desc });
          console.warn(`[命运罗盘] 🧹 世界大事 ${ev.id} 混入占卜术语，已清洗`);
        }
      } catch { /* 清洗失败不阻断演化 */ }
  ...
  async function runPortraitPhase() {
    ...
    for (const job of jobs) {
      ...
      try {
        ...
        const basePrompt = buildPortraitPrompt({ ...job.fields, imageTags: tags, charId: job.id });   // charId→衣柜钦定穿搭覆盖服装
        const refImages = await outfitRefImages(job.id);   // 👗 钦定穿搭参考图（仅 chatimg 多模态线生效，其余服务忽略）
        const prompt = refImages.length ? `${basePrompt}\n\n${OUTFIT_REF_HINT}` : basePrompt;
        const url = await shrinkDataUrl(await generateImage(service, { prompt, negative: ig.portraitNegative, refImages, label: `自动肖像 · ${job.name}` }));
  ...
  async function genStoryImagesFor(narrative: string, count: number, msgId: number): Promise<number> {
    ...
    const charLine = (id: string, name: string, gtag: string, base: string, now: string, tags: string, btRaw?: string) => {
      ...
      const outfit = outfitRosterLine(id);   // 👗 衣柜钦定穿搭：服装单一权威源（覆盖外观描述里的穿着）
  ...
  async function buildStructuredRecall(context: string, opts: { noLlmSelect?: boolean; userInput?: string } = {}): Promise<{ player: { role: 'system'; content: string }[]; rest: { role: 'system'; content: string }[] }> {
    ...
    const game = useGame.getState().player;
    const b1 = chars['B1'];
    // 「不常用空间」(archived) 的物品是玩家主动收纳、不想让叙事关注的 → 不进结构化档案/叙事回忆的背包序列化。
    // 已装备的照常保留（影响 HP/EP 等数值·且 UI 本就不允许归档已装备物）。
    const allItems = useItems.getState().items.filter((it) => it.equipped || !it.archived);
    ...
    if (TM.established && !TM.disbanded) {
      ...
      // 外派状态（P2 读回）：成员在外执行委托时正文此前完全不知道人去哪了——补一行，写作时这些人不在场、勿让其现身
      const disp = TM.dispatchActive;
      if (disp && (disp.memberNames?.length || disp.memberIds?.length)) {
        const who = (disp.memberNames?.length ? disp.memberNames : disp.memberIds).filter(Boolean).join('、');
        teamLines.push(`⚔ 外派中：${who} 正在外执行委托「${disp.offer?.title || '未名委托'}」（预计第 ${disp.endTurn} 回合归来·外派期间不在主角身边，正文勿让其现身）`);
      }
      cards.push(teamLines.join('\n'));
    }
  ...
  /* 🔎 乐园检索（借鉴Abstract外置手机 browser·拉取式情报）：检索词→3~5条世界内搜索结果。
     认知门与世界见闻同一道（buildNewsCandidates：hidden 连候选都进不来·trace 只给表象）；
     候选空≠不能搜——只是事实池为空，仍可搜生活化信息（与见闻"空则不白花钱"不同，检索是玩家主动行为）。 */
  async function paradiseSearch(query: string): Promise<{ ok: boolean; msg: string; hits: import('./systems/paradiseSearch').SearchHit[] }> {
    const q = (query || '').trim().slice(0, 60);
    if (!q) return { ok: false, msg: '输入点什么再搜', hits: [] };
    const M = useMisc.getState();
    const wn = (M.worldName || '').trim();
    const cands = buildNewsCandidates(M.worldEvents ?? [], M.rumors ?? [], wn, sameWorld);
    const ss = useSettings.getState();
    const chain = resolveApiChain('paradiseSearch', ss.textUseSharedApi ? ss.api : ss.textApi);
    if (!chain[0]?.baseUrl || !chain[0]?.apiKey) return { ok: false, msg: '接口未配置：接口路由 paradiseSearch 或正文 API（默认回退）', hits: [] };
    try {
      const sys = PARADISE_SEARCH_RULE
        + `\n\n【当前世界】${wn || '（未设定·轮回乐园本部）'}｜【世界时间】${M.worldTime || M.paradiseTime || '（未设定）'}`
        + `\n${searchCandidateBlock(cands)}`;
      const { content } = await apiChatFallback(chain, [
        { role: 'system', content: sys },
        { role: 'user', content: `检索词：「${q}」\n请按【输出】只输出 JSON。` },
      ], { timeoutMs: 90000 });
      const hits = parseSearchReply(content || '');
      if (!hits.length) return { ok: false, msg: '没检索到可用结果（格式不符或没相关内容），换个词试试', hits: [] };
      return { ok: true, msg: `${hits.length} 条结果`, hits };
    } catch (e) {
      console.warn('[检索] 失败', e);
      return { ok: false, msg: `检索失败：${e instanceof Error ? e.message : '网络或接口错误'}`, hits: [] };
    }
  }
  ...
  async function replyToChannelPost(channel: string, content: string, replyTo?: { authorName: string; content: string }) {
    ...
    // 场外行为通报：主角在公共频道的发言注入正文「前置须知」，让正文 AI 知晓主角刚在契约者广场公开说了什么（一次性·仅知晓）
    pushSceneNotice(`【场外·公共频道】主角在【${chDef?.label ?? channel}】频道${replyTo ? `回复「${replyTo.authorName}」` : '公开发言'}：「${content.trim().slice(0, 140)}」`);
...
【主角面板】${playerCard}`;
  ...
  /* 取对话历史（喂给 AI，过滤系统消息；带花样的消息压成方括号标记行——借鉴外置手机的历史压缩）。
     群聊线程：每条冠发言人名（主角消息也冠主角名，让 AI 分得清多方对话）。 */
  function dmHistory(th: import('./store/dmStore').DmThread, limit = 12): { role: 'user' | 'assistant'; content: string }[] {
    ...
    return th.messages.filter((m) => m.from !== 'system').slice(-limit).map((m) => ({
      ...
      content: isGroup
        ? groupMsgToHistoryText(m.from === 'player' ? pn : (m.senderName || '成员'), m.kind === 'sticker' ? `[表情包] ${m.text}` : m.text)
        : dmMsgToHistoryText(m),
    }));
  }

  /* 聊天：NPC 据人设回一轮（行协议：状态头+心声+1~4条消息，可带撤回/引用/戳花样；解析失败整段兜底纯文本）。
     opts.kind='sticker'：主角发的是表情包（playerText=表情包名，历史压缩成 [表情包] 名喂给对方）。*/
  async function dmReply(threadId: string, playerText: string, opts?: { kind?: 'sticker' }) {
    const dm = useDm.getState();
    const th = dm.threads[threadId]; if (!th) return;
    dm.addMsg(threadId, { from: 'player', text: playerText, kind: opts?.kind });
    const chain = dmChain();
    if (!chain[0]?.baseUrl || !chain[0]?.apiKey) { dm.addMsg(threadId, { from: 'system', text: '（私信 API 未配置：在「设置→变量管理→📡 公共频道」或接口路由 channel 配置）' }); return; }
    const sys = dmPersonaPrompt(th) + `\n\n【主角面板】${dmPlayerCard()}\n\n请以你本人的口吻回复主角这条私信，贴合你的性格与你和主角的关系。` + getPrompt('DM_REPLY_PROTOCOL_RULE', DM_REPLY_PROTOCOL_RULE) + buildStickerInjection();
    try {
      ...
      // 😊 表情包硬过滤（双保险）：名称不在库的 sticker 整条丢弃
      const stSet = stickerNameSet();
      const msgs = parsed.msgs.filter((m) => m.kind !== 'sticker' || stSet.has(m.text));
      if (msgs.length !== parsed.msgs.length) console.warn(`[私信] 😊 过滤了 ${parsed.msgs.length - msgs.length} 条无效表情包（名称不在库）`);
      if (!msgs.length) { dm.addMsg(threadId, { from: 'npc', text: '……' }); return; }
      msgs.forEach((m, i) => {
        dm.addMsg(threadId, {
  ...
  async function runInlineCommPhase() {
    ...
    const sys = dmPersonaPrompt(th) + `\n\n【主角面板】${dmPlayerCard()}\n\n【本次场景】这不是回复——是你**主动**给主角发来私讯。你的动机：${intent.reason || '（见大意）'}；要说的大意：${intent.gist}。据此自然开场，贴合你的性格与你和主角的关系。` + getPrompt('DM_REPLY_PROTOCOL_RULE', DM_REPLY_PROTOCOL_RULE) + buildStickerInjection();
    try {
      const { content } = await apiChatFallback(chain, [{ role: 'system', content: sys }, ...dmHistory(th, 8)], { timeoutMs: 60000 });
      const parsed = parseDmReply(content || '');
      const stSet = stickerNameSet();
      const msgs = parsed.msgs.filter((m) => m.kind !== 'sticker' || stSet.has(m.text));   // 😊 无效表情包名丢弃
      if (!msgs.length) return;
      msgs.forEach((m, i) => dm.addMsg(tid, {
        from: 'npc', text: m.text, kind: m.kind === 'text' ? undefined : m.kind,
        orig: m.orig, quote: m.quote, meta: i === 0 ? parsed.meta : undefined,
      }));
      useDm.getState().bumpUnread(tid, msgs.length);
      console.log(`[来讯] 📨 ${rec.name} 主动来讯 ${parsed.msgs.length} 条（动机：${intent.reason || intent.gist}）`);
    } catch (e) { console.warn('[来讯] 📨 生成失败（本窗口冷却已占用）', e); }
  }

  /* 👥 群聊回合（借鉴Abstract外置手机 group_message·代码全自写）：
     playerText 有值=主角发言→成员回应；undefined=「👂 潜水」→成员自治聊天（借"我方没有发言，让群成员继续聊天"思想）。
     成员人设=紧凑卡（不逐人全量 dmPersonaPrompt——多人全量会爆 token），已建档成员带 性格/称呼/好感/近况。 */
  async function groupChatReply(threadId: string, playerText?: string, opts?: { kind?: 'sticker' }) {
    const dm = useDm.getState();
    const th = dm.threads[threadId];
    if (!th || th.kind !== 'group' || !(th.members?.length)) return;
    const chain = resolveApiChain('groupChat', getChannelApi());   // featureKey='groupChat'·回退频道接口
    if (!chain[0]?.baseUrl || !chain[0]?.apiKey) { dm.addMsg(threadId, { from: 'system', text: '（群聊 API 未配置：接口路由 groupChat 或频道接口）' }); return; }
    const prof = usePlayer.getState().profile;
    const playerName = prof.name || '主角';
    if (playerText && playerText.trim()) dm.addMsg(threadId, { from: 'player', text: playerText.trim(), kind: opts?.kind });
    // 成员紧凑卡：已建档→档案关键项；未建档→只有名字
    const npcs = useNpc.getState().npcs;
    const cards = th.members.map((mb) => {
      const rec = mb.id ? npcs[mb.id] : Object.values(npcs).find((n) => (n.name || '').trim() === mb.name);
      if (!rec) return `- ${mb.name}（信息不详，按名字与上文语境即兴）`;
      const bits = [
        rec.npcTag && `${rec.npcTag}`, rec.realm && `${rec.realm}`, rec.profession && `职业${rec.profession}`,
        rec.personality && `性格:${String(rec.personality).slice(0, 40)}`,
        rec.callPlayer && `称主角:${rec.callPlayer}`,
        `好感${rec.favor}`,
        rec.onScene ? '此刻与主角同在场' : (rec.status ? `近况:${String(rec.status).slice(0, 30)}` : '离场各忙各的'),
        rec.innerThought && `心思:${String(rec.innerThought).slice(0, 30)}`,
      ].filter(Boolean);
      return `- ${mb.name}（${bits.join('；')}）`;
    }).join('\n');
    const sys = `这是「${th.targetName}」群聊（乐园终端群组）。群成员（除主角外，全部由你扮演）：\n${cards}\n\n【主角】${playerName}（${prof.tier || '一阶'}·Lv.${prof.level ?? 1}）也在群里，群里的消息他都看得见。` + getPrompt('GROUP_CHAT_RULE', GROUP_CHAT_RULE) + buildStickerInjection().replace('用一行「贴: 名称」', '用「发言人姓名|贴: 名称」');
    const user = playerText && playerText.trim()
      ? (opts?.kind === 'sticker'
        ? `${playerName}（主角）刚在群里发了一个表情包「${playerText.trim()}」。\n请按行协议输出群成员们的回应（可互相接话，也可以回敬表情包）。`
        : `${playerName}（主角）刚在群里说：「${playerText.trim()}」\n请按行协议输出群成员们的回应（可互相接话）。`)
      : `主角没有发言，只是在看群。让群成员**自发**聊起来——聊他们自己的事、彼此的事、最近的见闻，不必围绕主角，更不要@主角求回应。按行协议输出。`;
    try {
      const { content } = await apiChatFallback(chain, [{ role: 'system', content: sys }, ...dmHistory(useDm.getState().threads[threadId]!, 16), { role: 'user', content: user }], { timeoutMs: 60000 });
      const stSet = stickerNameSet();
      const msgs = parseGroupReply(content || '', { playerName, memberNames: th.members.map((m) => m.name) })
        .filter((m) => m.kind !== 'sticker' || stSet.has(m.text));   // 😊 无效表情包名整条丢弃（双保险）
      if (!msgs.length) { dm.addMsg(threadId, { from: 'system', text: '（群里静悄悄的，没人说话）' }); return; }
      for (const m of msgs) {
        await new Promise((res) => setTimeout(res, 350 + Math.random() * 550));   // 错峰上屏，像陆续有人说话
        useDm.getState().addMsg(threadId, { from: 'npc', senderName: m.sender, text: m.text, kind: m.kind });
      }
    } catch (e: any) { dm.addMsg(threadId, { from: 'system', text: `（群消息没发出去：${(e?.message ?? '').slice(0, 40)}）` }); }
  }

  /* 🕳 窃听（借鉴Abstract外置手机 peek·代码全自写）：监听两名离场角色的私下交谈。
     前端确定性扣乐园币（失败/污染退款·silent 走自己的窃听通报）；认知边界=eventsKnownTo 各自过滤；
     转写复用群聊行协议解析（白名单=这两人）；有概率被察觉→双方好感下降+通报点明。 */
  async function eavesdropNpcs(aId: string, bId: string): Promise<{ ok: boolean; msg: string; lines: { sender: string; text: string }[]; discovered: boolean }> {
    const fail = (msg: string) => ({ ok: false, msg, lines: [] as { sender: string; text: string }[], discovered: false });
    const npcs = useNpc.getState().npcs;
    const a = npcs[aId], b = npcs[bId];
    if (!a || !b || aId === bId) return fail('选两个不同的监听对象');
    if (a.onScene || b.onScene) return fail('只能窃听离场角色（在场的当面听就是了）');
    if (a.isDead || b.isDead) return fail('逝者已矣，监听不到了');
    const items = useItems.getState();
    if ((items.currency.乐园币 ?? 0) < EAVESDROP_COST) return fail(`乐园币不足（窃听一次需 ${EAVESDROP_COST}）`);
    items.adjustCurrency('乐园币', -EAVESDROP_COST, '窃听行动', true);
    const refund = (msg: string) => { useItems.getState().adjustCurrency('乐园币', EAVESDROP_COST, '窃听失败退款', true); return fail(msg); };
    const M = useMisc.getState();
    const wn = M.worldName || '';
    try {
      const sys = buildEavesdropSys(a, b,
        eventsKnownTo(M.worldEvents ?? [], a.name || '', wn, sameWorld),
        eventsKnownTo(M.worldEvents ?? [], b.name || '', wn, sameWorld),
        wn, M.worldTime || M.paradiseTime || '');
      const content = await npcChatCompletion(sys, '开始输出这段交谈（只按行协议，8~14行）。', 'observe');
      if (observeContaminated(content)) return refund('监听信号被污染（生成不合格），费用已退');
      const lines = parseGroupReply(content, { playerName: usePlayer.getState().profile.name || '主角', memberNames: [a.name || '', b.name || ''], cap: 14 });
      if (lines.length < 2) return refund('没听到有效交谈，费用已退');
      const discovered = rollDiscovered();
      if (discovered) {
        const U = useNpc.getState();
        U.upsertNpc(aId, { favor: Math.max(-100, (a.favor ?? 0) - EAVESDROP_FAVOR_HIT) });
        U.upsertNpc(bId, { favor: Math.max(-100, (b.favor ?? 0) - EAVESDROP_FAVOR_HIT) });
      }
      const gist = lines.slice(0, 2).map((l) => `${l.sender}:「${l.text.slice(0, 40)}」`).join('；');
      pushSceneNotice(eavesdropNotice(a.name || '', b.name || '', gist, discovered));
      return { ok: true, msg: '', lines, discovered };
    } catch (e) {
      console.warn('[窃听] 失败', e);
      return refund('监听失败（网络/接口错误），费用已退');
    }
  }
  ...
  function dmActionPlayerLine(kind: import('./store/dmStore').DmDealKind, payload: any, giveName?: string): string {
    ...
    if (kind === 'transfer') return `（我给你转账 ${payload.amount} ${payload.currency || '乐园币'}${payload.note ? `，附言：${payload.note}` : ''}）`;
...
**只输出 JSON**：{"accept":true或false,"reply":"你的话","price":你愿支付的数字(0=收下但不付钱/收礼道谢),"currency":"乐园币|灵魂钱币"}。`;
      ...
      instr = `\n\n主角给你【转账】${payload.amount} ${dmNormCur(payload.currency)}${payload.note ? `，附言：「${payload.note}」` : '（没写附言）'}。这是无偿赠予（不是买东西）。
结合你和主角的关系/好感/你的性格与骨气，决定收不收：可以爽快收下道谢、可以推辞两句再收、也可以拒收（不缺钱/有骨气/觉得被施舍/关系不到位……都是理由）。你也可以对这笔钱的用途或主角的心意说点什么。
**只输出 JSON**：{"accept":true或false,"reply":"你的话(本人口吻,1~2句)"}。`;
...
**只输出 JSON**：{"accept":true或false,"reply":"你的话","extraFromPlayer":数字(主角额外补给你的钱;负数=你补给主角;0=平换),"currency":"乐园币|灵魂钱币","item":{"name":"${payload.wantName}","gradeDesc":"","category":"","effect":"","appearance":""}}。`;
    ...
    try {
      ...
      if (kind === 'transfer') {
        // 💸 转账：金额/币种由玩家定死（AI 只决定收不收——数值前端说了算，AI 改不了）；拒收=只回话不出卡
        if (j.accept === true || j.accept === 'true') {
          const amt = Math.max(1, parseInt(String(payload.amount), 10) || 0);
          deal = { id: dmDealId(), kind: 'transfer', giveCurrency: { amount: amt, type: dmNormCur(payload.currency) }, note: reply, source: 'free', status: 'pending', ts };
        }
      } else if (kind === 'buy') {
...
- items：给 **3~6 件**符合其阶位/职业的随身物品，其中 **1~3 件 equipped:true**（武器≤1、防具1~2、饰品≤1），其余放储存空间；武器/防具的 effect 要写具体攻防数值。`;
  ...
  const dmHandlers: DmHandlers = {
    ...
    onGroupReply: groupChatReply,   // 👥 群聊：发言/潜水两模式
    onEavesdrop: eavesdropNpcs,     // 🕳 窃听：两离场角色私谈（花乐园币·有风险）
...
${getPrompt('EQUIP_CODEX', EQUIP_CODEX)}`;
  ...
  async function genShopItems() {
    ...
    const prof = usePlayer.getState().profile;
    const M = useMisc.getState();
    // P3 经济缝合：任务世界且已设当地货币名 → 商店部分商品用当地货币计价（当地货币此前只有 AI 记账、无任何玩法接口）
    const localName = (M.localCurrencyName || '').trim();
    const localLine = localName && !isHomeWorld(M.worldName || '')
      ? `\n- **当地货币**：本世界流通「${localName}」（主角现持 ${M.localCurrency}）。**约一半商品**（本地土产/凡俗物资/低阶消耗品）用它计价——"currency":"${localName}"；乐园制式装备/高阶物仍用乐园币/魂币。`
      : '';
    const sys = `你是「轮回乐园·系统商店」补货员。一次性生成 **10 件** 待售商品，类别要丰富搭配：消耗品、制式装备(武器/防具/饰品)、技能书/技能卷轴、材料、工具、特殊物品等。
- 贴合当前世界(${M.worldName || '轮回乐园'})与主角阶位(${prof.tier || '一阶'}·Lv.${prof.level})的强度区间；**价格一般偏高**（系统商店溢价，约市场价 1.2~1.8 倍）。${localLine}
- 每件按物品固定格式给全字段。**只输出 JSON**：{"items":[{"name","category"(武器/防具/饰品/消耗品/材料/工具/特殊物品/重要物品等),"subType","gradeDesc"(品质色由低到高:白/绿/蓝/紫/暗紫/淡金/金/暗金/传说级/史诗级/圣灵级/不朽级/起源级/永恒级/创世,按强度选合适档),"price"(数字),"currency"("乐园币"或"魂币"${localName ? `或"${localName}"` : ''}),"effect","combatStat"(装备攻防机器可读如"法术攻击力 60-135"/"防御力 8-12"),"durability","requirement","affix","origin","intro","appearance","qty"(默认1)}]}，共 10 件，不要任何多余文字或 markdown。
...
${getPrompt('EQUIP_CODEX', EQUIP_CODEX)}`;
  ...
  async function genSellQuotes(list: { id: string; name: string; gradeDesc: string; category: string; effect?: string; qty: number }[]) {
    ...
    const M2 = useMisc.getState();
    const localName2 = (M2.localCurrencyName || '').trim();
    const localOpt = localName2 && !isHomeWorld(M2.worldName || '') ? `或"${localName2}"(本地凡俗物/土产给当地货币价)` : '';
    const sys = `你是「轮回乐园·系统回收」估价员，为主角要出售的物品逐件给出**回收报价**（系统回收价约市场价 50%~80%，按品质/效果/稀有度/数量估算；品质越高价越高）。
**只输出 JSON**：{"quotes":[{"id":"原样照抄物品id","price":数字,"currency":"乐园币"或"魂币"${localOpt}}]}，每件一条，不要多余文字。
...
${lines}`;
  ...
  function applyFactionShortCommands(reply: string, onlyId?: string): number {
    ...
    const strFields = ['status', 'goal', 'territory', 'leader', 'resources', 'scale', 'powerLevel', 'type'];
    for (const f of strFields) {
      const re = new RegExp(`\\bfaction\\.(F\\d+)\\.${f}\\s*=\\s*"([^"]*)"`, 'g');
      while ((m = re.exec(reply))) { if (ok(m[1])) { fac.upsertFaction(m[1], { [f]: m[2] } as any); n++; } }
    }
    // ── 外交关系：**过八级闸门**（systems/diplomacy）而不是照单全收 ──
    //   relations 仍是那串自由文本存储（不动 schema），但每条边逐一比对新旧档位：
    //   跨 ≥2 档且无已完结事件链 → 回落到"朝目标渐变 1 档"，并打日志。
    //   例外直降（核心人物被杀/重大背叛/公开宣战）从本轮 reply 里找依据。
    {
      const relRe = /\bfaction\.(F\d+)\.relations\s*=\s*"([^"]*)"/g;
      const exc = normException(reply);
      while ((m = relRe.exec(reply))) {
        const fid = m[1];   // ⚠ 先取出来：m 会被下一次 exec 重置，闭包里直接用 m[1] 既不安全也过不了类型窄化
        if (!ok(fid)) continue;
        const prev = parseLegacyRelations(fac.factions[fid]?.relations);
        const next = parseLegacyRelations(m[2]);
        const prevBy = new Map(prev.map((e) => [e.target, e.level]));
        const gated = next.map((e) => {
          const from = prevBy.get(e.target);
          if (from == null) return e;   // 新出现的关系：没有历史可比，照收
          const v = canTransition(from, e.level, { exception: exc });
          if (!v.ok) console.warn(`[外交] ${fid}→${e.target} ${diploName(from)}→${diploName(e.level)} 被拦：${v.reason}`);
          return { ...e, level: v.ok ? e.level : v.fallback };
        });
        // 保留 AI 没提到的旧关系（治"每次只写两条，其余全被抹掉"）
        const merged = [...gated];
        for (const p of prev) if (!merged.some((x) => x.target === p.target)) merged.push(p);
        fac.upsertFaction(fid, { relations: formatEdges(merged) });
        n++;
      }
    }
    return n;
  ...
  function applyNarrativeVitals(narrative: string) {
    ...
    // ★优先认「以 主角/你/我 开头的整行」（<状态结算> 逐行一个角色，主角一行同时含 HP 与 EP）：
    //   从这一行里同取 HP+EP，与该行是否有 NPC、NPC 是否已建档全无关——根治"主角行(96/100)后紧跟
    //   NPC 行(女性村民 14/20)时、NPC 的当前值被 grabLast 当成最后匹配写成主角血量"。取最后一条主角行=最新。
    const SELF_LINE_RE = /^[ \t　【\[（(]*(?:主角|你|我)(?:[\s：:的】\]）)（(][^\n]*)?$/gim;
    let selfHp: [number, number] | null = null, selfEp: [number, number] | null = null;
    for (let lm: RegExpExecArray | null; (lm = SELF_LINE_RE.exec(narrative)) !== null; ) {
      const line = lm[0];
      const h = line.match(/当前\s*(?:HP|血量|生命值?)\s*[:：]\s*(\d{1,7})\s*\/\s*(\d{1,7})/i);
      const e = line.match(/当前\s*(?:EP|MP|蓝量|法力|能量|精力)\s*[:：]\s*(\d{1,7})\s*\/\s*(\d{1,7})/i);
      if (h) selfHp = [Number(h[1]), Number(h[2])];
      if (e) selfEp = [Number(e[1]), Number(e[2])];
    }
    // 先认「当前HP：X/Y」（主角行优先，否则 grabLast 兜底）；没有则认成长块箭头「HP: 180/180 -> 400/400」取箭头后的最终值
    let hp = selfHp ?? grabLast('当前\\s*(?:HP|血量|生命值?)\\s*[:：]\\s*(\\d{1,7})\\s*/\\s*(\\d{1,7})', true);
    ...
    let ep = selfEp ?? grabLast('当前\\s*(?:EP|MP|蓝量|法力|能量|精力)\\s*[:：]\\s*(\\d{1,7})\\s*/\\s*(\\d{1,7})', false);
  ...
  function ensureNpcVitalsCap() {
    ...
    for (const r of Object.values(npc.npcs)) {
      ...
      const aHp = npcBaseAttrs(r);   // HP/EP 基础六维含真实属性点直加(realAttrs)；bioInnate 资质档仍用基础六维 a
      const maxHp = Math.round(computeMaxHp(aHp, rmR, ratioOf(r)) * mult);
      const maxMp = Math.round(computeMaxEp(aHp, rmR, ratioOf(r)) * mult);
  ...
  function captureTurnSnapshot() {
    try {
      ...
      useTurnInsight.getState().pushSnapshot({
        ...
        facilityNotes: peekFacilityLog(),   // P4·设施动态：本窗口场外动作（peek 不清空·新回合发送时清，见 callApi）
        player: (() => {
  ...
  function pruneGhostNpcs(): number {
    // 回合末兜底：判定共用 npcStore.isGhostNpc（与 NPC facade 闸门单一来源）。幽灵的即时清除已由 facade
    // 闸门(subscribe·删沉淀幽灵)承担，本函数保留为回合末全量兜底（含 facade 未及的"本回合最后一次变动才冒出的幽灵"）。
    const npc = useNpc.getState();
    const charData = useCharacters.getState().characters;
    const ghosts = Object.values(npc.npcs).filter((r) => isGhostNpc(r, charData)).map((r) => r.id);
  ...
  function buildChoicesPhaseContext(text: string, includeQuest: boolean): string {
    ...
    // 主角人设（性格/身份/背景）单独提前置顶 → 选项必须贴合，别写成清一色冷酷装逼
    const pf = P.profile;
    const persona = [
      pf.name && `主角:${pf.name}`, pf.identity && `身份:${pf.identity}`, pf.profession && `职业:${pf.profession}`,
      pf.personality && `性格:${pf.personality}`, pf.personalityDetail && `性格描述:${pf.personalityDetail}`,
      pf.background && `背景:${pf.background}`,
    ].filter(Boolean).join('｜');
    return [
      worldName ? `【当前世界】${worldName}（若为已知原著世界，按"原著接轨"要求联网搜其剧情、让选项接入原著剧情线）` : '',
      persona ? `【主角人设·选项必须贴合此性格与语气（最高优先，勿写成清一色冷酷/逞强）】${persona}` : '',
      `【主角全部信息】\n${playerCard}`,
      `【在场角色全部信息（含持有物）】\n${npcBlocks || '（无）'}`,
  ...
  async function runChoicesFanficPhase(narrative: string, assistantMsgId?: number, direction?: string) {
    ...
    if (wantChoices) sysParts.push(renderPrompt(ss.choicesPrompt?.trim() || PLOT_CHOICES_RULE));   // 玩家在设置里自定义了就整段替换，否则用内置；过一遍变量标签
    if (wantTheater) {
      ...
      // 🎭 花样模板：从玩家维护的清单里随机抽 pickCount 条（全禁用/空则回退内置全集，见 pickTemplates 兜底）
      const T = useTheater.getState();
      T.ensureSeeded();
      const styles = buildTheaterStyleBlock(pickTemplates(useTheater.getState().templates, T.pickCount));
      if (styles) sysParts.push(styles);
    }
  ...
  async function runArenaLadderPhase(arenaId: string, def: ArenaDefType, ranks: number[], windowKey: string): Promise<void> {
    ...
    // 🛤 原著路线：白夜挂榜（同阶·乐园内部竞技场）——占最靠前的非主角名次，实力随站点查表；"他在你前头"的压迫感零成本常驻
    try {
      const CR = useCanonRoute.getState();
      if (CR.enabled && CR.suxiao.state !== 'dead' && def.kind !== 'championship' && def.kind !== 'tree') {
        const s = canonSuxiaoEntryStats(CR.stationIndex);
        if (normalizeTier(s.tier) === normalizeTier(pTier) && !entries.some((e) => e.name === '白夜')) {
          const spot = entries.filter((e) => !e.isPlayer).sort((a, b) => a.rank - b.rank)[0];
          if (spot) { spot.name = '白夜'; spot.tier = `${s.tier}·Lv.${s.lv}`; spot.job = CR.stationIndex >= 1 ? '灭法之影' : ''; spot.persona = '冷酷寡言·贪财务实'; spot.badge = ladderBadge(def.kind, spot.rank); }
        }
      }
    } catch { /* */ }
  ...
  async function genArenaOpponent(def: ArenaDefType, entry: ArenaLadderEntry): Promise<string | null> {
    // 🛤 原著路线：挑战「白夜」＝挑战 canon 锁定档案本体（不另建临时对手；数值查表锁死·可敌可杀）
    try {
      const CRo = useCanonRoute.getState();
      if (CRo.enabled && entry.name === '白夜' && CRo.suxiao.npcId && useNpc.getState().npcs[CRo.suxiao.npcId]) return CRo.suxiao.npcId;
    } catch { /* */ }
    const npc = useNpc.getState();
    const prof = usePlayer.getState().profile;
  ...
  async function grantArenaReward(pending: { arenaId: string; arenaName: string }, rank: number): Promise<string> {
    ...
    if (coin > 0) items.adjustCurrency('乐园币', coin, `竞技场·${pending.arenaName} 第${rank}名奖励`);
    if (soul > 0) items.adjustCurrency('灵魂钱币', soul, `竞技场·${pending.arenaName} 第${rank}名奖励`);
    let titleStr = '';
    let grantedTitle = '';
    if (band.giveTitle) {
    ...
    // 场外通报 + 发放登记：奖励物品此前没登记 granted（审计确认）→ 物品阶段可能把同名奖励再 createItem 一遍
    reportFacilityOutcome({
      source: '竞技场',
      summary: `主角在${pending.arenaName}晋升至第 ${rank} 名，领取${band.label}奖励：${names.map((n) => `「${n}」`).join('、')}已入背包${coin > 0 ? `、乐园币+${coin}` : ''}${soul > 0 ? `、灵魂钱币+${soul}` : ''}${titleStr}`,
      granted: names,
      growth: grantedTitle ? `因登顶${pending.arenaName}获得称号「${grantedTitle}」——可自然带过观众席的哗然与名声，不强求成段` : undefined,
    });
  ...
  function runPostNarrativePhases(narrative: string, assistantMsgId?: number) {
    ...
    const settleChain = pipe.snapshotReady.then(() => {
      ...
      try { enforceCanonLock(); } catch (e) { console.warn('[原著路线] canon 锁复位失败', e); }   // 🛤 白夜数值钉回站点基准（在体检后跑，体检的重生成/夹平也会被钉回）
  ...
  /* 🫀 逐角色心流独白（借鉴SoulLink·默认关）：sendMessage 一开始就把本批推演**并发起跑**——
     与检定审核/细纲生成与编辑/推进规划这些等待窗口重叠，正文 callApi 组装时限时收割（fail-open）。
     产物不落任何 store：⟳重生成=先回退再重放会重新起跑；每次发送先重置 ref，绝不消费上一回合的过期心流。 */
  const mindflowPendingRef = useRef<Promise<string> | null>(null);
  function startMindflowForTurn(userText: string): void {
    mindflowPendingRef.current = null;   // 先重置：本回合没起跑就是没有
    const ss = useSettings.getState();
    if (!ss.mindflowEnabled) return;
    if (useMp.getState().status === 'connected') return;   // 联机正文走房主权威流程，不掺单机心流
    const routeIds = ss.apiRoutes?.['npcMindflow'] ?? [];
    const chain = routeIds.length ? resolveApiChain('npcMindflow', getNpcApi()) : resolveApiChain('npc', getNpcApi());
    if (!chain[0]?.baseUrl || !chain[0]?.apiKey) return;   // 未配接口：零开销直接放行
    const lastNarr = [...messagesRef.current].reverse().find((m) => m.role === 'assistant')?.content ?? '';
    mindflowPendingRef.current = runMindflowBatch({
      npcs: Object.values(useNpc.getState().npcs),
      userText,
      recentTail: lastNarr.slice(-2600),
      chain,
      max: ss.mindflowMax ?? 2,
    });
  }
  ...
  async function runPlotGuidance(userText: string): Promise<string> {
    ...
    const questScene = [...buildQuestInjection(), ...buildWorldTimeInjection(), ...buildCanonWorldInjection(), ...buildSuxiaoTrackInjection()].map((m) => m.content).join('\n');
    // 剧情状态快照（数据库 4 张剧情记忆表：纪要最近/进程/伏笔/约定）——让导演状态感知：回收伏笔、兑现约定、推进进程，而非空转
    const plotState = buildPlotStateSnapshot();
    const sys = renderPrompt((guidancePrompt && guidancePrompt.trim()) ? guidancePrompt : PLOT_GUIDANCE_RULE);
  ...
  async function callApi(userText: string, extraHistory: ChatMessage[] = [], opts: { narrateOnly?: boolean; task?: 'outline' | 'dbAdvance'; outline?: string; guidance?: string; dbAdvance?: string; images?: string[]; onDelta?: (t: string) => void } = {}) {
    ...
    setLastUserMessage(userText);   // 供 {{lastUserMessage}}/${玩家输入} 在前置提示词等的变量标签里引用（含 outline/dbAdvance 草稿也用同一句）
    if (!narrateOnly) {
      turnCountRef.current += 1;
    ...
    reconcileWorldScope();                 // 🧊 人在乐园却还有任务世界土著没冻（读档/AI 直接改 worldName 等路径）→ 补冻
    ...
    // 🎯 activeWhen 条件门（P1·借鉴 ST-PT @@if 思想）：条目可选「激活条件」（<if cond> 语法：var:/cell:/seed:/random:），
    //   不满足=本回合视同禁用。vars 惰性采集（全库无条件条目=零开销）；seed: 原子口径=关键词命中同款 matchCtx（输入+近10楼）。
    let _wbVars: Record<string, string> | undefined;
    const _wbWhenPass = (e: { activeWhen?: string }) => {
      const w = (e.activeWhen || '').trim();
      if (!w) return true;
      if (!_wbVars) _wbVars = buildRuntimeVars();
      return passActiveWhen(w, { seedContent: matchCtx, vars: _wbVars });
    };
    const wbEntries = textWorldBooks
      .filter((b) => b.enabled)
      .flatMap((b) => b.entries.filter((e) =>
        e.enabled && _wbWhenPass(e) && (
    ...
    try {
      ...
      if (_needsSql) await ensureSqliteMirror();
    ...
    if (!narrateOnly) {
      const preludeTxt = renderPrompt((useSettings.getState().preludePrompt || '').trim());
      const sceneNotices = drainSceneNotices(useItems.getState().currency as unknown as Record<string, number>);
      clearFacilityLog();   // P4·洞察设施区：上一窗口的设施动作已随上回合快照定格，新回合从零记（与通报 drain 同窗口）
      const parts: string[] = [];
    ...
    const diploBlock: { role: 'system'; content: string }[] = (() => {
      try {
        const rows = Object.values(useFaction.getState().factions)
          .filter((f) => f.inCurrentWorld && !f.isDestroyed && f.name)
          .map((f) => ({ name: f.name, edges: parseLegacyRelations(f.relations) }));
        const txt = buildDiplomacyInjection(rows);
        return txt ? [{ role: 'system' as const, content: txt }] : [];
      } catch { return []; }
    })();

    // 市井流言：只把「街面上在传什么」喂进正文，**真相不喂**——NPC 因此可能基于错误信息说话/行动，
    // 这正是传闻系统的价值所在（真相只进演化阶段与玩家面板）。够不上「局部焦点」的不占预算。
    const rumorBlock: { role: 'system'; content: string }[] = (() => {
      try {
        const txt = buildRumorInjection(currentWorldRumors());
        return txt ? [{ role: 'system' as const, content: txt }] : [];
      } catch { return []; }
    })();
    ...
    if (vm.enabled && vm.apiBase && vm.apiKey) {
      ...
      try {
        ...
        // rerank 精排（可选·默认关）：开则余弦先粗召回一批候选(rerankCandidates) → rerank 交叉编码精排 → 取 topK；关/失败则纯余弦取 topK。
        const rrOn = !!(vm.rerankEnabled && vm.rerankBase && vm.rerankKey);
        const candN = rrOn ? Math.max(vm.topK ?? 6, vm.rerankCandidates ?? 40) : (vm.topK ?? 6);
        let hits = qvec ? factVecSearch(qvec, pool.map((p) => p.key), candN, vm.threshold ?? 0.3) : [];
        let reranked = false;
        if (rrOn && hits.length > 1) {
          try {
            const cands = hits.map((h) => ({ key: h.key, body: byKey.get(h.key)?.body ?? '' })).filter((c) => c.body);
            const rr = await factVecRerank(ctx, cands, vm);
            if (rr.length) { hits = rr; reranked = true; }
    ...
    // 🤖 Agent 正文模式·初始历史裁剪（仿 TT context policy initialChatHistoryMessages·省 token 核心杠杆）：
    //   只裁「发给模型的原文楼层」（recent）——世界书关键词匹配/向量与叙事召回/结构化档案照常用真实近况，
    //   上面的记忆去重也已按完整 recent 做完；模型可用 chat_search/chat_read_messages 主动查更早剧情
    //   （系统提示词会注明本回合历史被裁）。-1=不裁（默认·跟随全局楼层限制）。
    if (!narrateOnly) {
      const _agHist = useSettings.getState().agentNarrative;
      const _n = _agHist?.enabled ? Math.floor(_agHist.initialHistoryMsgs ?? -1) : -1;
      if (_n >= 0 && recent.length > _n) recent = _n === 0 ? [] : recent.slice(-_n);
    }
    ...
    // 🫀 <角色心流·导演注>（借鉴SoulLink·默认关）：收割发送时并发起跑的逐角色推演。
    //   多数情况下细纲/推进的等待早把它熬熟了（此处秒取）；纯直发路径最多再等 20s，超时/失败=不注入（fail-open）。
    //   只进真正的正文调用：细纲/推进等规划任务(task)与分头渲染(narrateOnly)不吃；消费即清，绝不复用过期心流。
    let mindflowMsgs: { role: 'system'; content: string }[] = [];
    if (!opts.task && !narrateOnly && mindflowPendingRef.current) {
      const _mfP = mindflowPendingRef.current; mindflowPendingRef.current = null;
      try {
        const _mfTxt = await Promise.race([_mfP, new Promise<string>((r) => setTimeout(() => r(''), 20000))]);
        if (_mfTxt) mindflowMsgs = [{ role: 'system', content: _mfTxt }];
        else console.log('[心流] 超时或无产出，本回合不注入');
      } catch { /* fail-open：心流绝不拦正文 */ }
    }
    const history = [
      ...examples,
      ...
      ...mindflowMsgs,                                  // 🫀 <角色心流·导演注> 逐角色独立推演的第一人称心流（借鉴SoulLink·默认关·fail-open·见 systems/mindflow）
    ...
    // 🎯 自定义注入（预设中心·P1·借鉴 ST-PT @INJECT 思想）：玩家条目按位置插入最终 history。
    //   最后一步应用、与预设内部 injection_position/order 体系不混算；prefill 由 protectTail 保住末位；
    //   activeWhen 不满足 / 内容展开为空 / 正则未命中 = 该条静默跳过。细纲(isOutline)走自己的 outlineHistory，不受影响。
    try {
      const _injList = usePromptOverride.getState().injects ?? [];
      if (_injList.some((j) => j && j.enabled)) {
        const _injCtx = makePromptExpandCtx({
          user: usePlayer.getState().profile?.name || '主角',
          char: usePlayer.getState().profile?.name || '主角',
          lastUserMessage: userText,
          seedContent: matchCtx,   // seed: 口径与世界书激活一致（输入+近10楼）
        });
        applyCustomInjects(history, _injList, {
          expand: (t) => expandPromptText(t, _injCtx, false, true),   // 玩家文本：不清残留+未知变量原样保留
          cond: (x) => passActiveWhen(x, _injCtx.tableCtx),
          protectTail: effectivePrefill ? 1 : 0,
        });
      }
    } catch (e) { console.warn('[自定义注入] 应用失败（忽略·不阻断正文）', e); }
    ...
    try {
      ...
      for (let ci = 0; ci < apiChain.length; ci++) {
        ...
        // 多 Key 内环：401/403/429 换下一个 Key 重试同一接口（Gemini 免费池玩法）；其余错误切下一条接口
        const epKeys = orderedKeys(ep);
        let gotRes: Response | null = null;
        for (let ki = 0; ki < epKeys.length; ki++) {
          try {
            // 按协议出请求：openai=Bearer+/chat/completions；anthropic/gemini=原生端点+头+body 变换（见 systems/apiUrl）
            const rq = buildProtocolRequest(ep, epKeys[ki], applyOmitParams(reqBody, ep.omitParams));
            const r = await fetchWithProxy(rq.url, { method: 'POST', headers: rq.headers, body: JSON.stringify(rq.body), signal: ac.signal });
            if (!r.ok) {
              const errText = await r.text();
              lastErr = new Error(`HTTP ${r.status}: ${errText.slice(0, 160)}`);
              if (isKeyRotatableStatus(r.status) && epKeys.length > 1) {
                markKeyBad(ep, epKeys[ki], r.status);
                if (ki < epKeys.length - 1) { console.warn(`[正文] Key ${ki + 1}/${epKeys.length} HTTP ${r.status}，轮换下一个 Key：${ep.modelId}`); continue; }
              }
              console.warn(`[正文] 接口失败${ci < apiChain.length - 1 ? '，回退下一条' : ''}：${ep.modelId}`, lastErr);
              break;
            }
            gotRes = r; markKeyGood(ep, epKeys[ki]); break;
          } catch (e) { lastErr = e; console.warn(`[正文] 接口异常${ci < apiChain.length - 1 ? '，回退下一条' : ''}：${ep.modelId}`, e); break; }
        }
        if (gotRes) { res = gotRes; usedApi = ep; break; }
      }
      ...
      if (useStream) {
          ...
          while (true) {
            ...
            for (const line of lines) {
              ...
              try {
                ...
                const delta = String(extractStreamDelta(protocolOf(usedApi), json) ?? '');   // 按 usedApi 协议抽增量（openai/anthropic/gemini）
                if (delta) {
                  accumulated += delta;
        ...
        const reply: string = String(extractOnceContent(protocolOf(usedApi), data) ?? '');
  ...
  function runNavAction(label: string) {
    const open =
      ...
      /* P4·⌘K 深动作：不占右侧导航、只从命令面板直达的二级功能（此前二级以下在 ⌘K 完全搜不到） */
      label === '装备强化' ? () => setEnhancePanelOpen(true) :
      label === '技能升级' ? () => setSkillUpPanelOpen(true) :
      label === '竞技场' ? () => { sweepArenaOpponents(); setArenaPanelOpen(true); } :
      label === '赌场' ? () => setCasinoOpen(true) :
      label === '欢愉宫' ? () => setJoyPanelOpen(true) :
      ...
      label === '形象工坊' ? () => setOutfitWorkshopOpen(true) :
  ...
  async function sendMessage(textArg?: string, fromAuto = false, sendOpts: { skipOutline?: boolean; forceMpSend?: boolean } = {}) {
    ...
    // 🫀 逐角色心流独白（默认关）：此刻起跑，与下面检定/细纲的等待窗口并行，正文组装时收割（fail-open 不拦发送）
    try { startMindflowForTurn(text); } catch (e) { console.warn('[心流] 起跑失败（本回合跳过）', e); }
  ...
  async function confirmCreation(d: CreationData) {
    ...
    // 🛤 原著路线：创建时勾选则激活（clearProgress 已清上一局路线进度；指向第 1 站）
    if (d.canonRoute) { const cr = useCanonRoute.getState(); cr.setEnabled(true); cr.enterStation(0); }
  ...
  // 🌐 世界观生成核心：读主角当前阶位/等级 + 卡片全字段 → AI → 返回 Worldview（失败弹窗并返回 null）。卡片生成/面板重生成共用。
  async function genWorldviewCore(world: {
    name: string; tier?: string; worldType?: string; dangerLevel?: string; peakPower?: string; contractorDist?: string;
    region?: string; desc?: string; entryPoint?: string; identity?: string; mainMission?: string; sideMission?: string; warning?: string; reward?: string;
  }): Promise<Worldview | null> {
    const chain = resolveApiChain('world', useSettings.getState().api).filter((a) => a.baseUrl && a.apiKey);
    if (chain.length === 0) { alert('请先配置 API（世界选择的接口路由或全局 API），再生成世界观'); return null; }
    const prof = usePlayer.getState().profile;
    const tier = prof.tier || realmFromLevel(prof.level ?? 1);
    const playerBlock = [
      '【主角档案（据此把世界的强度/规模/剧情烈度调到与主角相称·务必合理）】',
      `阶位：${tier}　等级：Lv.${prof.level ?? 1}`,
      prof.homeParadise && `所属乐园：${prof.homeParadise}`,
      prof.profession && `职业：${prof.profession}`,
      prof.identity && `当前身份：${prof.identity}`,
    ].filter(Boolean).join('\n');
    const cardBlock = [
      '【世界卡片（唯一事实来源·世界观必须与此一致、不得矛盾或另造世界）】',
      `世界名：${world.name}`,
      world.worldType && `类型：${world.worldType}`,
      world.tier && `阶位：${world.tier}`,
      world.dangerLevel && `难度：${world.dangerLevel}`,
      world.peakPower && `巅峰战力：${world.peakPower}`,
      world.contractorDist && `契约者分布：${world.contractorDist}`,
      world.region && `任务区域：${world.region}`,
      world.identity && `主角身份：${world.identity}`,
      world.desc && `世界简介：\n${world.desc}`,
      world.entryPoint && `切入点：\n${world.entryPoint}`,
      world.mainMission && `主线任务：\n${world.mainMission}`,
      world.sideMission && `支线任务：\n${world.sideMission}`,
      world.warning && `警告：\n${world.warning}`,
    ...
    try {
      ...
      const wv = lenientJsonParse(jsonText) as Worldview;
      if (!wv || typeof wv !== 'object' || !wv.世界名) throw new Error('返回的不是有效世界观 JSON（可点「查看返回」排查）');
      wv.世界名 = world.name;   // 世界名以卡片为准，防 AI 改名导致同名匹配错位
      return wv;
    } catch (e: any) {
      alert('世界观生成失败：' + (e?.message ?? String(e)));
      return null;
    }
  }

  // 🌐 卡片阶段：生成世界观 → 存 draft 记录（含完整卡片快照供日后重生成）。
  async function generateWorldview(index: number, world: WorldOption) {
    setGenWorldviewBusy(index);
    try {
      const wv = await genWorldviewCore(world);
      if (wv) {
        const prof = usePlayer.getState().profile;
        useWorldRecord.getState().upsertDraft({
          name: world.name, tier: String(world.tier || ''), worldview: wv,
          cardSnapshot: {
            name: world.name, tier: String(world.tier || ''), worldType: world.worldType, dangerLevel: world.dangerLevel,
            peakPower: world.peakPower, contractorDist: world.contractorDist, region: world.region, desc: world.desc,
            entryPoint: world.entryPoint, identity: world.identity || '', mainMission: world.mainMission,
            sideMission: world.sideMission, warning: world.warning, reward: world.reward,
          },
          playerTier: prof.tier || realmFromLevel(prof.level ?? 1), playerLevel: prof.level ?? 1,
        });
      }
  ...
  async function regenWorldviewForRecord(recordId: string) {
    ...
    if (!rec) return;
    if (worldviewBusyId) { alert('已有一个世界观在生成中，请稍候'); return; }
    setWorldviewBusyId(recordId);
    try {
      const cs = rec.cardSnapshot || {};
      const wv = await genWorldviewCore({ ...cs, name: rec.name, tier: rec.tier || cs.tier });
      if (wv) useWorldRecord.getState().setWorldview(recordId, wv);
    } finally { setWorldviewBusyId(null); }
  }

  // 📜 离世总结：读本世界经历（叙事切片=【进入世界】章节 worldTag + 已结算任务 + 世界大事 + 长期事实 + 主角定格）→ AI → 存进世界记录。
  //   玩家自选触发（切世界时 confirm 询问·可拒绝；或世界记录面板手动补生成）。
  async function runWorldSummaryPhase(recordId: string): Promise<boolean> {
    const rec = useWorldRecord.getState().getById(recordId);
    if (!rec) return false;
    if (summaryBusyId) { alert('已有一个离世总结在生成中，请稍候'); return false; }
    const chain = resolveApiChain('misc', useSettings.getState().api).filter((a) => a.baseUrl && a.apiKey);
    if (chain.length === 0) { alert('请先配置 API，再生成离世总结'); return false; }

    const M = useMisc.getState();
    const prof = usePlayer.getState().profile;
    const norm = normWorldName(rec.name);
    // 1) 本世界叙事切片：从最后一条【进入世界：该世界】楼层到结尾（worldTag = 进入世界章节标记，精准世界作用域）。
    const msgs = messagesRef.current ?? [];
    let start = 0;
    for (let i = msgs.length - 1; i >= 0; i--) {
      const m = /【进入世界[:：]\s*([^\]】]+)[\]】]/.exec(msgs[i].content || '');
      if (m && normWorldName(m[1]) === norm) { start = i; break; }
    }
    const sliceRaw = msgs.slice(start).map((mm) => `${mm.role === 'user' ? '▶ ' : ''}${stripStateBlocks(mm.content || '')}`).join('\n\n').trim();
    const narrative = sliceRaw.length > 16000 ? sliceRaw.slice(-16000) : sliceRaw;   // 取近段防超长
    // 2) 本世界已结算任务（archivedTasks 带 worldName）。
    const tasks = M.archivedTasks.filter((t) => normWorldName(t.worldName || '') === norm)
      .map((t) => `· ${t.name}｜${t.status || ''}｜奖:${t.reward || '—'}`).join('\n') || '（无）';
    // 3) 世界大事 + 长期事实（narrativeFacts）。
    const events = M.worldEvents.slice(-12).map((w) => `· [${w.time || ''}${w.location ? '·' + w.location : ''}] ${w.desc}`).join('\n') || '（无）';
    const facts = M.narrativeFacts.slice(-20).map((f) => `· ${f.title}：${f.text}`).join('\n') || '（无）';
    // 4) 主角离世定格。
    const tier = prof.tier || realmFromLevel(prof.level ?? 1);
    const a = prof.attrs || ({} as any);
    const sixDim = `力${a.str ?? '?'}/敏${a.agi ?? '?'}/体${a.con ?? '?'}/智${a.int ?? '?'}/魅${a.cha ?? '?'}/幸${a.luck ?? '?'}`;

    const userMsg = [
      `【离开的世界】${rec.name}${rec.tier ? `（${rec.tier}）` : ''}`,
      rec.worldview ? `【本世界世界观骨架（背景参照）】\n${formatWorldviewForInjection(rec.worldview)}` : '',
      `【本世界经历·正文切片（主要依据·主角的所作所为都在这里）】\n${narrative || '（无正文记录）'}`,
      `【本世界已结算任务（奖励参考）】\n${tasks}`,
      `【世界大事】\n${events}`,
      `【长期事实 / 记忆】\n${facts}`,
      `【主角离世定格（据此填「离世定格」）】\n阶位:${tier} Lv.${prof.level ?? 1}｜六维:${sixDim}｜所属乐园:${prof.homeParadise || '—'}`,
      `请据以上，为主角总结这一趟离开【${rec.name}】的经历，输出 WorldSummary JSON（只输出 JSON）。`,
    ].filter(Boolean).join('\n\n');
  ...
  async function enterWorld(world: WorldOption, opts?: { profQuests?: string; profTasks?: { name: string; desc?: string; reward?: string }[] }) {
    ...
    try {
      const prevActive = useWorldRecord.getState().getActive();
      if (prevActive && !prevActive.summary && normWorldName(prevActive.name) !== normWorldName(world.name || '')
        && confirm(`离开【${prevActive.name}】——要让 AI 为这一趟生成「离世总结」吗？\n（记录主角的所作所为与收获，供日后回看 / 同名再入继承；会调用一次 AI）\n\n确定 = 生成　取消 = 不生成`)) {
        void runWorldSummaryPhase(prevActive.id);   // 后台生成，不阻塞进入新世界（用当前 messagesRef 切片，先捕获）
      }
    } catch { /* */ }
    ...
    // 前尘提要（P2 读回）：上一世界的编纂正史（编年史·玩家修过的金银档）→ 新世界过场的跨世界记忆；
    // 无编纂退回离世总结。此刻 getActive() 仍是上个世界（WR 激活在下方才发生）。让编年史从"相册"变成 AI 的前情记忆。
    try {
      const prev = useWorldRecord.getState().getActive();
      if (prev && normWorldName(prev.name) !== normWorldName(world.name || '')) {
        const saga = buildPriorSaga(prev.name, useChronicle.getState().compiled[prev.id], prev.summary as never);
        if (saga) lines.push(`\n前尘提要（主角此前的轮回足迹·背景记忆——心境/名声/习惯可自然延续，勿重播旧事件、勿把旧世界人物搬进本世界）：\n${saga}`);
      }
    } catch { /* 无前尘则跳过 */ }
    ...
    try {
      ...
      const turnNow = useMisc.getState().turnCount;
      const priors = WR.priorLeftByName(world.name || '');
      if (priors.length > 0) {
        const last = priors[priors.length - 1];
        const evalStr = last.summary?.综合评价 ? `·上次评价 ${last.summary.综合评价}` : '';
        if (confirm(`【${world.name}】检测到历史记录（第 ${last.instanceId} 次${evalStr}）。\n\n确定 = 继承上次进度（把你上次在此世界的身份 / 名声 / 遗留局势注入正文·续写）\n取消 = 重置为全新世界（干净开局·不继承）`)) {
          WR.inheritWorld({ name: world.name || '', tier: String(world.tier || ''), fromId: last.id, turn: turnNow });
          inheritedThisEntry = true;   // → 下面 thawWorld 把上次冻结在这个世界的土著/势力捞回来
        } else {
          WR.activateWorld({ name: world.name || '', tier: String(world.tier || ''), turn: turnNow });
        }
      } else {
        WR.activateWorld({ name: world.name || '', tier: String(world.tier || ''), turn: turnNow });
      }
    } catch { /* */ }
    // 进入新任务世界：主角「身份」采用卡片生成的 identity（无则清空·不带上个世界的身份）；治"进新世界还挂着上个世界身份"。
    try { if (world.name && !isHomeWorld(world.name)) usePlayer.getState().setProfile({ identity: (world.identity || '').trim() }); } catch { /* */ }
    // ── 世界作用域·冻结/解冻（systems/worldScope.ts·适配公理1）──
    //   离开上个任务世界 → 把属于它的 world 作用域数据（土著NPC + 势力）冻结进库房：退出活跃视图，
    //   不再被演化选焦点 / 轨道A 每回合织行动（此前旧土著会一直烧 API 和 tick），但数据仍在、可捞回。
    //   ⚠ 只有玩家在上面选了「继承」才 thaw；选「重置」= 干净开局，旧数据留在库房不动。
    try {
      const turnNow = useMisc.getState().turnCount;
      if (prevWorldForScope && !sameWorld(prevWorldForScope, world.name || '')) {
        // 四维声誉是 world 作用域：离世折算成一句名声写进上一世界的「继承要点」，然后重置。
        // 同名再入选「继承」时，这句话会随 inheritAnchors 注入正文续写。
        try {
          const rep = usePlayer.getState().profile.repute;
          if (rep) {
            const line = summarizeRepute(rep);
            const WR = useWorldRecord.getState();
            const prev = WR.records.find((r) => sameWorld(r.name, prevWorldForScope) && r.status !== 'draft');
            if (prev) WR.updateRecord(prev.id, { summary: { ...(prev.summary ?? {}), 继承要点: { ...(prev.summary?.继承要点 ?? {}), 主角名声: line } } });
            usePlayer.getState().setProfile({ repute: undefined });
      ...
      if (!inheritedThisEntry) {
        const seeds = seedRumorsFromWorldCard({
          plotDrift: world.plotDrift, priorLegacy: world.priorLegacy,
          worldTime: useMisc.getState().worldTime, turn: turnNow,
        });
        for (const s of seeds) useMisc.getState().addRumor({ name: s.name, impact: s.impact, scope: s.scope, node: s.node });
        if (seeds.length) console.log(`[传闻] 🌱 从世界卡播种 ${seeds.length} 条初始传闻`);
      }
      if (inheritedThisEntry) {
    ...
    // 进入新任务世界：把"所处世界已知、且明显不属于新世界"的旧势力移出当前世界
    //   （兜底上面按名冻不到的：worldName 与新世界对不上但也不等于上个世界名的历史遗留）
    try {
      const F = useFaction.getState();
      const norm = (s: string) => s.replace(/[\s·•・\-—_,，。、|｜（）()【】]/g, '').toLowerCase();
  ...
  return (
    <div className="h-[100dvh] flex flex-col bg-void text-slate-300 overflow-hidden" style={{ fontFamily: 'var(--app-font)' }}>
      ...
      {/* 🛤 原著路线·结算盖章横幅（你 vs 白夜，可关） */}
      {canonSettleNotice && (
        <div className="shrink-0 flex items-center gap-2 px-4 py-2 bg-god/15 border-b border-god/40 text-god text-[13px] font-mono">
          <span className="shrink-0">🏁</span>
          <span className="flex-1 leading-snug">{canonSettleNotice}</span>
          <button onClick={() => { setCanonSettleNotice(''); setCanonRouteOpen(true); }} className="shrink-0 px-2 py-0.5 border border-god/40 rounded hover:bg-god/10">🗺 看路线图</button>
          <button onClick={() => setCanonSettleNotice('')} className="shrink-0 px-2 py-0.5 border border-god/40 rounded hover:bg-god/10">知道了</button>
        </div>
      )}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── 左侧角色面板（桌面常驻列 / 手机左侧抽屉）── */}
        <aside
          className={`shrink-0 w-72 border-r border-edge bg-panel flex flex-col overflow-y-auto overflow-x-hidden lg:overflow-hidden
        ...
        <main className="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          <div ref={chatBubbleHostRef} className="flex-1 overflow-hidden relative">
            ...
            {worlds.length > 0 ? (
              ...
              <div ref={chatScrollRef} onScroll={onChatScroll} id="chat" className="h-full overflow-y-auto px-6 max-lg:px-3 py-4 space-y-4 max-w-4xl mx-auto w-full border-x border-edge">
                ...
                {/* 楼层信息条：贴在最新正文末尾（时间·天气 / 任务 / 伏笔）。零 props ⇒ memo 恒跳过，
                    App 打字/流式重渲不会波及它；只跟 misc/tables/settings 自己的订阅走。⚠勿传 props。 */}
                {messages.length > 0 && <StoryStrip />}
                {generating && (
          ...
          {/* 选择世界 / 🛤 原著路线栏 */}
          {canonEnabled ? (worldBarOpen && (
            <div className="shrink-0 border-t border-edge bg-panel px-4 py-2 flex items-center gap-2 flex-wrap text-[12px] font-mono">
              <span className="text-god/85 font-bold shrink-0">🛤 原著路线</span>
              <span className="text-dim truncate">第 {canonIdx + 1} 站 · {getCanonStation(canonIdx)?.name ?? '已到当前收录终点'}</span>
              <button onClick={() => setCanonRouteOpen(true)}
                className="px-2.5 py-1 rounded border border-god/40 text-god hover:bg-god/10 transition-colors">🗺 路线图 / 进入世界</button>
              <button onClick={openSettleFlow}
                className="px-2.5 py-1 rounded border border-edge text-dim hover:border-god/40 hover:text-god transition-colors">⚖ 结算任务</button>
            </div>
          )) : (
          <WorldSelector
          ...
          {canonRouteOpen && (
            <Suspense fallback={<PanelLoading />}>
              <CanonRoutePanel
                ...
                onDmSuxiao={() => {
                  // 💬 一键私信白夜：无档则按当前站建（数值锁死），随后开 DM 线程
                  const id = upsertSuxiaoNpc(useCanonRoute.getState().stationIndex);
                  const rec = id ? useNpc.getState().npcs[id] : undefined;
                  openDmFor({ targetId: id, targetName: rec?.name || '白夜', targetTier: rec?.realm, targetJob: rec?.profession, targetPersona: rec?.personality, targetTag: '契约者' });
                  setCanonRouteOpen(false);
                }}
              />
            </Suspense>
          )}
                ...
                {!collapsed && sec.items.map((item) => (
                  ...
                  >
                    ...
                    {(navBadges[item.label] ?? 0) > 0 && (
                      <span className="ml-auto min-w-[18px] h-[18px] px-1 rounded-full bg-blood text-white text-[10px] font-bold flex items-center justify-center leading-none animate-pulse">{(navBadges[item.label] ?? 0) > 99 ? '99+' : navBadges[item.label]}</span>
                    )}
                  </button>
                ))}
              </div>
      ...
      {cleanupNpcs.length > 0 && (
        <div className="fixed inset-0 z-[80] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-md bg-void border border-edge rounded-2xl overflow-hidden flex flex-col max-h-[80dvh] shadow-[0_0_60px_rgba(0,0,0,0.8)]">
      ...
      {/* ── 杂项（任务/世界大事）面板 ── */}
      {channelPanelOpen && (
        <ChannelPanel onClose={() => setChannelPanelOpen(false)} onRefresh={refreshChannel} onNewsRefresh={refreshWorldNews} onPaper={genWorldPaper} onSearch={paradiseSearch} onSolicit={solicitQuotes} onPost={replyToChannelPost} onMoreReplies={moreChannelReplies} onOpenShop={() => setShopOpen(true)} onJoin={joinPartyFromPost} onInvite={inviteToParty}
      ...
      {outfitWorkshopOpen && <OutfitWorkshop onClose={() => setOutfitWorkshopOpen(false)} />}
      ...
      <ImageBusyToast />
      <GlobalToasts />
      {showVer && <VersionToast version={APP_VERSION} notes={VERSION_NOTES} onNav={runNavAction} onClose={() => setShowVer(false)} />}
        ...
        <div className="group relative mes_block">
          ...
          <div className="absolute top-0 right-0 z-10 flex items-center gap-1">
            ...
            <button
              onClick={() => onStartEdit(msg.id)}
              title="编辑这段正文"
              className="opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity w-7 h-7 flex items-center justify-center rounded-md border border-edge bg-void/85 text-dim/60 hover:text-god hover:border-god/40"
            >
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
            </button>
          </div>
          ...
          <div
            ...
            dangerouslySetInnerHTML={{ __html: toHtmlWithImagesCached(msg.id, renderPhaseExpand(msg.id, fenceSplit && fenceSplit.fences.length ? fenceSplit.text : msg.content), msg.images, ttsDlgOpts) }}
          />
