...
function copyBuiltinPresets(): Plugin {
  const map: [string, string][] = [
    ...
    ['../../正文预设/轮回乐园 Alu v4.1 破晓.json', 'zhushen-alu.json'],   // 内嵌「轮回乐园 Alu v4.1 破晓」为内置正文预设（默认不激活，玩家自选）；2026-08-09 从 v2.0 升级——同一 zhushen-alu.json 槽位，旧版内置不再补种即自然消失（builtin 不入库）
