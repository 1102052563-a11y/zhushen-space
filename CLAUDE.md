...
> **综合对账纠错（`runMergedAuditPhase`，物品+主角两阶段【合并成一次】调用）**：旧的「物品对账」与「主角面板对账」两次独立调用**已合并**。在 `runPostNarrativePhases` 里 `Promise.allSettled([itemP, playerP]).then(...)`——**等物品演化 + 主角演化都跑完后只调一次** AI，看「应用后真实数据(主角面板 + 物品清单,真实ID) + **最近两回合正文**(messagesRef 取上一条 assistant)」，逐项纠正**遗漏更新/错误更新**：① 主角全部面板(六维/HP·EP·SAN/状态/等级阶位/进阶点数/位置/外观/技能天赋)；② 物品 删/扣/穿脱 + 合并重复条目。`MERGED_AUDIT_SYSTEM`/`MERGED_AUDIT_PROMPT`。**两个 auditEnabled 开关**(`itemStore`/`playerStore`，各自 UI 有勾选)现**共用这一次调用**——`checkPlayer`=主角段是否纳入、`checkItems`=物品段是否纳入，都关则不调。**安全网**：物品段硬过滤掉 createItem/货币指令(applyItemCommands 仅删/扣/穿脱)；主角段 applyStateUpdates+applyPlayerProfileCommands+applyCharacterCommands(`charId==='B1'` 过滤)；**NPC 物品只纠正 `npcTag∈{随从,宠物}` 的，其它 NPC 一概不碰**。
...
> **宽松 JSON 解析 `lenientJsonParse`（重要·防"指令解析失败"）**：AI 常把指令对象写成 **JS 字面量**而非合法 JSON——**裸键**（`{name:"…", tier:"…"}` 而非 `{"name":…}`）、单引号、尾随逗号。旧 `JSON.parse` + 仅"单引号转双引号"的兜底**无法处理裸键**，导致 `addTalent/addSkill/createItem/add(...)` 等被整条丢弃（控制台 `[Char]/[Upstore]/[NPC] 解析失败`）。`lenientJsonParse(s)` 逐级放宽：标准 JSON → 给 `{`/`,` 后的 ASCII 裸键补引号(正则只碰 ASCII 键，不误伤中文值里的全角「：，」) → 单引号转双引号 → 去尾逗号。**所有命令解析器**(char/item/npc/faction/territory/team 的 parseUpstoreBlock/parseCharBlock/parseNpcBlock/FAC_ADD/parseJsonArg/pj)统一改用它。

> **技能/天赋"按正文更新 + 不拥挤"三件套（重要）**：① **字段别名**——AI 常用 `description`(而非 `desc`)、`tier`/`grade`(而非 `rarity`) 写天赋/技能；apply 侧 `addTrait`/`addSkill` 的 `desc` 接受 `description` 别名、`rarity` 接受 `tier`/`grade`，否则正文里的详细效果会被丢弃、天赋只剩个名字。② **同名 upsert 合并保旧**(`characterStore.mergeKeepRich`)——`addTrait`/`addSkill` 同名更新时，新条目里**空/缺失的字段保留旧值**，防"对账纠错/演化只给了名字的极简重复 add"把已有的详细 desc/effect 冲掉。③ **卡片断行**(`CharacterPanel.breakSentences`)——技能/天赋卡的 desc/effect 按句末标点 `。；！？` 断成多行(`whitespace-pre-wrap`)，长效果不再挤成一坨。
>
> **物品近似重名去重**：`劣质餐刀` vs `劣质的餐刀` 只差一个「的」——`itemStore`/`npcStore` 的去重归一化(stackNorm/dedupeByName/npcStackNorm)现在把 `的/之` 也并入要剥除的标点集，让这类"差个虚词"的重复物品被 `dedupeByName`/`dedupeNpcItems` 确定性合并(不再只靠 AI 对账)。
...
> **已装备物品·删除策略（`stateParser`）**：
> - **`consumeItem`** 对 `equipped===true` 的物品（主角 + NPC 两路）**仍一律拒绝**并打 warn——消耗品不会处于穿戴态，多为 AI 幻觉（防"穿戴装备被无故消耗"）。
> - **`destroyItem`** 对 `equipped===true` 的物品**改为自动先卸下再移除**（销毁=物品从世界消失：丢弃/卖掉/损毁/被夺走）——兑现 prompt 里"引擎会自动先卸下再销毁"的承诺。**修复"正文丢弃/损毁了穿戴中的装备、但对账纠错的 destroyItem 被旧守卫拒绝、装备删不掉"的缺口**。主角走 `unequipItem`+`removeItem`，NPC 直接 `removeNpcItem`（移除即清空装备槽）。
> - AI 侧约束未变：物品阶段/对账纠错 prompt 仍要求「只有正文明确丢弃/损毁/卖掉/被夺走穿戴装备时才 destroyItem」，避免无理由销毁。
> - **换装≠销毁（prompt 铁则，物品阶段 + 综合对账都注入）**：替换装备时只对**新装备** equipItem，引擎自动把同槽旧装备卸回储存空间（`itemStore.equipItem`/`npcStore.equipNpcItem` 第二分支已实现）；**被换下来的旧装备只是放回储存空间、没消失，绝不要对它 destroyItem/consumeItem**（这是 destroyItem 放宽后最易出现的误删）。对账侧也明确"换下的旧装备清单里还在是正确的、别删"。

> **同名物品堆叠 + 防重复生成（三层）**：① **数据层堆叠**——`itemStore.addItem` / `npcStore.addNpcItem` 对**可堆叠类**（非 武器/防具/饰品/特殊物品/法宝，即 `isStackableCat`）未装备的同名同品质物品**累加数量**到已有条目，不新建行；装备类不堆叠（保各自杀敌数/耐久/词缀）。② **每回合合并**——物品阶段 `applyAllUpdates` 后调 `useItems.dedupeByName()`(主角，可堆叠累加/装备取大值) + `useNpc.dedupeNpcItems()`(全 NPC) 合并残留同名重复。③ **提示词读已有清单**——主物品阶段 system prompt 注入 `${player_items}` + **`${npc_items}`(在场NPC持有物，新增)**，铁则要求"清单里已存在/名称相近的同一件物品别再 createItem，重复消耗品用 updateItemQuantity 加数量"。④ **对账纠错合并**——`runItemAuditPhase` 现注入**最近两回合正文**(messagesRef 取上一条 assistant)，`ITEM_AUDIT_SYSTEM` 加"重复条目合并职责"：两条相似名其实是同一件被重复生成→destroy 多余那条（保守判断、装备/完整那条优先保留）。技能侧 `characterStore.addSkill` 本就按 `nameEq` 同名原地更新、不重复。
...
- **技能** `Skill`（固定格式 名称|等级|类型|品级|消耗|目标|效果|伤害|层级|属性加成|描述|标签）：除原有 id/name/level/cooldown/desc/effect/layers/layerProgress/cost/layerEffects 外，加 `skillType/rarity/target/damage/attrBonus/tags`。`addSkill("B1",{命名键…})`，CharacterPanel/NpcDetail 技能卡展开显示全部字段。
  - **技能品级·7 档系统（轮回乐园）**：`skill.rarity` 取 **普通→精良→稀有→史诗→传说→奥义→极境**（`characterStore.SKILL_TIER_CLS/SKILL_TIER_DOT/normSkillTier`，含旧值/D-SSS/颜色别名归一化；**极境**给渐变+辉光特效）。技能卡顶部品级徽章 + 边框按此着色（主角 `CharacterPanel.SkillCard`、NPC `NpcDetail.NpcSkillCard`）。**奥义**=常带「唯一被动」(同源对立两被动不可同触发)；**极境**=最高层次，靠实战体悟+技能融合练成、非堆资源可换。
  - **技能等级**：Lv.1→Lv.10→**Lv.EX**(满级)；满级后升一档品级、等级重置 Lv.1。
  - 规则注入：`SKILL_TIER_RULE`(App 代码注入主角+NPC演化，**改即生效**) + 预设条目「技能品级与等级系统」(已加入 `演化预设/主角演化.json` entrySharedRules、`NPC演化.json` prompts.npc.rules，及 PLAYER_KEEP_NAMES/NPC_KEEP_NAMES)。技能/天赋 `addSkill/addTrait` 同名 upsert 走 `mergeKeepRich`(空字段保留旧值)，卡片 desc/effect 经 `breakSentences` 按句断行。
...
- **发帖人信息**：每条帖/回复带 `authorPersona/authorJob/authorStrength`（性格/职业/生物强度，App 常量 `CHANNEL_AUTHOR_INFO_RULE` 注入要求职业多样化·隐藏职业，非法师牧师），供建临时队友档案；MessageCard 头部展示。
- **临时队伍组队入队**（与永久冒险团分离的"本世界临时队"）：复用 NPC 系统——`npcStore.NpcRecord` 加 `partyMember/partyWorld/partyRole` + 动作 `createPartyMember(info)→C-id`/`leaveParty(id)`/`disbandPartyForWorld(world)→ids`。① **加入组队帖**(`kind='recruit'` 显「🤝 加入队伍」)=`App.joinPartyFromPost` 确定性建队友+markTraded；② **邀请入队**(任意非己帖显「➕ 邀请入队」)=`InviteDialog` 输邀请词→`App.inviteToParty(m,text)` **AI 判定**(注入主角面板+该契约者性格/目的，走频道 API)→`{accept,reason}`，答应自动建队友、拒绝可再邀；③ **世界结束自动解散**=`App.reconcilePartyLifecycle()`(每回合 callApi 开头)调 `disbandPartyForWorld(当前世界)`，`partyWorld≠当前世界`的队友离队+离场归档；④ **转正**(仅有冒险团时)=解散有人则弹 `PartyPromoteDialog` 勾选 `upsertMember`(**注意 adventureTeamStore 是 `upsertMember` 不是 `addTeamMember`**) 转正进 `adventureTeamStore`；⑤ **中途离队** `partyLeave("C1")`/`leaveParty("C1")` 短指令(`applyNpcShortCommands`)。`partyMember` NPC 在 OnScenePanel/NpcPanel 打**「队」蓝徽章**，`buildStructuredRecall` 注入「【主角的临时队伍】」段。

## 私信系统（一对一私聊 + 私下交易，`dmStore`/`dmTrade`/`DmPanel`）

对单个契约者的点对点私聊+交易，**独立界面**（右侧导航 **✉ 私信** → `DmPanel`，不附在公共频道）。也可从公共频道每条帖「✉ 私信」、「📇 NPC」列表每卡「✉ 私信」进入（开 DM 关来源面板、聚焦该会话）。**详细沿革见记忆 `private-message-feature.md`**。要点：
- **可私信对象** `isDmableTag(tag)`（dmStore 导出）：契约者/随从/宠物/无标签可；**土著/召唤物不可**。两类：已建档 NPC(`targetId`=C-id，按 NPC 面板回应)、频道未建档发帖人(按发言+性格/职业/生物强度回应)。
- **数据** `dmStore`(drpg-dm)：`DmThread`(targetId?/targetName/.../sourceContent/archived/messages[]) + `DmMessage`(from player|npc|system, deal?) + `DmDeal`(kind buy|sell|request|barter；giveItem/giveCurrency/getItem/getCurrency；source have|source|free；status)。线程 key=`c:<cid>`或`n:<name>`去重。
- **结算** `systems/dmTrade.ts`：`settleDmDeal(thread,deal)` 校验+确定性转账；**对方收到的物品入其 NPC 储存空间**(`addNpcItem`)；对方收物却未建档→就地 `createArchivedContractor`(离场)建档兜底。
- **App AI 处理器**（走 `resolveApiChain('channel', …)` 复用频道 API）：`dmReply`(聊天) / `dmPropose(kind,payload)`(购买→have/source代购加价/no；给予→收下付钱/白拿道谢/拒绝；索取→give/price/no；换物→accept+extraFromPlayer 补差) / `dmHaggle`(讨价还价重报价，旧卡作废生成新卡) / `dmAccept`(成交=settleDmDeal) / `dmGenArchive`(未建档→AI 生成完整离场档案)。`dmPersonaPrompt` 建人设、`dmPlayerCard` 注入主角面板供报价判断。
- **UI** `DmPanel`：左会话列表+右对话(气泡+DealCard 交易卡：你交出/你获得+话术+成交/讨价还价/取消)+动作按钮(💰购买/🤲给予出售/🙏索取/🔄换物)内联表单+聊天输入。
- **存档**：`drpg-dm` 纳入 saveManager + `clearProgress`(`useDm.clearAll`)。
- **物品流向**：买/索取/换→主角背包；给予/出售/换出→对方 NPC 储存空间。

## 好友系统（好友栏 + 每回合参与 NPC 演化）

把契约者/随从/宠物收藏进好友栏，好友每回合参与 NPC 演化（数量可设、轮换）。右侧导航 **👥 好友** → `FriendsPanel`。
- **数据**：`NpcRecord.isFriend`/`friendedAt` + `npcStore.setFriend(id,on)`。可加好友判定复用 `isDmableTag`（土著/召唤物不可）。
- **演化整合**（仅策略 B）：`npcEvoStore.scheduling.friendsPerTurn`（默认 3）。`App.computeFocusList` 末尾把 `isFriend && alive` 的 NPC 按 **`lastEvolvedTurn` 升序（最久未演化优先）轮换** 取 `friendsPerTurn` 个塞入焦点（与在场/离场配额独立，最终 `new Set` 去重）。策略 A 下好友不单独演化（FriendsPanel 有提示）。
- **加好友入口**：① 「📇 NPC」列表每卡 `☆好友/⭐已好友` 切换(`setFriend`)；② 公共频道每帖「⭐ 加好友」=`App.addFriendFromChannel`(走 `addFriendByInfo`)；③ 私信会话头「⭐ 加好友」=`dmHandlers.onAddFriend`。
- **加好友落地** `App.addFriendByInfo(info)`：已建档/重名→直接 `setFriend(true)`；**频道未建档→`createArchivedContractor`(离场) + setFriend + 异步 `fleshOutContractor` AI 补全完整档案**（`fleshOutContractor` 由 dmGenArchive 与加好友共用，AI 填 阶位/性格/背景/外观/动机/关系/六维/好感 **+ 3~6 件随身物品(1~3 件 equipped)写入该 NPC 储存空间/装备面板**）。
  - **离场≠死亡**：`fleshOutContractor` 提示词明确"该角色活着、只是离场"，并用 `stripDeadWords()` 剥掉 AI 误写进 realm 的"已阵亡/死亡/已故"等字样、强制 `isDead:false`（修"频道加好友生成的 NPC 显示已阵亡"）。
  - **储存空间/装备面板**：物品管理阶段**只演化正文里的物品**，不碰离场角色——故友档案的储物/装备靠建档时一次性生成的初始物品（equipped 项按 category 分配 `weapon:main`/`armor:upper`/`accessory:#1`… 槽位，NpcEquip 直接显示）。
- **FriendsPanel 私信**：好友卡有「✉ 私信」按钮 → `onDm`→关好友面板+`openDmFor`。
- **FriendsPanel**：列好友卡(头像/姓名/阶位/在场离场/好感/上次演化回合) + 点击跳 NpcDetail(`onOpenNpc`→`setOnSceneDetailId`) + 移出好友 + 顶部「每回合演化好友数」输入(`setScheduling({friendsPerTurn})`)。
- **存档**：好友标记在 `drpg-npc`（NpcRecord）已随存档；演化人数在 `drpg-npc-evo`。
