...
const dir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '产出', '批次488');
...
// also run compile-worldbook --check
const compile = path.resolve(path.dirname(fileURLToPath(import.meta.url)), 'compile-worldbook.mjs');
for (const f of files) {
  const r = spawnSync(process.execPath, [compile, '--check', path.join(dir, f)], { encoding: 'utf8' });
  process.stdout.write(r.stdout || '');
  if (r.stderr) process.stderr.write(r.stderr);
  if (r.status !== 0) allOk = false;
}
console.log(allOk ? 'COMPILE CHECKS OK' : 'COMPILE CHECKS FAIL');
