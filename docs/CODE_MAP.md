...
| NPC 演化（策略A/B、登场判断、调度） | `App.tsx` → `runNpcPipelineB` / `runEntryJudgment` / `computeFocusList` / `gateFocusIds`(演化预筛 Gate·默认关)；`store/npcStore.ts` / `npcEvoStore.ts`；`systems/rosterGate.ts` |
...
| 环推进闸门（反乱推进度：evidence 正文锚定/每轮每任务一种环操作/跨环限幅/环状态单向/成功结算闸） | `systems/questGuard.ts` → `evidenceAnchored`/`gateRingAdvance`/`gateRingsPatch`/`gateTaskSettle`；接线 `systems/miscParser.ts` `applyMiscCommands`（`opts.narrative` 供证据核验·被驳回记仲裁日志+转 progress）；设置 `miscStore.settings.questAdvanceGate`/`questRingJumpMax`（默认开/1）；UI QuestManager「环推进闸门」卡 |
| 推进复核裁判（推进/跳环/整条结算主张逐要件核验正文证据·FAIL 剔除·fail-open） | `systems/questAdvanceReview.ts` → `reviewQuestAdvancement`/`extractAdvanceClaims`（含"见好就收/继续赌"选择点只核表态、progress 补跨回合早先要件）；接线 `App.tsx` `runQuestEvolutionPhase`（applyMiscCommands 之前）；开关 `miscStore.settings.questAdvanceReview`（默认开·仅 AI 试图推进的回合多一次调用） |
...
| 世界百科（📖同人世界原著情报·联网考据·🗂已存回看） | `components/WorldCodexPanel.tsx`（标签分页面板；**已存资料页回看**：byWorld 全量列表（archives）→点开只读浏览 `viewKey/readOnly`·↩返回当前·🗑confirm 删除传精确存储键走 `clearWorld`；乐园/主神空间空态也列已存可回看）；模块+提示词 `worldCodexModules.ts`（`CODEX_GLOBAL_RULE`+五模块·改即生效）；生成 `systems/worldCodex.ts`→`genCodexSection`（`resolveApiChain('codex')`）；存储 `store/worldCodexStore.ts`（`drpg-world-codex`·byWorld 按世界名·`resolveKey` 归一/作品名模糊找回·`setSection` 覆盖式·纳入 saveManager）；设置卡 `components/WorldCodexManager.tsx`（开关+ApiRoutePicker `codex` 路由） |
...
| 👗 衣柜（穿搭预设·服装单一权威源） | `store/outfitStore.ts`（`drpg-outfit`·byChar[charId]={outfits,activeId}·进度类进 STORES 带 clear）；`systems/outfit.ts`（`activeOutfit`/`outfitRosterLine`/`applyOutfitCommand` AI换装/`outfitImageKey`）；接线：生图三线（`imageGen.buildPortraitPrompt` charId 字段·钦定穿搭>装备栏>外观/`App.genStoryImagesFor` charLine/`comic.buildRoster`+`collectRefs` 穿搭参考图）+ **正文**（`promptInjections.buildOutfitInjection` `<钦定穿搭>` 块·两注入位）+ **<state> 指令** `outfit.<角色ID>=穿搭名|场景标签|无`（`stateApply.applyOneUpdate`·名/标签模糊·NPC名唯一自纠）；参考图存 imageDb `outfit:<charId>:<id>`（随存档快照·chatimg 线随参考图发送·`outfitRefImages`+`OUTFIT_REF_HINT` 接立绘5生成点）；**跨存档模板库** `store/outfitTemplateStore.ts`（drpg-outfit-tpl·⚠不进 STORES·同名覆盖）+模板图 `systems/outfitTemplateDb.ts`（独立 IDB `drpg-outfit-templates`）；UI `components/OutfitPanel.tsx`（弹层壳+可嵌入 `OutfitPanelBody`：增删改+激活单选+从当前穿着导入+🎨试衣+🎲随机搭配(`outfitStore.rollRandom`·候选池优先/池空全衣柜/>1套必换不同·≥2套才显)+🔎搜索(≥8套)+📷参考图+⭐存模板+📚模板库导入），入口 PlayerSidebar 👗 + NpcDetail 肖像绘卷 + 形象工坊；单测 `systems/outfit.test.ts`（12例） |
| 👗 形象工坊（右栏入口·读形象·试衣生图·生图提示词·成衣库） | `components/OutfitWorkshop.tsx`（角色列=B1+NPC(排archived/frozen·👗数角标)·「当前形象」读取区·生图提示词区(avatarPrompt ✏️查看编辑/💾仅保存/🧹恢复自动/🔄按新词重生成＋imageTags 内联编辑)·嵌 `OutfitPanelBody`·📦成衣库Tab=📥导入(进度/可停/按源id去重/单图坏不拖垮)+来源与标签chips+🔎搜索+⤵入柜+🗑单条/整包删）；`systems/outfitTryOn.ts`（`charLook` 读当前人物形象/`listWardrobeChars`/`buildTryOnPrompt`/`generateTryOnImage`(锁脸 refImages+`TRYON_FACE_HINT`)/`currentPortraitPromptFor`/`saveAvatarPrompt`/`regenAvatarWithPrompt`）；试衣核心=`imageGen.buildPortraitPrompt` 新参 **`outfitOverride`**（优先于激活穿搭·不必激活即可预览）；🎨试衣弹层 `OutfitPanel.tsx`→`TryOnModal`（生成→💾存该套参考图/➕另存新穿搭/🔁重roll）；成衣包=`systems/outfitPack.ts`（`parseOutfitPack`/`mergePackTags`·纯函数·源=Outfit-Manager 2.0 导出 outfit-mgr-char-*.json）+`systems/outfitPackDb.ts`（独立 IDB `drpg-outfit-packs`·⚠跨存档**不进 STORES/存档快照**）；App.tsx 接线=rightMenuItems 👗形象工坊+NAV_GROUPS 角色组+NAV_FX+runNavAction+lazy+PREFETCH_PANELS+i18n en；`ImagePromptEditModal` 新可选参 `onSaveOnly`；单测 `systems/outfitPack.test.ts`（4例）；**🎁内置成衣包**：`scripts/build-outfit-pack.mjs`（扫仓库根 outfit-mgr-char-*.json → 按 ≤20MB 切片到 `public/outfit-packs/`packN-sM.json+manifest.json，⚠绕 Pages 单文件 25MiB 硬限·分片名刻意不匹配 gitignore 规则须提交）→ 成衣库 Tab 顶部「🎁内置成衣包」卡片=读 manifest(404 静默隐藏)→顺序下载分片→`runImport` 共用管线（与手动导整包同 dedupe 键=包名#源id，互不重复·可校对补缺）；⚠47.6MB 源文件已 .gitignore（分片是构建产物要入库）；⚠**vite preview 重 build 后必须重启**（cleanDist 全删重建→旧 preview 进程文件表失效连 / 都 404） |
...
| 公共频道 / 系统商店 / 临时队伍 | `App.tsx` → `refreshChannel`/`replyToChannelPost`/`joinPartyFromPost`/`inviteToParty`；`store/channelStore.ts`；`systems/channelTrade.ts`；副本取材 `systems/worldLib.ts` |
| 私信（聊天/交易/讨价还价） | `App.tsx` → `dmReply`/`dmPropose`/`dmHaggle`/`dmAccept`；`store/dmStore.ts`；`systems/dmTrade.ts` |
| 私信·行协议（状态头/心声/撤回/引用/戳·借鉴Abstract外置手机） | `systems/dmProtocol.ts`（`parseDmReply` 行协议解析·失败整段兜底纯文本 / `dmMsgToHistoryText` 历史压缩）+ `promptRules.DM_REPLY_PROTOCOL_RULE`（进 promptRegistry 可编辑）+ `dmStore`（`DmMsgKind`/`meta`/`unread`+`bumpUnread`/`clearUnread`）+ `DmPanel.MsgBubble`（状态条/💭心声/撤回点开偷看/戳/引用块） |
| 📨 NPC主动来讯（意图两阶段·借鉴Abstract外置手机） | `systems/inlineComm.ts`（`parseCommIntents` `<通讯>`意图块·管道数防御 / `eligibleCommNpcs` 白名单=isDmableTag+离场 / `buildInlineCommInjection` 注入门=开关+冷却+名单空零块·⚠中文标签正则勿用`\b`）+ `promptRules.INLINE_COMM_RULE` + `App.runInlineCommPhase`（phases 表 `'inlineComm'`·读 `lastRawNarrativeRef`·二阶段走私信链路→`bumpUnread`红点）+ `settingsStore.inlineComm{on,everyN}`（DmPanel 头部📨开关）；`<通讯>`剥离挂 `stateParser.stripStateBlocks` |
| ⟳ 频道追评（增量评论·借鉴外置手机 forum_comment） | `App.moreChannelReplies`（四段式增量约束+内容判重双保险·不发场外通报）+ `ChannelPanel.MessageCard` `onMoreReplies`（仅主角 speak 帖显示） |
| 👥 群聊（团队频道·借鉴外置手机 group_message） | `systems/groupChat.ts`（`parseGroupReply`「发言人\|内容」行协议·禁代主角+成员白名单两道硬过滤·无竖线行并上一条）+ `promptRules.GROUP_CHAT_RULE`（registry 可编辑）+ `App.groupChatReply`（featureKey=`'groupChat'` 回退频道接口·成员紧凑卡非全量人设·发言/👂潜水两模式·错峰上屏）+ `dmStore`（`kind:'group'`/`members`/`senderName`/`createGroupThread` 同名群合并）+ `DmPanel`（➕建群 `GroupCreateModal` 白名单勾选≥2 / 群头显成员 / 群聊无交易动作行 / `dmHistory` 群感知冠名） |
| 💸 私信转账（纯币赠予·借鉴外置手机 transfer） | `dmStore.DmDealKind` 加 `'transfer'` + `App.dmPropose` transfer 分支（**金额/币种玩家定死，AI 只答收/不收**——可拒收·拒收只回话不出卡）+ `DmPanel` 💸按钮+表单（金额/币种/附言）；成交走 `settleDmDeal` 既有纯币路径（校验余额→扣款→facilityBridge 通报），仅玩家→NPC 方向（反向=经济口子不做） |
| 😊 表情包库（按名点播·借鉴外置手机 sticker） | `store/stickerStore.ts`（`drpg-stickers`·**配置类无 clear**·name→URL·`buildStickerInjection` 库空零块 / `stickerNameSet` 消费侧硬过滤）+ 协议：`dmProtocol` `贴:` 字段 / `groupChat` `发言人\|贴:名` / `DmMsgKind` `'sticker'`；消费点=dmReply/groupChatReply/runInlineCommPhase 三处**注入+无效名整条丢弃双保险**；UI=`DmPanel` 😊管理弹窗+发送picker（群发表情不自动引回复）；已进 saveManager（无 clear）+configExport |
| 🔎 乐园检索（拉取式情报·借鉴外置手机 browser） | `systems/paradiseSearch.ts`（`parseSearchReply` 宽容JSON·claim归一·≤5条 / `searchCandidateBlock` 事实池=复用 `worldNews.buildNewsCandidates` **同一道认知门**：hidden 搜不出·trace 只给表象）+ `promptRules.PARADISE_SEARCH_RULE`（载体随世界/80-20/claim三档）+ `App.paradiseSearch`（featureKey=`'paradiseSearch'` 回退正文API·候选空仍可搜生活信息）+ `ChannelPanel` 特殊页签 `'search'`→`SearchView`（模块级·只读观察层不发言） |
| 🕳 窃听（两离场NPC私谈·借鉴外置手机 peek） | `systems/eavesdrop.ts`（`buildEavesdropSys` 双认知边界各给 / `rollDiscovered` 可注入rng / `eavesdropNotice`·`EAVESDROP_COST=500`乐园币）+ `App.eavesdropNpcs`（**前端确定性扣款·失败/污染退款均 silent**·转写=`parseGroupReply` 白名单这两人 cap14·被察觉→双方 favor-8·所得走 `pushSceneNotice` 进正文须知）+ `DmPanel` 🕳按钮+`EavesdropModal`（仅离场在世可选） |
| 📔 NPC日记（偷看·借鉴外置手机 diary） | `systems/npcDiary.ts`（`parseDiary` 头字段+正文 / `tokenizeDiary` `~~划掉~~`/`██涂黑██`/`【【着重】】`三标记 / `generateNpcDiary` **内聚生成零props**——所有 NpcDetail 渲染点自动获得·observe 路由·污染拒收·`pushDiary` 存 `npc.extra['日记']` **单一真相源** cap3）+ `promptRules.NPC_DIARY_RULE`（**涂改稀缺配额：合计≤3处每种≤1次**·registry 可编辑）+ `NpcDetail` 📔按钮（favor≥50 解锁·`DIARY_FAVOR_GATE`）+日记浮层+`DiaryBody`（涂黑点击显形·模块级组件） |
| 📺 乐园直播间（礼物分档演出·借鉴外置手机 live） | `systems/liveRoom.ts`（`LIVE_GIFTS` 12礼三档 / `giftGuideBlock` **价值→反应分档注入** / `parseLiveReply` 宽容JSON夹取 / `generateLiveShow` 自包含·featureKey=`'liveRoom'` 回退频道接口）+ `promptRules.LIVE_ROOM_RULE`（弹幕千人千面/禁对小礼过度感激/thought幕后·registry 可编辑）+ `components/LivePanel.tsx`（lazy·主播=契约者/随从名册·弹幕/匿名/送礼→**前端扣乐园币 silent+facilityBridge 通报+非匿名小幅涨好感 clamp100**）；导航「📺直播」（NAV_GROUPS 社交联机组） |
...
| 中心 API 库 / 多接口路由 / fallback | `store/settingsStore.ts`（`apiLibrary`/`apiRoutes`/`resolveApiChain`）；`systems/apiChat.ts`（`apiChatFallback`）；`systems/apiUrl.ts`（地址归一化+参数排除）；`components/ApiRoutePicker.tsx` |
...
| ✨ 正文校正（一键去八股·借鉴story-oracle思想） | `systems/polish.ts`（`splitProtectedBlocks` 散文/受保护块拆分=检测收据·`joinProseForModel`/`mergePolished` ⟦P{n}⟧分段标记协议·标记丢失即抛错绝不硬拼·`runPolish` featureKey='polish'·偏好 plain localStorage `drpg-polish-prefs`）；UI `components/PolishModal.tsx`（五目标+轻校/精校·流式预览·结果可手改）；入口+应用 `App.tsx` 操作行「✨ 正文校正」→ `polishTarget` state（应用=`saveBranchPoint({origin:'manual'})` 存原稿🌿支线 + `saveMessageEdit` 替换 content）；提示词 `POLISH_RULE`（promptRegistry 可改）；测试 `systems/polish.test.ts` |
| 🩺 状态诊断·正文对账（借鉴story-oracle思想） | `systems/stateDoctor.ts`（`describeDoctorKey` 白名单预检=applyOneUpdate 真实路由镜像（未知键会被静默跳过、可用性只能预检把关）·`parseDoctorReply`「补丁:/依据:」行协议·`applyDoctorPatches` 包 `<state>` 走 `applyStateUpdates` 正规管道（点数结算门槛等护栏全生效）·`inverseLine` 旧值逆补丁撤销·`runStateDoctor` featureKey='stateDoctor'·最近正文读 chatDb 动态 import）；UI `components/StateDoctorPanel.tsx`（VariableManager 底部条块·范围勾选→逐条勾选应用→撤销）；提示词 `STATE_DOCTOR_RULE`；测试 `systems/stateDoctor.test.ts` |
| 🧭 故事弧线·多拍长线引导（借鉴story-oracle思想） | `store/arcStore.ts`（drpg-arc·进 STORES **且进 ROLLBACK_KEYS**——判定挂杂项阶段会随演化漂移）；`systems/storyArc.ts`（`planArc`「弧名:/拍N:」行协议·`genBeatInstruction` 每拍指令现编一次缓存（`looksLikeNarrativeText` 跑偏拒收=guidanceLooksLikeNarrative 同口径复刻）·`buildArcInjection` 每回合内联注入（App.tsx 历史组装 guidanceBlock 旁·零额外串行调用）·`arcJudgeInjection`+`applyArcJudgment` 完成判定挂杂项阶段（system 尾追加协议+扫 `arcBeat.N = done`/`arcRedline =`·**幂等：拍号必须=当前拍**，仅重算变量重放安全）·featureKey='storyArc'）；UI `components/StoryArcPanel.tsx`（VariableManager 底部·规划可编辑→启动→盲盒遮罩/手改指令/手动过拍/退出）；提示词 `ARC_PLAN_RULE`/`ARC_BEAT_RULE`；测试 `systems/storyArc.test.ts` |
| 🧭 参谋问答增强（P1-2·借鉴story-oracle） | `systems/proposalCard.ts` → `buildAdvisorContext` 尾部加 在场角色/truths/`buildPlotStateBrief` 剧情坐标 + 新 `buildAdvisorNarrativeTail()`（最近正文尾巴·chatDb 动态 import·参谋问答与弧线规划共用）；`components/AdvisorPanel.tsx` → `EXAMPLE_CHIPS` 空态可点胶囊 + send 时附带正文尾巴与问答职责行 |
| 🔮 剧情工具·集中设置页（弧线/诊断/校正/参谋） | `components/OracleToolsPanel.tsx`（overlay 页：内嵌 `StoryArcPanel`/`StateDoctorPanel`（均支持 `defaultOpen` prop·页内默认展开）+ 校正默认偏好（与 PolishModal 共用 drpg-polish-prefs）+ polish/advisor 接口路由 + 预设中心指路）；入口 `components/VariableManager.tsx` GROUPS「剧情工坊」组 🔮卡片（**零 App 布线**：本地 `oracleOpen` state 打开、overlay 渲染在 VariableManager 子树内，与其它经 App onOpenXxx 的卡不同） |
...
| 🗂 导航分组折叠（不用的整组收起来） | `settingsStore.navCollapsed: string[]`（折叠的组名·默认 `[]` 全展开·`toggleNavGroup`/`setNavCollapsed`·**读取处一律 `Array.isArray` 兜底**，老存档无此字段）+ App.tsx 右侧 `<aside>` 渲染（组标题 `<div>` → `<button>`·▶ 箭头 rotate-90·折叠时标题右侧显示「组内红点汇总 + 项数」）+ SettingsPanel「导航布局」块的说明与「全部展开」按钮。⚠ 折叠会让组内红点一起消失 → **必须**把 `navBadges` 汇总到标题上，不然未读会被静默吃掉；⚠ 平铺模式（`navGrouped=false`，`sec.name===''`）恒展开，逐像素不变 |
...
| 📚 纪要压实·大总结（借鉴 ACU 飞行模式） | `systems/acuTableSpec.ts`：`big_summary` 表定义（orderNo 25·不可变阶梯）+ `CHRONICLE_UID`/`BIG_SUMMARY_UID`/`BIG_SUMMARY_THRESHOLD(15)`/`KEEP_RECENT(6)`/`SLICE_MAX(25)` + 纯函数 `visibleChronicleRows`/`planChronicleCompaction`（prompt 与写路径共用同一公式）+ `AcuSheet.compactedThrough`（纪要水位线·row_id≤它=已折叠·物理保留）；`tablePrompt.buildTableFillPrompt` 满阈值注「## ⚠ 大总结·本回合归纳」板块（列最早一段可见纪要+已有大总结尾2条）；`tableEditParser.applyTableEdits` 确定性护栏（没点名拒收/一批一条/updateRow·deleteRow 一律拒=不可变）+系统补记「覆盖纪要」列+`tableStore.advanceCompaction`（只进不退）；正文 L2 注入=App.tsx `addRule('剧情大总结')`（最近2条·~2875）；剧情快照 `buildPlotStateSnapshot` 带【大总结·最近】；TableManager 纪要表已压实行折叠（▸ 已压实 N 行·可展开）；migrate v10→v11=`tableStore.addMissingDefaultSheets`（**只补缺表·勿整库 evolveTables**——那会把 row_id 重排 1..N）；测试 `bigSummary.test.ts` |
| 🧲 纪要向量召回（借鉴 ACU 交火模式） | `tablePrompt.chronicleRecallCorpus(excludeRecent=3)`（全部纪要行**含已压实行**+掉出注入窗的大总结）→ `factVec.buildMemPool` 新增 `chronicleRows` 源（kind `'chron'`·factsOnly 时不进池）→ App.tsx 向量召回分支并池（标签 `[纪要]`·`vm.chronicleRecall !== false` 默认开）；SettingsPanel 向量记忆区「纪要召回」开关+重建索引同口径；配置字段 `VecMemConfig.chronicleRecall` |
| ⏳ 填表积压检测 + 🔒 行锁（借鉴 ACU 仪表盘/patch_sheet_locks） | 积压：`tablePrompt.chronicleFillBacklog()`（纪要末行 rowMeta 回合 vs misc.turnCount·gap≥everyN+3=overdue·老档无索引返回 null 不误报）→ TableManager 琥珀横幅指路 ♻🗂 补填；行锁：`AcuSheet.lockedRowIds` + `tableStore.toggleRowLock` + `tableEditParser.isRowLocked`（AI updateRow/deleteRow 拒收进失败清单·玩家面板编辑不受限）+ TableManager 每行 🔒/🔓（仅 进程/伏笔/约定/纪要/自定义表）+ 填表提示词锁定行打 `[n]🔒` 标 |
| 🩺 运行概览（借鉴 ACU 仪表盘 dashboard-copy） | `systems/healthOverview.ts`：`buildHealthRows()`（正文接口配置/填表[遗留失败>积压>正常]/状态指令/看门狗一致性 四行·绿琥珀红+行动指引）+ `classifyFailures()`（最近失败归因四桶：API 连接/输出格式/指令被拦/其他·特征词正则+各一句处置）+`collectRecentFailures()`（tableJournal.lastErrors+turnReport 末10条）；UI `components/HealthOverview.tsx` 挂 VariableManager 标题下（订阅 turnReport/lastErrors 自动刷新·↻手动重查）；测试 `healthOverview.test.ts` |
| 🧪 模板变量试验台 + 🪶 轻装长跑 + 🤖 AI 建表助手 | 试验台：`components/TemplateTester.tsx`（TableManager 内折叠块·复用正文同款 `resolveTableTemplates`+`needsSqlite→ensureSqliteMirror` 懒加载·对当前表实时解析 `<if>`/`{[db]}`/`<calc>`）；轻装：SettingsPanel 综合设置·聊天历史块（一键 historyLimit=10·依赖 大总结+纪要召回 兜长期记忆·向量记忆未开时给琥珀建议）；建表助手：`systems/tableDesign.ts`（`generateTableDesign` 走 featureKey 'table' 路由·`DESIGN_RULE` 内嵌建表指南契约[逐列说明/枚举全列/排除词/单行表禁增]·`validateTableDesign` 机械校验打回不合规稿·`extractDesignJson` 容围栏）+ CustomTableModal「🤖 帮我设计」回填表单预览（玩家确认才建表·uid 冲突检查走原 create 路径）；测试 `tableDesign.test.ts` |
| 🧩 变量条件注入（`<if var>`·统一提示词展开·借鉴 ST-Prompt-Template 只借思想/AGPL 不抄码） | `systems/promptExpand.ts`（`makePromptExpandCtx`+`expandPromptText`＝resolveTableTemplates→processMacros·runtimeVars 只采集一份、宏层与模板层共享）；`systems/tableTemplate.ts` 新增 `evaluateVarExpr`（`<if var="变量名 op 值">`·`<if cond>` 的 `var:` 原子·`~=`＝包含·**未知/空值仅 `!=` 判真**·**纯数字才数值比**防 parseFloat 截头"3阶"→3）+`hasTableTemplates` 快路径；`runtimeVars.ts` 补衍生 主角.HP/EP/理智百分比；三接入点：App `buildPresetMessages`（seedContent=末条正文）/ `promptOverrideStore.renderPrompt`（keepUnknown=true·**该通道 `<if seed>` 恒否**）/ `apiChat.apiChatFallback` 全局兜底（**升级：已带 runtimeVars**）；测试 `tableTemplate.test.ts` + `promptExpand.test.ts` + `customInject.test.ts`。**P1（08-15 同日）**：①世界书条目「激活条件」`WorldBookEntry.activeWhen`（<if cond> 语法·主正文/欢愉宫/赌场战斗/合成工坊四处激活点全过 `passActiveWhen`(tableTemplate)·vars 惰性采集·seed: 口径=各自 matchCtx；编辑栏在 SettingsPanel `EntryEditor`；`sigOfEntry` **条件展开**追加不动旧签名——⚠该行 join 分隔符是不可见 `\x02`，Edit 打不出须走 PS+[char]2）②🎯 自定义注入 `systems/customInject.ts`（`applyCustomInjects`·条目存 `promptOverrideStore.injects`·App 在 history 组装完、**prefill 之前**最后一步应用·位置=整轮最前/最后/倒数N楼/正则锚点前后·从后往前插·protectTail 保 prefill 末位·细纲 outlineHistory 不吃注入）③🧩 片段库 `store/snippetStore.ts`（`drpg-snippets`·`{{include::名}}` 在 expandPromptText 最先展开·深度3防循环·**不进 saveManager**（与 drpg-prompt-override 同精神）·进 configExport）；UI=预设中心头部「🧩 片段库 / 🎯 自定义注入」两视图（⚠模块级组件防 IME 重挂）。**P2（08-15 同日收官）**：①🧩 渲染期变量 `systems/renderVars.ts`（`renderPhaseExpand(msgId,text)` 包在 App 楼层渲染 `toHtmlWithImagesCached` 的**输入文本**外——展开产物即缓存键，值变 sig 自然变，\x01 sig 行零改动；random 按 msgId 播种 mulberry32 恒定不闪变；开关 `settings.renderVars` 默认关·UI 在设置自定义CSS区上方）②🌱 内容包初始变量 `systems/initialVars.ts`（条目标题 `[初始变量]`/`[InitialVariables]`·识别不看 enabled·宽容 JSON 两形态·**已存在 key 跳过绝不覆盖**·入口=世界书区「🌱 提取初始变量」按钮）③🩺 语法体检 `systems/promptLint.ts`（`lintPromptTemplate`/`lintCondExpr` 纯函数·只提示不阻断·`LintHints` 组件=PromptCenterPanel named export·预设中心编辑页/注入条目/世界书 EntryEditor 三处复用）；测试 `promptLint.test.ts`+`initialVars.test.ts`+`renderVars.test.ts` |
| 🫀 角色心流·逐角色独立推演（借鉴SoulLink·默认关） | `systems/mindflow.ts`（详见 systems 表）；设置 `settingsStore.mindflowEnabled/mindflowMax`(默认 off/2·上限4)；UI=SettingsPanel 正文生成页(简报开关下方·开启才显 `ApiRoutePicker routeKey="npcMindflow"`)；注入位=callApi history 的 castHint 之后/prelude 之前；与简报分工：简报=群像意向书(零新增调用)、心流=逐人深推演(每人一调·能演误读·信息盲区物理隔离) |
| 📊 API 消耗观测 + 思考强度（借鉴SoulLink） | `systems/apiDebugLog.ts`（`ApiCallLog` 补 charsIn/charsOut/model/host/attempt/usage·CAP 30→60·`normApiUsage` 归一各家字段）；`systems/apiChat.ts`（`scrapeUsage` 抠流式末块 usage·`hostOf`·finish 带命中 meta·**超大输入哨兵**：单次输入>30万字记一致性哨兵 `'api-input-bloat'` 防演化超token复发）；UI=`ApiPromptPanel` 「📊 消耗排行」按 label 聚合(输入量降序·tokens 仅上游报了才有)+列表行/详情头带消耗与命中接口；**思考强度**=`ApiConfig.reasoningEffort`(默认不发送·'none'关思考治 content 空回)→body `reasoning_effort`+**400点名即去参对同一接口软重试**(仿 aliasGuard)·UI 在接口库编辑卡 |
| 🔔 预设中心·底稿版本感知（defSig·借鉴SoulLink旧默认迁移·免快照hash方案） | `systems/promptRegistry.ts` → `promptSig`(FNV-1a·**\r\n归一**防CRLF/LF跨构建失配)/`LEGACY_DEF_SIGS`(已知历史默认签名·⚠改「全文初始化的field默认」如记忆提示词时把**改前**文本sig追加进来·scratchpad legacy-sig.mjs 可从git历史算)/`reconcilePromptDefaults`(幂等对账：存文=当前默认→override清冗余副本、field对齐sig；hash命中保存时sig或legacy→**没真改**自动升级吃新默认；真改过+底稿更新→仅徽章绝不动文本)/`promptDefUpdated`；`store/promptOverrideStore.ts` → `sigs`表(setOverride第3参defSig·import一律清sig防误判)；调用点=App 开机 useEffect(**动态import** 防 promptRegistry 拽进主chunk)+`PromptCenterPanel` 开面板(升级toast+列表🔔底稿已更新徽章+编辑页提示)。⚠典型病：memoryStore 把默认记忆提示词**全文初始化**进持久层，内置更新老档全部静默冻旧版 |
...
**NPC 阶段**（策略 B 为主）
- `runNpcPipelineB` (~2508) — 三段管线总入口；`runEntryJudgment` (~2348) 登场判断；`applyEntryResult` (~2278) 建档/归档/去重防撞；`computeFocusList` (~2387) 调度选焦点（含好友轮换）；`gateFocusIds`（演化预筛 Gate·借鉴SoulLink·off/local/ai 默认off·只裁「在场非新建」·失败fail-open·每N回合强制全量·独立槽 `npcGate` 未配置回退 npc 链·0入选/失败记一致性哨兵 'evo-gate'）；`runNpcFocusEvolution` (~2476) + `runNpcEvolutionForTarget` (~2428) 逐NPC并发
...
| `worldLib.ts` | 世界库解析/抽样（纯函数）：`parseWorldLib`(五格式,从 WorldSelector 抽出)/`sampleWorldPoolText`(按「选择N阶世界」+「休闲世界」条目每阶随机抽世界名→频道副本取材注入) |
| `channelFlavor.ts` | 频道发帖动机抽签（借鉴Zsd论坛）：`CHANNEL_MOTIVES` 每频道动机池 + `buildMotiveDraw`(每次刷新随机抽N注入) |
| `channelBuzz.ts` | 📡 <广场舆论> 回注：`buildChannelBuzzInjection`(抽热度top-N的AI帖给正文当舆论氛围·双护栏·`injectBuzzCount`门控·排除system/speak/byPlayer) |
| `arenaReport.ts` | 📣 战报卡（借鉴Zsd）：`buildBattleReport`(从BattleState提取回合/最痛/MVP/险胜)/`formatBattleReportContent`/`reportBaseHeat`；App 侧 `postBattleReportToChannel`+`solicitSpectatorChatter`(围观议论)，钩在竞技场结算+讨伐BOSS胜利 |
| `dmTrade.ts` | 私信结算：`settleDmDeal`(确定性转账，对方收物入其NPC储存)/`dealSummary` |
| `apiChat.ts` | `apiChatFallback(chain,messages,opts)` 多接口轮流+失败切换；多 Key 轮换 `orderedKeys`/`markKeyGood`/`markKeyBad`/`isKeyRotatableStatus`(401/403/429 换 Key 重试同接口·429冷却60s/401冷却5min·仅内存·App正文与 agent 传输共用) |
| `apiUrl.ts` | 地址归一化+参数排除+**三协议层**（纯函数·有测试）：`chatCompletionsUrl`/`modelsListUrl`/`normalizeApiBase`(#字面量·剥误粘后缀·⚠缺/v1 绝不静默补)/`endsWithVersionSegment`/`maskApiKey`/`applyOmitParams`+`OMITTABLE_PARAMS`(⚠stream 永不可排·非破坏式)；P1：`protocolOf`/`keysOf`(多Key拆分·⚠/api/gw/ 整串不拆)/`chatUrlFor`(按协议预览)/`buildProtocolRequest`(anthropic=x-api-key+直连头+system提级+max_tokens必填回填+交替合并；gemini=x-goog-api-key头+contents/parts+generationConfig+data:图片→inlineData·model进URL)/`extractStreamDelta`/`extractStreamReasoning`/`extractOnceContent`(三协议流/一次性解析)/`modelsFetchArgs`(拉模型 url+headers·取首Key) |
| `apiThrottle.ts` | `acquireApiSlot`(并发+最小间隔限流) |
| `rosterGate.ts` | 名单预筛 Gate 通用件(借鉴SoulLink)：`runRosterGate`(max_tokens1024+temp0.1+最快返回·失败即 ok:false 调用方必须fail-open全跑)/`parseRosterGateReply`(名单归一化求交·名单外忽略留rawNames审计)/`localNameScan`(零API名扫描)/`extractFirstJsonBlock`(通用抠JSON·npcRefine 也用)；判定标准由调用方传(如 `NPC_EVO_GATE_RULE`)；NPC演化预筛用之，来讯意图等同构判定可复用 |
| `mindflow.ts` | 🫀 逐角色心流独白(借鉴SoulLink·默认关)：`runMindflowBatch`(每位在场目标一次独立小调用·只见自己瘦档案=信息盲区物理隔离·`pickMindflowTargets` 复用 castBrief 排序只留在场)/`sanitizeMindflowReply`(污染检测·<state>/状态栏即废)/`buildMindflowBlock`(导演注六条执行规则)；App: `startMindflowForTurn`(sendMessage 起跑·与检定/细纲等待窗口并行)+callApi 收割(20s race·fail-open·task/narrateOnly 不吃·消费即清)；槽 `npcMindflow` 回退 npc 链；`NPC_MINDFLOW_RULE` 进预设中心 |
| `npcRefine.ts` | ✨ NPC 档案精编(借鉴SoulLink·预览确认才落库)：`generateNpcRefine`(只整理 deedLog+relations·走 npc 链)/`parseRefineReply`(⚠防清档:原有非空结果为空即抛·防膨胀:条数>2倍+4即抛)/`applyNpcRefine`(全量替换+同步旧 deeds 字符串·relations 空则保留原文)；⚠绝不碰数值/私密extra/三层记忆；UI=NpcDetail 经历栏 `RefineBox`(动态 import) |
...
| `dbAdvanceStore.ts` | `drpg-dbadvance` | 数据库推进(Stitches 规划层)：**一键两域**——预设/`enabled`/`useRecall`/`waitSec`(子调用等待上限秒·默认120·15~600·超时真 abort 底层请求，治「前端判空回、后台还在流式生成白烧 token」2026-08-13)=全局配置(读档不回滚)；`lastTabletop`({{tabletop}} 桌面态)/`lastStage`/`lastScene`/`lastRecall`=本时间线运行态，**随存档快照 + 读档/回退一起回滚**(规则 `systems/dbAdvanceRuntime.ts`·2026-08-07 修「回档后推进还记着之前内容」)。新游戏由 clearProgress 显式 `clearRuntime()`；手动清=设置→数据库推进→🧹清上轮记录 |
...
| `worldNewsStore.ts` | `drpg-worldnews` | 🌍 世界见闻快照（≤8·带 worldName 读时过滤·进度类随存档/新游戏清空）+ 📰 本世界日报 `papers`（≤6·`pushPaper`·头条+栏目+读者来信）|
