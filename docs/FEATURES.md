...
**公共频道**（契约者公共广场,`channelStore`/`channelTrade`/`ChannelPanel`）：七频道,单机=AI 模拟虚拟契约者,懒刷新,走 `resolveApiChain('channel',textApi)`。交易(出售帖一键买/玩家挂单 `solicitQuotes`→`acceptQuote` 确定性成交,成交自动删帖)、系统商店(`SystemShop`,买=`genShopItems`/卖=`genSellQuotes`)、主角发言(`addPlayerSpeak` 立即上墙→AI 逐条错峰回复,注入近20条上下文,「↩回复」定向)、发帖人信息(authorPersona/Job/Strength,`CHANNEL_AUTHOR_INFO_RULE` 职业多样化)、副本取材(`CHANNEL_WORLD_POOL_RULE`+`systems/worldLib.sampleWorldPoolText`:刷新/发言回复时从世界书「选择一~九阶世界」+「休闲世界」条目按阶随机抽世界名注入~700字,帖子谈论的副本覆盖整个世界库随刷新轮换、要求阶位相称+照抄原名;世界库空则不注入)、防劣化(借鉴Zsd网游论坛:`CHANNEL_ANTI_SLOP_RULE`七条=观点碰撞/发言水平分层/禁水评/ID有个性/新帖独立禁续写历史/别围着主角转/长短错落,注入刷新+回复两处;`systems/channelFlavor.buildMotiveDraw` 每频道动机池12~20条每次刷新随机抽5注入)、热度🔥(`ChannelMessage.heat` 0-999 AI按质量分配·`likeMessage`玩家一帖一赞+1·MessageCard徽章·系统频道无)、舆论回注📡(`systems/channelBuzz.buildChannelBuzzInjection`:抽热度最高N条AI帖注入正文<广场舆论>块,仅氛围参考+禁论坛格式双护栏,排除system/speak/byPlayer;`settings.injectBuzzCount`默认4、0=关,ChannelManager可调)、战报卡📣(借鉴Zsd:竞技场结算/讨伐BOSS胜利→`systems/arenaReport.buildBattleReport`确定性对阵卡(回合/最痛/MVP/险胜/晋升note)发战斗频道,`reportBaseHeat`按名次定热度,MessageCard渲染对阵卡代替纯文本;再异步`solicitSpectatorChatter`按`SPECTATOR_CHATTER_RULE`生成2~4条阵营感围观帖,API未配置只发卡不议论)、本世界日报📰(见闻页签「出一期日报」按钮→`App.genWorldPaper`走worldNews路由,`WORLD_PAPER_RULE`+见闻同款可见性门控候选为唯一素材,往期头条注入防重复报道,`parsePaperReply`头条缺失整期作废;`worldNewsStore.papers`≤6期,报头+头条+栏目文章(风格必须区分)+读者来信(强制观点碰撞),头条可📣让主角看到——见闻页三处「让主角看到」均为**勾选式可反悔**(NoticeToggle模块级组件·勾=pushSceneNotice入队/取消勾=removeSceneNotice撤回·通报整句即key·下回合发送才消费·勾选态读peekSceneNotices非React状态))、临时队伍(复用 NPC:`createPartyMember`/`leaveParty`/`disbandPartyForWorld`;加入组队帖 `joinPartyFromPost`/邀请 `inviteToParty`AI判定/世界结束自动解散 `reconcilePartyLifecycle`/转正 `PartyPromoteDialog`→`upsertMember`/中途 `partyLeave`)。`partyMember` 打「队」蓝徽章。详见记忆 `public-channel-feature`。
