...
### 第 4 批 · 长线 — ✅ 已实装 2026-08-04

| 项 | 新增 | 要点 |
|---|---|---|
| ⑫ 精简经济 | `systems/economy.ts` + 17 例 | 只留**相位 / 大宗三类 / 经济事件≤3**，砍掉投机市场·期货·信贷（那套要专门做个"证券所"才撑得起）。**价格公式前端算死**（`新价 = 旧价×(1+趋势±扰动)`，单次封顶 ±30%，指数夹 20~500 防跑飞），AI 只给方向与理由。进世界按世界卡描述**零 API 播种**相位。⚠ `priceFactor` 是**乘**在既有公允价阶梯上的系数（夹 [0.6,1.8]），**绝不替换**那套权威；已接 `genShopItems` |
| §5.6 时代演化 | `systems/eraModel.ts` + 16 例 | **搬到宇宙层**（`cosmosStore.eras`）——任务世界几十回合就离开，进度涨不到临界，这套慢变量在那儿跑不动。净干预（高±2/中±1/低±0.5）→ 定鼎/归墟/派生；进度**单向不可回退**（被压制也只是涨得极慢）；≥80% 自动派生临界事件；逻辑关联合并（保守判据：≥2 个公共词）。全部前端算，AI 只命名与叙述 |
| ⑬ 账本三护栏 | `systems/ledgerCheck.ts` + 19 例 | 不移植整套记账，只取三条通用护栏：**周期门禁**（不到期整块跳过·⚠主角个人钱包变动不算重大事件）、**四点闭环 Pass/Fail**（对不上就标「待补明细」不给净值）、**常备收支对称**（「正文未写成交 ≠ 无收入」，运转中零收入且无合格理由 → 按 baseIncome 给出应补数） |
| ⑭ 社交圈 | `systems/socialCircle.ts` + 17 例 | 按计划**不新建 store**，做成 `relationGraph` 的社区检测派生层（确定性标签传播·零 API·零存档）。只借两个用法：**信息传播判定**（不同圈 ≤2 跳／同圈 ≤3 跳，超出即拒绝＝可判定的防超距）和**注入过滤**（借卡里"关键词命中最近消息才注入"的策略）。⚠ 好感虚拟边不算同圈——那是主角的私人关系 |

三条新提示词（`ECONOMY_RULE`／`ERA_EVOLUTION_RULE`）已进预设中心；`LEDGER_GUARD_HINT` 供产业/领地类阶段按需拼接。

> 门禁：`npm test` **2116 例全绿**（新增 69）· `tsc` **0 错** · `lint` 0 error · `build` ✓ · dist 预览启动正常 ✓

**实装时发现并修掉的一个真 bug**：社交圈的标签传播会把一条长链并成**一个大圈**，若"同圈"就无条件放行，
链两端相距 4~5 跳的人会被判成消息互通 —— 防超距当场失效。改成同圈也受跳数约束（放宽到 3 跳，但有限）。

---

### 收尾 · 接线补完 — ✅ 2026-08-04

四批做完后做了一次**导出消费审计**（grep 每个新导出在非测试文件里的引用），发现前面报「已实装」报早了：
`memoryTiers` 与 `ledgerCheck` **两个模块零调用**，另有若干核心函数只建不接。已全部补上：

| 原本悬空的 | 现在接在哪 |
|---|---|
| `memoryTiers` **整个模块** | `runMemoryCompressionPhase`：`planDecay` 算好待衰退项 → `serializeTiersForEvo` 喂 AI（配 `MEMORY_TIERS_RULE`）→ 结果写回 `core` 层（⚠ AI 没给 core 时**保留既有**，不清空）；`dmPersonaPrompt` 用 `buildMemoryInjection` 注入私聊（只给核心+最近3条） |
| `socialCircle.canKnowAbout` | 新增 `infoReachInjection(charId)` 挂 `buildNpcPhaseSystemPrompt`——按关系图谱算最短路，**点名列出"他不可能知道这些人的事"**。这是防超距的第三道：前两道（ANTI_OMNISCIENCE / NPC_DECENTER）是提示词层的"别全知"，这道是**可判定的** |
| `reputation.reputeSeenBy` | `applyEntryResult` 的建档基线——**土著**按其所属圈子看的那一维定初始 trust/respect（对称于契约者读乐园声望）。⚠ 两条通道互斥，混起来破 `NATIVE_UNAWARE_RULE` |
| `reputation.checkCombos` | 杂项演化后：复合效应（庙堂之高／双面身份／黑白通吃／通缉）达成 → 落成一条**区域世界事件**当剧情钩子（同名去重） |
| `diplomacy.forceSettle` | `enterWorld` 离世时按事件链已完成阶段比例强制折算，不留悬案 |
| `ledgerCheck.LEDGER_GUARD_HINT` | 领地演化阶段（领地有仓库进出与被动产出，正是三条护栏的适用面） |

> 门禁：`npm test` **2123 例全绿** · `tsc` **0 错** · `lint` 0 error · `build` ✓ · dist 预览启动正常 ✓
> 复审：重跑导出消费审计，11 个关键函数**全部**在 App.tsx 有真实调用点。

---

## 全部完成

第 0~4 批 + 收尾接线，共新增 **13 个纯逻辑模块 + 约 350 例单测 + 10 条可编辑提示词**，
全部遵守同一条哲学：**能前端算的一律前端算死，AI 只写散文**。
剩余未做的只有卡里那些被明确判定为**不该移植**的部分（见 `WORLD_ENGINE_PORT_PLAN.md` §2.3）。

**唯一仍缺的验证**：端到端实机跑一轮演化（需可用的正文 API）。建议首次实跑时重点看两件事——
① 十条新注入块的实际 token 开销；② 各阶段的提示词是否过长导致 AI 抓不住重点。必要时按玩法取舍关掉几条。
