...
> **P3（2026-08-02）：TT 预设内嵌资产完整承接**——`extensions.tauritavern` 的 skill 包（zip·`miniZip.ts` 零依赖解包）与 agent 档案导入 `agentSkillStore`（`drpg-agentskills`）；`skill_list/search/read` 三工具（20k/60k 预算）；`agent_list`+`agent_delegate` **同步**委派子代理（TT 的后台并发/`agent_await` 不移植——await 留兼容桩、handoff 软拒绝；子代理共享父工作区、`task_return` 收尾、未知工具软回喂、独立路由 `agentSub-<id>`）；主档案 instructions → `writerNotes[预设名]` 追加进系统提示词（带同步适配说明）。⚠ 同日事故与回退：`parseSTPreset` 默认忠实化崩过玩家预设正文格式——忠实语义降为 **opt-in 第 4 参**（仅两枚 Agent 内置预设补种用），默认只保留「多 prompt_order 优先 character_id=100001」。
