...
### Giải thích từng nút ở đáy giao diện (mấy nút nhỏ trong ảnh chụp)

Phía dưới khu vực nội dung chính, từ trên xuống dưới có tổng cộng **bốn hàng**, hàng nào cũng hữu ích, giải thích từng cái:

**Hàng 1 · 📋 Thanh Lệnh Trạng Thái** (hiển thị là `📋 Lệnh Trạng Thái Lượt · N lượt`)
- **Bấm vào để mở/đóng khung "Chọn Thế Giới · Kết Toán Nhiệm Vụ"** —— muốn đi thế giới mới, hoặc làm **kết toán nhiệm vụ** cho thế giới nhiệm vụ hiện tại, đều vào từ đây.
- Thanh này lúc bình thường còn kiêm làm **thanh tiến độ tiến hóa**: quản lý vật phẩm, tiến hóa Nhân vật chính/NPC/Thế Lực/Lãnh Địa/Đội Phiêu Lưu/Vạn Tộc/Linh tinh, hồi tưởng ký ức, sinh ảnh…… đang chạy bước nào, sẽ **cuộn hiển thị theo thời gian thực** trên thanh này (như "Đang xử lý quản lý vật phẩm…", "Đang xử lý tiến hóa NPC…"). Thấy nó đang quay, tức là hệ thống đang ghi sổ giúp bạn ở hậu trường.
- Ngoài cùng bên phải (chỉ xuất hiện sau khi đã tạo một lần) có mấy **nút gỡ lỗi**: `🛠 Nhà phát triển` (xem prompt đầy đủ thực sự gửi cho AI ở lượt này), `🧠 Xem ký ức đã chèn`, `Xem đầu vào`, `Xem phản hồi`. Rất hữu ích khi muốn biết AI thực sự nhận được gì, hoặc để tìm lỗi.

**Hàng 2 · Hàng thao tác lượt này** (chỉ xuất hiện sau khi đã gửi ít nhất một lượt; trên điện thoại nó nằm trong "⚙ Thao tác lượt này ▾")
- **⟳ Tạo lại** —— Không hài lòng với nội dung chính lượt này, hủy lượt này (gồm toàn bộ tiến hóa) và **viết lại** bằng cùng một nội dung nhập (sẽ tải lại trang, cần xác nhận).
- **↩ Quay lại lượt trước** —— Hủy nội dung chính và toàn bộ tiến hóa của lượt này, **quay về trạng thái lúc kết thúc lượt trước** (tải lại trang, cần xác nhận). Đây mới là cách "hủy" an toàn.
- **♻ Tính lại biến** —— Hài lòng với nội dung chính nhưng **một mục dữ liệu nào đó bị sai / bị thiếu**? Bấm vào để mở "menu Roll lại", có thể **chỉ chạy lại một tiến hóa nhất định** (Vật phẩm, Thuộc Tính nhân vật chính, NPC, Thế Lực, Lãnh Địa, Đội Phiêu Lưu, Vạn Tộc, Nhiệm Vụ, Ký Ức, Sinh Ảnh), cũng có thể chọn "Toàn bộ biến" để tính lại tất cả. Các dữ liệu khác không đổi.
- **🔊 Đọc to / ⏹ Dừng đọc** (nút ⚙ bên cạnh là cài đặt giọng nói) —— Đọc to nội dung chính lượt này (xem chi tiết Chương 10 TTS).
- **⛔ Dừng tạo** —— Một cú bấm dừng **toàn bộ tiến hóa và sinh ảnh đang chạy** (dùng khi bị kẹt, hoặc không muốn chờ).
  > 🧭 **Quy tắc sắt**: ba nút ⟳ / ↩ / ♻ phụ thuộc vào "văn bản gốc vừa được tạo ra của lượt này", **hễ tải lại trang hoặc tải bản lưu là mất hiệu lực** (văn bản gốc chỉ nằm trong bộ nhớ). Muốn dùng thì dùng sớm, đừng đóng trang trước.

**Hàng 3 · 📌 Khung Prompt tiền đặt** (bấm `▶ 📌 Prompt tiền đặt` để mở)
- Những gì bạn viết ở đây sẽ **được chèn vào phần sâu nhất của câu chuyện ở mỗi lượt, trọng số cao nhất**, dùng để ràng buộc AI trong thời gian dài (ví dụ "giữ phong cách u tối", "đừng để nhân vật phụ xoay quanh tôi", "luôn viết ở ngôi thứ hai"). **Luôn có hiệu lực, không cần gõ lại mỗi lượt.**
- Các **thao tác ngoài cảnh** của bạn (cộng điểm / chế tạo / cường hóa / tiêu tiền…) cũng sẽ tự động được thêm bên dưới nó, làm thông báo "ngoài cảnh lượt này", để nội dung chính khớp với những gì bạn làm trong bảng, không kết toán lặp lại.

**Hàng 4 · Hàng ô nhập liệu** (từ trái sang phải)
- 🔴 **Nút ↺ màu đỏ = Xóa hội thoại** —— ⚠️ **Cẩn thận**: nó sẽ xóa các tin nhắn hội thoại đang hiển thị trên màn hình, làm khu nội dung chính trống trơn. **Nó KHÔNG phải "Quay lại", cũng không xóa bản lưu**, nhưng xóa xong thì hội thoại hiện tại không còn hiển thị. Muốn hủy an toàn thì dùng **↩ Quay lại** ở Hàng 2, đừng bấm nhầm nút đỏ này.
- **Ô nhập liệu** —— Chỗ gõ chữ (`Nhập hành động của bạn tại đây…`). Enter để gửi, `Shift+Enter` để xuống dòng (có thể đổi thành "Enter xuống dòng, bấm nút để gửi" trong cài đặt).
- **↵** —— Chèn xuống dòng thủ công (giống `Shift+Enter`, tiện trên điện thoại).
- ⏩ **Màu xanh · Thúc đẩy cốt truyện** —— **Không cần gõ chữ**, để cốt truyện **tự nhiên tiến thêm một nhịp** theo cục diện, mạch dẫn và phục bút hiện tại. Cực hữu ích khi lười nghĩ cách viết, hoặc chỉ muốn đẩy nhịp truyện (bản nháp trong ô nhập của bạn vẫn được giữ lại).
- 🔁 **Tự động thúc đẩy vòng lặp** —— Tự động đẩy liền mấy nhịp (số lần / khoảng cách chỉnh trong cài đặt); hễ bạn gửi nội dung mới thì nó tự dừng. Dùng để "xem kịch" tự động.
- ▶ **Màu thần · Gửi** —— Gửi hành động bạn đã nhập (đang tạo sẽ chuyển thành biểu tượng quay).

### Xúc xắc, ngôi kể, và ba chế độ "thúc đẩy"

- **🎲 Lắc xúc xắc** —— Khi cần "phán định vận mệnh" (có cạy được khóa không, thuyết phục có thành công không), tự tay lắc xúc xắc, kết quả cùng với hiệu chỉnh Thuộc Tính/Kỹ Năng của bạn được đưa vào cho AI, khiến thành bại có tính ngẫu nhiên và kịch tính hơn. Có thể chuyển giữa hai chế độ "công cụ xác định" hoặc "AI làm trọng tài".
